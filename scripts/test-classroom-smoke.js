...
function assertGraphConceptDirectAnswer(response, { name, answerMatch }) {
  assert.equal(response.body.routeType, 'graph_concept', `${name} should answer directly as a graph concept`);
  assert.notEqual(
    response.body.routeType,
    'motion_force_knowledge_tutor',
    `${name} should not start Guided General Tutor`
  );
  assert.ok(!response.body.tutor, `${name} should not include tutor metadata`);
  assert.match(response.body.response, answerMatch, `${name} direct answer`);
}
...
async function testDistanceTimeSlopeDirectAnswer() {
  const classroom = await createClassroom({ studentGuidedFormulaTutoringEnabled: true });
  const studentHubId = 'distance-time-slope-direct';
  const question = 'On a distance vs. time graph, the slope of the line equals the object’s';

  const answer = await send(classroom, studentHubId, question);
  assertGraphConceptDirectAnswer(answer, {
    name: 'distance-time-slope',
    answerMatch: /speed/i
  });
  assert.equal(currentTutorProblem(classroom, studentHubId), null, 'distance-time slope should not create tutor state');
}

async function testVelocityTimeSlopeDirectAnswer() {
  const classroom = await createClassroom({ studentGuidedFormulaTutoringEnabled: true });
  const studentHubId = 'velocity-time-slope-direct';
  const question = 'On a velocity vs. time graph, the slope of the line equals the object’s';

  const answer = await send(classroom, studentHubId, question);
  assertGraphConceptDirectAnswer(answer, {
    name: 'velocity-time-slope',
    answerMatch: /acceleration/i
  });
  assert.equal(currentTutorProblem(classroom, studentHubId), null, 'velocity-time slope should not create tutor state');
}
...
async function main() {
  ...
  await testDistanceTimeSlopeDirectAnswer();
  await testVelocityTimeSlopeDirectAnswer();
