...
function tryElectricityVocab(message) {
  ...
  if (shouldLeaveForThermalEnergyVocab(text)) return null;
  if (shouldLeaveForGeneralEnergyVocab(text)) return null;
...
function findConcept(text) {
  if (looksLikeClosedCircuitElectronFlow(text)) {
    return localConcept('closed_circuit_electron_flow', 'In a closed circuit, electrons move continuously around the complete path. The battery provides the voltage difference that pushes charge, the wires provide the conducting path, and the lamp is the load that converts electrical energy primarily into light and thermal energy.');
  }
  if (hasPhrase(text, 'battery') && /\belectrons?\b/.test(text) && /\bnegative\b/.test(text) && /\bpositive\b/.test(text)) {
    return localConcept('battery_electron_flow_negative_positive', 'A battery uses chemical reactions to create a voltage difference between its negative and positive terminals. In a closed circuit, that voltage difference pushes charges; electron flow is from negative to positive.');
  }
  if (hasPhrase(text, 'electric circuit') && /\bwhat\s+happens\b/.test(text)) {
    return vocabById('em_vocab_028');
  }
  ...
  if (hasPhrase(text, 'battery') && /\bpositive\b/.test(text) && /\bnegative\b/.test(text)) {
    return conceptById('em_concept_009');
  }
...
function answerDirectElectricityPrompt(text) {
  if (asksCurrentResistanceComparison(text)) {
    return localConcept('electric_current_vs_resistance', 'Electric current is the movement, or rate of flow, of electric charge and is measured in amperes (amps). Resistance is the opposition to charge flow and is measured in ohms.');
  }
  if (hasPhrase(text, 'charging by friction')) {
    return localVocab('electric_charge_friction', 'Friction transfers electric charge when two objects rub together. Electrons move during rubbing, which can build up static electricity.');
  }
  ...
  if (isAskingTerm(text, ['series circuit'])) return vocabById('em_vocab_038');
  if (isAskingTerm(text, ['parallel circuit'])) return vocabById('em_vocab_039');
...
function looksLikeClosedCircuitElectronFlow(text) {
  const hasChargeMovement = /\b(?:electrons?|charge)\b/.test(text) &&
    /\b(?:flow|flows|move|moves|moving|travel|travels|traveling)\b/.test(text);
  const hasCompleteCircuit = hasPhrase(text, 'closed circuit') || hasPhrase(text, 'complete circuit');
  const hasRequiredComponents = hasPhrase(text, 'battery') &&
    /\bwires?\b/.test(text) &&
    /\b(?:lamp|light bulb|bulb)\b/.test(text);
  return hasChargeMovement && hasCompleteCircuit && hasRequiredComponents;
}

function asksCurrentResistanceComparison(text) {
  return hasPhrase(text, 'current') &&
    hasPhrase(text, 'resistance') &&
    /\b(?:compare|comparison|difference|different|vs|versus)\b/.test(text);
}
...
function shouldLeaveForThermalEnergyVocab(text) {
  return /\b(?:thermal|heat|temperature)\b/.test(text) &&
    /\b(?:conductor|conductors|insulator|insulators|conduction)\b/.test(text);
}

function shouldLeaveForGeneralEnergyVocab(text) {
  return isAskingTerm(text, ['power']) &&
    !/\b(?:electrical|electric|electricity|circuit|voltage|current|battery|bulb|watts?|watt)\b/.test(text);
}
