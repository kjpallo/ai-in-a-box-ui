...
const { makeRoute } = require('../router/answerBuilder');
const {
  buildQuestionContract,
  deepFreeze
} = require('../router/questionContract');
const { applyRouteValidation } = require('../router/answerContractValidator');
...
function createQuestionAnswerService({
  ...
  function getRelevantKnowledge(message, questionContract) {
    return findRelevantKnowledge(
      message,
      getStudentAnswerKnowledge(),
      maxKnowledgeItems,
      { questionContract }
    );
  }
  ...
  function getStudentAnswerKnowledge() {
    return (Array.isArray(teacherKnowledge) ? teacherKnowledge : [])
      .filter((item) => (
        !isApprovedKnowledgeItem(item) ||
        isScienceApprovedKnowledgeItem(item)
      ));
  }

  function routeMessage(message, options = {}) {
    const questionContract = buildQuestionContract(message, options);
    reloadTeacherKnowledge();

    if (questionContract.missingContext.length > 0) {
      const guardedCandidate = buildMissingContextQuestionRoute(questionContract);
      const guarded = applyRouteValidation({
        message,
        contract: questionContract,
        route: guardedCandidate,
        matchedKnowledge: [],
        stage: 'pre_retrieval_guard',
        candidateOrigin: 'contract_guard'
      });
      const questionRoute = attachQuestionContract(
        guarded.route,
        questionContract,
        guarded.validation
      );
      return {
        ...
        graphContext: null,
        questionContract,
        answerValidation: guarded.validation
      };
    }

    const matchedKnowledge = getRelevantKnowledge(message, questionContract);
    const candidateRoute = routeStudentQuestion(
      message,
      matchedKnowledge,
      questionContract
    );
    const graphContext = getGraphContext(message, knowledgeGraph);
    const validated = validateCandidateRoute({
      message,
      questionContract,
      candidateRoute,
      matchedKnowledge,
    ...
    // Graph context is independently assembled by the service. Attach it only
    // after candidate validation so route-owned metadata cannot establish graph
    // authority, while validated no-match routes can still support graph-based
    // answering and tutoring downstream.
    attachGraphContextToRoute(validated.questionRoute, graphContext);
    return {
      matchedKnowledge,
      questionRoute: validated.questionRoute,
      graphContext,
      questionContract,
      answerValidation: validated.answerValidation
    };
  }

  function validateCandidateRoute({
    message,
    questionContract,
    candidateRoute,
    matchedKnowledge,
    stage,
    validFallbackRoute = null,
    candidateOrigin = 'untrusted',
    graphContext = null
  }) {
    const result = applyRouteValidation({
      message,
      contract: questionContract,
    ...
    if (result.rejectedRoute) {
      logAiImprovementProblem({
        message,
        questionRoute: result.rejectedRoute,
        matchedKnowledge,
        answerGiven: result.rejectedRoute.directAnswer || '',
        category: 'answer_contract_rejection',
        reason: result.validation.reasons.join(', ') || result.validation.reason,
        source: 'answer_validator',
        debug: {
          validationStage: stage,
          answerValidation: result.validation,
          rejectedCandidate: summarizeRejectedCandidate(result.rejectedRoute),
          questionContract: summarizeQuestionContract(questionContract)
        }
      });
    }

    const route = result.rejectedRoute && validFallbackRoute
      ? validFallbackRoute
      : result.route;
    const answerValidation = result.rejectedRoute && validFallbackRoute
      ? validFallbackRoute.answerValidation
      : result.validation;

    return {
      questionRoute: attachQuestionContract(
        route,
        questionContract,
        answerValidation
      ),
      answerValidation,
      rejectedRoute: result.rejectedRoute
    };
  }

  function finalizeImmediateResponse({
    message,
    contextMessage = message,
    responsePayload = {},
    candidateRoute = responsePayload.questionRoute,
    missingContext = [],
    stage = 'resolved_followup'
  }) {
    const questionContract = buildResolvedFollowUpContract(
      contextMessage,
      missingContext
    );
    const preparedRoute = prepareResolvedFollowUpRoute(
      candidateRoute,
      questionContract,
      stage
    );
    const validated = validateCandidateRoute({
      message,
      questionContract,
      candidateRoute: preparedRoute,
      matchedKnowledge: [],
      stage,
      candidateOrigin: 'resolved_followup'
    });
    const questionRoute = validated.questionRoute;
    ...
    return {
      ...
      response: questionRoute.directAnswer ||
        'I do not have a trusted answer for that yet. Please ask your teacher.',
      routeType: questionRoute?.public?.type || questionRoute?.type || 'unknown',
      confidence: questionRoute.confidence,
      questionRoute,
      questionContract,
      answerValidation: validated.answerValidation,
      pendingClarification: responsePayload.pendingClarification || null
    };
  }

  async function answerStudentMessage(message, options = {}) {
    const preflightContract = buildQuestionContract(message, options);
    if (preflightContract.missingContext.length > 0) {
      const guarded = routeMessage(message, options);
      maybeLogReviewQuestion({
        message,
        questionRoute: guarded.questionRoute,
        matchedKnowledge: guarded.matchedKnowledge,
        answerGiven: guarded.questionRoute.directAnswer || ''
      });
      return {
        response: guarded.questionRoute.directAnswer,
        routeType: guarded.questionRoute?.public?.type ||
          guarded.questionRoute?.type ||
          'missing_context',
        confidence: guarded.questionRoute.confidence,
        questionRoute: guarded.questionRoute,
        questionContract: guarded.questionContract,
        answerValidation: guarded.answerValidation,
    ...
    if (contextCarryover.needsContext && !contextCarryover.topic) {
      const clarification = buildContextClarificationResponse();
      return finalizeImmediateResponse({
        message,
        contextMessage: options.lastAnsweredPrompt || message,
        responsePayload: clarification,
        missingContext: ['topic'],
        stage: 'context_clarification'
      });
    }
    ...
    if (options.intent === 'why_this_matters' || isWhyThisMattersFollowUp(message)) {
      ...
      if (!whyThisMatters.matched) {
        responsePayload.questionRoute.type = 'missing_context';
        responsePayload.questionRoute.public.type = 'missing_context';
      }
      return finalizeImmediateResponse({
        message,
        contextMessage: contextCarryover.topic || options.lastAnsweredPrompt || message,
        responsePayload,
        missingContext: whyThisMatters.matched ? [] : ['topic'],
        stage: 'why_this_matters_followup'
      });
    }
    ...
    if (clarificationFollowUp?.handled) {
      return finalizeImmediateResponse({
        message,
        contextMessage: options.lastAnsweredPrompt || options.lastAnsweredAnswer || message,
        responsePayload: {
        ...
        missingContext: clarificationFollowUp.pendingClarification
          ? ['clarification_choice']
          : [],
        stage: 'pending_clarification_followup'
      });
    }
    ...
    if (circuitDiagramFollowUp) {
      return finalizeImmediateResponse({
        message,
        contextMessage: `circuit diagram ${message}`,
        responsePayload: circuitDiagramFollowUp,
        stage: 'circuit_diagram_followup'
      });
    }
    ...
    if (standardsFollowUp?.handled) {
      return finalizeImmediateResponse({
        message,
        contextMessage: options.lastAnsweredPrompt || routedMessage,
        responsePayload: {
        ...
        missingContext: standardsFollowUp.pendingClarification
          ? ['clarification_choice']
          : [],
        stage: 'standards_followup'
      });
    }

    const {
      matchedKnowledge,
      questionRoute: routedQuestionRoute,
      graphContext,
      questionContract,
      answerValidation: routedAnswerValidation
    } = independentRouting || routeMessage(routedMessage, options);
    let questionRoute = routedQuestionRoute;
    let answerValidation = routedAnswerValidation;
    const graphAnswer = canAnswerWithGraph(routedMessage, questionRoute, graphContext)
      ? buildGraphAnswer(routedMessage, graphContext)
      : null;
    if (graphAnswer) {
      const graphCandidate = buildGraphQuestionRoute(questionRoute, graphAnswer);
      const validatedGraph = validateCandidateRoute({
        message: routedMessage,
        questionContract,
        candidateRoute: graphCandidate,
        matchedKnowledge,
        stage: 'graph_answer',
        validFallbackRoute: questionRoute,
        candidateOrigin: 'graph_builder',
        graphContext
      });
      questionRoute = validatedGraph.questionRoute;
      answerValidation = validatedGraph.answerValidation;
    }
    ...
    if (graphRoutingSupportAnswer) {
      const graphSupportCandidate = buildGraphRoutingSupportQuestionRoute(
        questionRoute,
        graphRoutingSupportAnswer
      );
      const validatedGraphSupport = validateCandidateRoute({
        message: routedMessage,
        questionContract,
        candidateRoute: graphSupportCandidate,
        matchedKnowledge,
        stage: 'graph_support',
        validFallbackRoute: questionRoute,
        candidateOrigin: 'graph_builder',
      ...
      questionRoute = validatedGraphSupport.questionRoute;
      answerValidation = validatedGraphSupport.answerValidation;
    }
    ...
    if (!response || questionRoute.aiAllowed) {
      ...
      const aiCandidate = {
        ...
        toolsUsed: uniqueRouteTools([
          ...(questionRoute.toolsUsed || []),
          'ollama_ai_fallback'
        ]),
        answerTopics: [],
        answerConcepts: [],
      ...
      const validatedAi = validateCandidateRoute({
        message: routedMessage,
        questionContract,
        candidateRoute: aiCandidate,
        matchedKnowledge,
        stage: 'ai_fallback',
        candidateOrigin: 'ai',
        graphContext
      });
      questionRoute = validatedAi.questionRoute;
      answerValidation = validatedAi.answerValidation;
      response = questionRoute.directAnswer || '';
    }
...
function buildResolvedFollowUpContract(contextMessage, missingContext = []) {
  const base = buildQuestionContract(
    `Explain this resolved classroom follow-up: ${String(contextMessage || '').trim()}`
  );
  const missing = uniqueRouteTools(missingContext);
  const candidateTopics = Array.isArray(base.candidateTopics)
    ? base.candidateTopics.slice()
    : ['unknown'];
  ...
  return deepFreeze({
    ...
    targetConcept: '',
    suppliedChoices: [],
    missingAnswerChoices: false,
    numericalGivens: [],
    requestedUnits: [],
    requiredAnswerForm: missing.length > 0
      ? 'clarification'
      : 'contextual_response',
    unseenDiagramRequired: false,
    missingContext: missing,
    evidenceRequirements: {
      ...base.evidenceRequirements,
      minimumEvidenceItems: missing.length > 0 ? 0 : 1,
      trustedEvidenceRequired: missing.length === 0,
      compatibleTopicRequired: false,
      targetConceptSupportRequired: false,
      requiredConcepts: [],
      requiredUnits: [],
      clarificationRequired: missing.length > 0,
...
function prepareResolvedFollowUpRoute(route, questionContract, stage) {
  const candidate = route && typeof route === 'object' ? route : {};
  const topics = (questionContract?.candidateTopics || [])
    .filter((topic) => topic && topic !== 'unknown');
  const toolsUsed = uniqueRouteTools([
    ...(candidate.toolsUsed || []),
  ...
  const evidence = candidate.evidence && typeof candidate.evidence === 'object'
    ? candidate.evidence
    : {
      quality: candidate.confidence === 'strong' ? 'high' : 'low',
      trust: 'local_rule',
      source: stage || 'resolved_followup',
      topics,
      concepts: []
    };
  ...
  return {
    ...candidate,
    toolsUsed,
    answerTopics: Array.isArray(candidate.answerTopics)
      ? candidate.answerTopics
      : topics,
    answerConcepts: Array.isArray(candidate.answerConcepts)
      ? candidate.answerConcepts
      : [],
    evidence,
    aiAllowed: false,
    public: {
      ...(candidate.public || {}),
      type: candidate.public?.type || candidate.type || 'contextual_followup',
      toolsUsed,
      aiAllowed: false
    }
  };
...
function buildMissingContextQuestionRoute(questionContract) {
  const missingContext = Array.isArray(questionContract?.missingContext)
    ? questionContract.missingContext
    : [];
  let directAnswer =
    'I need more information before I can answer safely. Please provide the missing context.';

  if (missingContext.includes('answer_choices')) {
    directAnswer =
      'I need the answer choices before I can choose safely. Please provide all of the choices.';
  } else if (missingContext.includes('diagram')) {
    directAnswer =
      'I need to see the referenced diagram before I can answer safely. Please upload or share the diagram, or describe the circuit and its labels.';
  } else if (missingContext.includes('cloze_context')) {
    directAnswer =
      'I need the full sentence or topic before I can fill in the blank safely. Please provide that context.';
  }

  return makeRoute({
    type: 'missing_context',
    confidence: 'none',
    toolsUsed: ['question_contract_guard'],
    notes: `Question contract requires missing context: ${missingContext.join(', ') || 'unspecified'}.`,
    directAnswer,
    answerTopics: [],
    answerConcepts: [],
    evidence: {
      quality: 'high',
      trust: 'question_contract_guard',
...
function attachQuestionContract(route, questionContract, answerValidation) {
  if (!route || typeof route !== 'object') return route;
  return {
    ...route,
    questionContract,
    answerValidation: answerValidation || route.answerValidation || null
  };
}

function summarizeRejectedCandidate(route = {}) {
  return {
    type: String(route.type || route.public?.type || ''),
    originalConfidence: String(route.confidence || route.public?.confidence || ''),
    toolsUsed: uniqueRouteTools([
      ...(route.toolsUsed || []),
      ...(route.public?.toolsUsed || [])
    ]),
    answerTopics: Array.isArray(route.answerTopics) ? route.answerTopics.slice() : [],
    answerConcepts: Array.isArray(route.answerConcepts) ? route.answerConcepts.slice() : [],
    formulaId: String(route.formulaWork?.formulaId || ''),
    solveFor: String(route.formulaWork?.solveFor || '')
  };
}

function summarizeQuestionContract(contract = {}) {
  return {
    taskType: String(contract.taskType || ''),
    candidateTopics: Array.isArray(contract.candidateTopics)
      ? contract.candidateTopics.slice()
      : [],
    targetConcept: String(contract.targetConcept || ''),
    requestedUnits: Array.isArray(contract.requestedUnits)
      ? contract.requestedUnits.slice()
      : [],
    missingContext: Array.isArray(contract.missingContext)
      ? contract.missingContext.slice()
      : []
  };
}
...
function buildGraphQuestionRoute(questionRoute, graphAnswer) {
  const toolsUsed = ['knowledge_graph'];
  const graphMetadata = buildGraphAnswerMetadata(graphAnswer);
  ...
  return {
    ...
    directAnswer: graphAnswer.directAnswer,
    answerTopics: graphMetadata.topics,
    answerConcepts: graphMetadata.concepts,
    evidence: graphMetadata.evidence,
    aiAllowed: false,
    graphAnswer,
    public: {
      ...(questionRoute.public || {}),
...
function buildGraphRoutingSupportQuestionRoute(questionRoute, graphRoutingSupportAnswer) {
  const toolsUsed = ['knowledge_graph'];
  const graphMetadata = buildGraphAnswerMetadata(graphRoutingSupportAnswer);
...
function buildGraphAnswerMetadata(graphAnswer = {}) {
  const concepts = uniqueRouteTools([
    ...(Array.isArray(graphAnswer.connectedConcepts) ? graphAnswer.connectedConcepts : []),
    ...(Array.isArray(graphAnswer.prerequisiteConcepts) ? graphAnswer.prerequisiteConcepts : [])
  ]);
  const analyzed = buildQuestionContract(
    [concepts.join(' '), graphAnswer.directAnswer].filter(Boolean).join('. ')
  );
  const topics = (analyzed.candidateTopics || [])
    .filter((topic) => topic && topic !== 'unknown');
  const source = String(graphAnswer.source || 'approved_knowledge_graph');
