...
function sendStudentSessionAccessError(res, result, studentHubId = '') {
  const body = {
    error: result?.error || 'Student session not found.',
    code: result?.code || 'SESSION_NOT_FOUND'
  };
  if (result?.session && safeText(studentHubId)) {
    try {
      requireStudentSessionParticipant(result.session, studentHubId);
      body.reopenRequest = {
        pending: hasStudentSessionReopenRequest(result.session, studentHubId)
      };
    } catch {
      // Do not expose request state to a student outside this session.
    }
  }
  return res.status(Number(result?.statusCode || 404)).json(body);
}
...
function applyStudentSessionLifecycleAction(session, action, options = {}) {
  ...
  if (cleanAction === 'extend') {
    ...
    session.endedAt = null;
    session.expiresAt = new Date(now.getTime() + (minutes * 60 * 1000)).toISOString();
    clearStudentSessionReopenRequests(session);
  } else {
...
function createStudentSessionReopenRequest(session, studentHubId, options = {}) {
  const studentHub = requireStudentSessionParticipant(session, studentHubId);
  const status = refreshStudentSessionStatus(session, options.now);
  if (status === SESSION_STATUS_ACTIVE) {
    throw reopenRequestError(409, 'SESSION_ALREADY_ACTIVE', 'This classroom session is already open.');
  }

  const cleanStudentHubId = studentHub.studentHubId;
  if (hasStudentSessionReopenRequest(session, cleanStudentHubId)) {
    return { created: false };
  }

  if (!session.reopenRequests || typeof session.reopenRequests !== 'object') {
    session.reopenRequests = Object.create(null);
  }
  session.reopenRequests[cleanStudentHubId] = true;
  return { created: true };
}

function requireStudentSessionParticipant(session, studentHubId) {
  const cleanStudentHubId = safeText(studentHubId);
  const hasStudentHub = cleanStudentHubId
    && session?.anonymousHubs
    && Object.prototype.hasOwnProperty.call(session.anonymousHubs, cleanStudentHubId);
  const studentHub = hasStudentHub ? session.anonymousHubs[cleanStudentHubId] : null;
  if (!studentHub || safeText(studentHub.studentHubId) !== cleanStudentHubId) {
    throw reopenRequestError(
      403,
      'STUDENT_NOT_IN_SESSION',
      'This student is not a participant in that classroom session.'
    );
  }
  return studentHub;
}

function hasStudentSessionReopenRequest(session, studentHubId) {
  const cleanStudentHubId = safeText(studentHubId);
  return Boolean(
    cleanStudentHubId
    && session?.reopenRequests
    && Object.prototype.hasOwnProperty.call(session.reopenRequests, cleanStudentHubId)
    && session.reopenRequests[cleanStudentHubId] === true
  );
}

function countStudentSessionReopenRequests(session) {
  return Object.keys(session?.reopenRequests || {}).filter((studentHubId) => {
    try {
      requireStudentSessionParticipant(session, studentHubId);
      return hasStudentSessionReopenRequest(session, studentHubId);
    } catch {
      return false;
    }
  }).length;
...
function clearStudentSessionReopenRequests(session) {
  const clearedCount = countStudentSessionReopenRequests(session);
  if (session && Object.prototype.hasOwnProperty.call(session, 'reopenRequests')) {
    delete session.reopenRequests;
  }
  return clearedCount;
}

function serializeSessionReopenRequests(session) {
  const count = countStudentSessionReopenRequests(session);
  return {
    pending: count > 0,
    count
  };
...
function reopenRequestError(statusCode, code, message) {
  const error = lifecycleError(statusCode, message);
  error.code = code;
  return error;
}
