...
function canStartFormulaTutor(questionRoute) {
  ...
  return Boolean(
    ...
    !shouldBypassFormulaTutorForOpenEndedWork(formulaWork)
  );
}

function shouldBypassFormulaTutorForOpenEndedWork(formulaWork = {}) {
  const formulaId = String(formulaWork.formulaId || formulaWork.formulaKey || '').trim();
  if (formulaId !== 'net_force') return false;

  const firstStep = Array.isArray(formulaWork.steps) ? formulaWork.steps[0] : null;
  const firstPrompt = String(firstStep?.prompt || '').trim().toLowerCase();
  return firstStep?.type === 'text' &&
    /\bwhat\s+forces\s+are\s+given\b|\bwhat\s+direction\s+is\s+each\s+force\b/.test(firstPrompt);
}
...
function checkMultipleChoice(step, message) {
  ...
  if (correctChoice && normalizedChoiceNumber === String(correctChoice.number)) return { correct: true };
