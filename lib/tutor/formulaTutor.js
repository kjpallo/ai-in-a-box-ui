...
function answerFormulaTutorStep(currentTutorProblem, studentMessage) {
  ...
  if (command === 'stop_and_explain') {
    return {
      response: `No problem. I’ll stop the Guided Formula Tutor and explain it directly.\n\n${completionResponse(problem)}`,
      currentTutorProblem: null,
      completed: false,
      stopped: true
    };
  ...
  if (command === 'show_answer') {
    return {
      response: `Okay, I’ll stop the Guided Formula Tutor and give the direct answer.\n\n${completionResponse(problem)}`,
      currentTutorProblem: null,
      completed: false,
      stopped: true
    };
  ...
  if (looksLikeStuckMessage(message)) {
    const scaffold = buildStepScaffold(problem, step);
    return {
      response: scaffold,
...
function normalizeCommand(message) {
  ...
  if (normalized === 'i dont like this' || normalized === 'i do not like this') return 'stop_and_explain';
...
function checkQuantity(step, message) {
  ...
  if (
    !numbersClose(answerValue, expectedValue) &&
    !matchesRoundedNumber(message, expectedValue) &&
    !matchesOriginalQuantity(step, message) &&
    !matchesConvertedQuantity(step, message)
  ) {
    return { correct: false };
  }
...
function checkCalculation(step, message) {
  const expectedValue = Number(step.expectedValue);
  const answerValue = extractNumber(message);
  return { correct: numbersClose(answerValue, expectedValue) || matchesRoundedNumber(message, expectedValue) };
}
...
function buildQuantityConversionResponse(step, message) {
  if (!step?.conversionReminder || !step?.originalDisplay) return '';
  const label = quantityLabelFromStep(step);
  const acceptedDisplay = matchesOriginalQuantity(step, message) ? step.originalDisplay : step.expectedDisplay;
...
function isLikelyNewFormulaQuestionDuringTutor(message) {
  ...
  const asksForTarget = /\b(?:what|find|calculate|solve|determine|how)\b.+\b(mass|density|volume|speed|distance|time|force|acceleration|current|resistance|voltage|power|energy|work|momentum|wavelength|frequency)\b/i.test(text);
  const formulaKeywordMatches = normalized.match(/\b(mass|density|volume|speed|distance|time|force|acceleration|current|resistance|voltage|power|energy|work|momentum|wavelength|frequency|travels?|moves?|circuit|block|object|train|car|runner|student|machine)\b/g) || [];
...
function buildQuantityScaffold(problem, step) {
  ...
  const display = buildExpectedQuantityDisplay(step) || variable.display || step.expectedDisplay || '';
  const hint = getHint(step);
