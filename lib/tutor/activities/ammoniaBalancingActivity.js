...
function buildBalancingActivityMetadata(problem) {
  ...
  return {
    ...
    balancedNotSimplified: buildPublicBalancedNotSimplified(state),
    reductionProgress: buildPublicReductionProgress(state),
...
function buildBalancingVisualMetadata(stateInput) {
  return buildStudentBalancingVisualMetadata(stateInput);
}

function buildStudentBalancingVisualMetadata(stateInput) {
...
function buildPublicBalancedNotSimplified(state) {
  if (state.phase !== 'reduction' || !state.balancedNotSimplified) return null;
  return {
    factor: state.balancedNotSimplified.factor,
    selectedCoefficients: clone(state.balancedNotSimplified.selectedCoefficients)
  };
}

function buildPublicReductionProgress(state) {
  const progress = buildReductionProgress(state);
  ...
  const items = progress.items.map((item) => {
    const projected = {
      ...
      originalCoefficient: item.originalCoefficient,
      divisor: item.divisor,
      reduced: item.reduced
    };
    if (item.reduced) projected.reducedCoefficient = item.reducedCoefficient;
    return projected;
  });
  const projected = {
  ...
  if (progress.complete) {
    projected.reducedCoefficients = coefficientVector(
      AMMONIA_BALANCING_ACTIVITY.coefficientReduction.reducedCoefficients
    );
  }
  return projected;
}
