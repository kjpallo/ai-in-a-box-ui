...
function normalizeChoice(step, choice, index) {
  ...
  return {
    ...
    nextStepId: String(choice?.nextStepId || choice?.nextStep || '').trim(),
    completeTutor: choice?.completeTutor === true || choice?.complete === true
  };
}
...
function advanceProblem(problem, step, selectedChoice = null) {
  ...
  const nextStepIndex = getNextStepIndex(problem, selectedChoice);
  return touch({
    ...problem,
    completedSteps,
...
function getNextStepIndex(problem, selectedChoice = null) {
  if (selectedChoice?.completeTutor) return problem.steps.length;
  const nextStepId = String(selectedChoice?.nextStepId || '').trim();
  if (nextStepId) {
    if (/^(?:complete|done|finish)$/.test(nextStepId.toLowerCase())) return problem.steps.length;
    const matchingIndex = problem.steps.findIndex((step) => String(step?.id || '') === nextStepId);
    if (matchingIndex >= 0) return matchingIndex;
  }
  return problem.currentStepIndex + 1;
}
