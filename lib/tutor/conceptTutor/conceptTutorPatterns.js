...
function buildUnit1VariablesConceptTutorPattern(message) {
  ...
  const target = classifyUnit1VariableTarget(text);
  if (!scenario || !target || !scenario.answers[target]) return null;

  const answer = scenario.answers[target];
  const finalAnswer = getUnit1VariablesFinalAnswer(target, answer);
  ...
  return {
    ...
    originalQuestion: String(message || '').trim(),
    supportedExample: true,
    triggerGroup: `${scenario.id}:${target}`,
    steps: [
      {
        id: 'identify-experiment-part',
        type: 'choice',
...
function shouldStartUnit1VariablesTutor(text) {
  ...
  return Boolean(classifyUnit1VariableTarget(text));
}
...
function classifyUnit1VariableTarget(text) {
  if (/\b(?:independent\s+variable|independant\s+variable|independent\s+varible|changed\s+on\s+purpose|changed\s+variable|manipulated\s+variable|which\s+variable\s+is\s+changed|what\s+is\s+changed\s+on\s+purpose)\b/.test(text)) {
    return 'independent';
  }
  if (/\b(?:dependent\s+variable|dependant\s+variable|dependent\s+varible|measured\s+(?:as\s+)?(?:the\s+)?result|measured\s+variable|responding\s+variable|which\s+variable\s+is\s+measured|what\s+is\s+measured)\b/.test(text)) {
    return 'dependent';
  }
  if (/\b(?:constants?|controlled\s+variables?|controled\s+variables?|controlled\s+varible|kept\s+the\s+same|should\s+be\s+kept\s+the\s+same|make\s+the\s+test\s+fair)\b/.test(text)) {
    return 'constant';
  }
  return '';
}
...
function getUnit1VariablesStepPrompt(target) {
  if (target === 'dependent') return 'What part of the experiment is measured as the result?';
  if (target === 'constant') return 'What part should be kept the same to make the test fair?';
  return 'What part of the experiment is changed on purpose?';
}
