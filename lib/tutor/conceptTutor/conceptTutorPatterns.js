...
function hasSeriesCircuitClue(text) {
  return /\b(?:one\s+path|single\s+path|only\s+one\s+path|same\s+current\s+through\s+all|same\s+current|one\s+bulb\s+(?:goes\s+)?out\s+and\s+all\s+bulbs\s+(?:go\s+)?out|all\s+bulbs\s+(?:go\s+)?out|all\s+lights?\s+(?:go\s+)?out)\b/.test(text);
}

function hasParallelCircuitClue(text) {
  return /\b(?:more\s+than\s+one\s+path|multiple\s+paths?|many\s+paths?|branches?|branch|one\s+bulb\s+(?:goes\s+)?out\s+and\s+(?:the\s+)?others\s+stay\s+on|others\s+stay\s+on|other\s+bulbs\s+stay\s+on|other\s+lights?\s+stay\s+on)\b/.test(text);
}
...
function asksForEndothermicExothermicIdentification(text) {
  return hasEndothermicExothermicChoice(text) ||
    /\bwhich\s+is\s+(?:an?\s+)?(?:endothermic|exothermic)\b/.test(text) ||
...
function classifyEndothermicExothermicPrompt(text) {
  const whichTarget = text.match(/\bwhich\s+is\s+(?:an?\s+)?(endothermic|exothermic)\b/);
...
function hasEndothermicClue(text) {
  return /\b(?:melting|melts?|melting\s+ice|ice\s+melting|evaporation|evaporating|evaporates?|instant\s+cold\s+pack|cold\s+pack|absorbs?\s+(?:thermal\s+)?(?:energy|heat)|(?:energy|heat)\s+(?:is\s+)?absorbed|absorbed\s+from\s+(?:the\s+)?surroundings)\b/.test(text);
}

function hasExothermicClue(text) {
  return /\b(?:burning|burns?|fire|fire\s+burning|freezing|freezes?|freezing\s+water|water\s+freezing|hand\s+warmer|releases?\s+(?:thermal\s+)?(?:energy|heat)|(?:energy|heat)\s+(?:is\s+)?released|released\s+to\s+(?:the\s+)?surroundings)\b/.test(text);
}
...
function asksForMechanicalEnergyTypesIdentification(text) {
  return hasMechanicalEnergyTypesChoice(text) ||
    /\bwhich\s+is\s+(?:an?\s+)?(?:kinetic|gravitational\s+potential|elastic\s+potential)\b/.test(text);
}

function classifyMechanicalEnergyTypesPrompt(text) {
  const whichTarget = text.match(/\bwhich\s+is\s+(?:an?\s+)?(kinetic|gravitational\s+potential|elastic\s+potential)\b/);
...
function hasKineticEnergyClue(text) {
  return /\b(?:moving\s+car|rolling\s+ball|moving|motion|energy\s+of\s+motion|mechanical\s+kinetic\s+energy)\b/.test(text);
}

function hasGravitationalPotentialEnergyClue(text) {
  return /\b(?:book\s+on\s+a\s+shelf|ball\s+held\s+above\s+(?:the\s+)?ground|above\s+(?:the\s+)?ground|height|position\s+above\s+ground|stored\s+energy\s+(?:because|due)\s+of\s+height|stored\s+energy\s+(?:because|due)\s+of\s+position)\b/.test(text);
}

function hasElasticPotentialEnergyClue(text) {
  return /\b(?:stretched\s+rubber\s+band|compressed\s+spring|stretched|compressed|spring|rubber\s+band|stored\s+energy\s+in\s+something\s+stretched|stored\s+energy\s+in\s+something\s+compressed)\b/.test(text);
}
