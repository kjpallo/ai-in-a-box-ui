...
function buildUnit7AtomicStructureConceptTutorPattern(message) {
  ...
  if (!text || isPureUnit7AtomicDefinitionQuestion(text) || isUnit7AtomicFormulaPrompt(text) || isUnit7AtomicComparisonPrompt(text)) return null;
...
function classifyUnit7PeriodicClue(text) {
  ...
  let answer = '';
  let triggerGroup = '';
  if (/\bsame\s+group\b.*\b(?:similar|properties|react|act)\b|\bsimilar\b.*\bsame\s+group\b/.test(text)) {
