...
function shouldStartAccelerationTutor(text) {
  ...
  if (isDirectAccelerationSignPrompt(text)) return false;
...
function isAccelerationFormulaCalculationPrompt(text) {
  const asksToCalculate = /\b(?:calculate|find|solve|determine|compute)\b/.test(text) ||
    /\bwhat\s+is\s+(?:(?:the|your|its|his|her|their)\s+)?acceleration\b/.test(text) ||
    /\bwhat\s+acceleration\b/.test(text);
  const hasNumber = /\d/.test(text);
  const hasFormulaUnit = /\b(?:kg|kilograms?|n|newtons?|m\/s|m\/s2|m\/s\^2|m\/s²|km\/h|km\/hr|kilometers?\s+per\s+hour|seconds?|secs?|s\b|hours?|hrs?|h\b)\b/.test(text);
...
function isDirectAccelerationSignPrompt(text) {
  const hasBlank = /_{2,}|\bblank\b/.test(text);
  const asksSign = /\b(?:positive|negative|sign)\b/.test(text);
  return /\bacceleration\b/.test(text) && (hasBlank || asksSign) && /\b(?:slowing\s+down|slows?\s+down|speeding\s+up|speeds?\s+up)\b/.test(text);
}
...
function buildEndothermicExothermicConceptTutorPattern(message) {
  ...
  if (!classification) return null;
  const scenarioType = classifyEndothermicExothermicScenarioType(text);

  const finalAnswer = getEndothermicExothermicFinalAnswer(classification, scenarioType);
        ...
        choices: [
          {
            ...
            finalAnswer: getEndothermicExothermicFinalAnswer('endothermic', scenarioType)
          },
          {
            number: 2,
            ...
            finalAnswer: getEndothermicExothermicFinalAnswer('exothermic', scenarioType)
          }
        ],
        correctAnswer: classification,
        hints: [
...
function classifyEndothermicExothermicScenarioType(text) {
  const explicitlyIdentifiesChemicalReaction =
    /\bchemical\s+reaction\b/.test(text) ||
    /\b(?:endothermic|exothermic)\s+reaction\b/.test(text) ||
    /\b(?:an?|the|this)\s+reaction\s+(?:absorbs?|releases?|gives?\s+off|takes?\s+in)\b/.test(text);
  return explicitlyIdentifiesChemicalReaction ? 'chemical_reaction' : 'process';
}
...
function getEndothermicExothermicFinalAnswer(classification, scenarioType) {
  if (scenarioType !== 'chemical_reaction') {
    if (classification === 'exothermic') {
      return 'This is an exothermic process because it releases thermal energy to the surroundings.';
    }
    return 'This is an endothermic process because it absorbs thermal energy from the surroundings.';
  }
  if (classification === 'exothermic') {
    return 'This is an exothermic reaction because it releases thermal energy to the surroundings. The surroundings gain thermal energy and usually become warmer.';
  }
  return 'This is an endothermic reaction because it absorbs thermal energy from the surroundings. The surroundings lose thermal energy and usually become cooler.';
}
...
function buildUnit7AtomicStructureConceptTutorPattern(message) {
  ...
  if (!text ||
      shouldBlockMixedTargetValenceQuestion(message) ||
      isPureUnit7AtomicDefinitionQuestion(text) ||
      isUnit7AtomicFormulaPrompt(text) ||
      isUnit7AtomicComparisonPrompt(text) ||
      isUnit7AtomicOpenResponsePrompt(text)) return null;
...
const UNIT1_VARIABLE_SCENARIOS = [
  {
    id: 'sunlight-plant-height',
    patterns: [
      /\bsunlight\b.*\bplants?\b.*\b(?:height|tall(?:er|est)?)\b/
    ],
    choices: [
      { number: 1, label: 'Amount of sunlight', value: 'amount_of_sunlight' },
      { number: 2, label: 'Plant height', value: 'plant_height' },
