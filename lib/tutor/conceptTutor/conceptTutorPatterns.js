...
function classifyUnit1VariableTarget(text) {
  ...
  if (/\b(?:control\s+group|comparison\s+group|no\s+treatment|no\s+change|does\s+not\s+receive\s+the\s+treatment|did\s+not\s+receive\s+the\s+treatment|without\s+the\s+treatment)\b/.test(text)) {
    return 'control_group';
  }
  if (/\b(?:experimental\s+group|experiment\s+group|treatment\s+group|receives?\s+the\s+treatment|received\s+the\s+treatment|gets?\s+the\s+treatment|gets?\s+the\s+change)\b/.test(text)) {
