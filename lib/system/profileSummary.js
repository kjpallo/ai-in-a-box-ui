...
function getDailyQuestionSummary(date, now = new Date()) {
  ...
  const questions = loadProfileEntries()
    ...
    .map((entry) => ({
      ...
      primaryStandards: entry.primaryStandards,
      possibleStandards: entry.possibleStandards,
      standards: entry.standards,
      units: entry.units,
      conceptConfidence: entry.conceptConfidence,
      standardsConfidence: entry.standardsConfidence,
      possibleStandardsConfidence: entry.possibleStandardsConfidence,
      standardsBankId: entry.standardsBankId,
      standardsError: entry.standardsError,
      source: entry.source,
...
function normalizeEntry(entry, source) {
  ...
  const date = reportingDateKeyFromEntry(entry);
  ...
  return {
    ...
    matchedConcepts: normalizeObjectArray(entry.matchedConcepts),
    primaryStandards: normalizeObjectArray(entry.primaryStandards),
    possibleStandards: normalizeObjectArray(entry.possibleStandards),
    standards: normalizeObjectArray(entry.standards),
    units: normalizeTextArray(entry.units),
    conceptConfidence: firstText(entry.conceptConfidence),
    standardsConfidence: firstText(entry.standardsConfidence),
    possibleStandardsConfidence: firstText(entry.possibleStandardsConfidence),
    standardsBankId: firstText(entry.standardsBankId),
    standardsError: firstText(entry.standardsError),
    dedupeKey: [
      source,
