...
function buildStandardsSummaryReport(entries, options = {}) {
  ...
  const reportableEntries = (Array.isArray(entries) ? entries.filter(isPlainObject) : [])
    .filter(isReportableForStandards);
  const availableDates = Array.from(new Set(reportableEntries.map(reportingDateKeyFromEntry).filter(Boolean)))
    .sort()
    .reverse();
  const validEntries = reportableEntries
    .filter((entry) => !selectedDate || reportingDateKeyFromEntry(entry) === selectedDate);
  ...
  for (const rawEntry of validEntries) {
    const entry = enrichMissingStandardsMetadata(rawEntry);
    ...
    questions.push(compactQuestion(entry, {
      routeType,
      standardsConfidence,
      conceptConfidence,
      standards,
...
function enrichMissingStandardsMetadata(entry) {
  if (hasStandardsMetadata(entry)) return entry;

  const question = normalizeText(entry.studentQuestion || entry.question || entry.message);
  if (!question) return entry;

  return {
    ...entry,
    ...buildStandardsLogMetadata(question)
  };
}

function isReportableForStandards(entry) {
  return entry?.reportableForStandards !== false;
}

function hasStandardsMetadata(entry) {
...
function compactQuestion(entry, metadata) {
  const timestamp = normalizeText(entry.timestamp || entry.createdAt || entry.updatedAt || entry.date);
  const archived = entry.archived === true;
  const sessionId = normalizeText(entry.sessionId || entry.classSessionId);
  const archiveId = normalizeText(entry.archiveId);
  const archiveCsvFilename = normalizeText(entry.archiveCsvFilename);
  return {
    id: normalizeText(entry.id),
    timestamp,
    date: reportingDateKeyFromEntry(entry),
    sessionId,
    classSessionId: normalizeText(entry.classSessionId || entry.sessionId),
    sessionKey: sessionId || (archiveId ? `archive:${archiveId}` : '') || (archiveCsvFilename ? `archive-file:${archiveCsvFilename}` : ''),
    className: normalizeText(entry.className || entry.sessionLabel || entry.debug?.className),
    sessionLabel: normalizeText(entry.sessionLabel || entry.className || entry.debug?.className),
    topic: normalizeText(entry.topic || entry.category || entry.source) || 'other',
    source: normalizeText(entry.source),
    question: normalizeText(entry.studentQuestion || entry.question || entry.message),
    responsePreview: normalizeText(entry.responsePreview),
    routeType: metadata.routeType,
    confidence: normalizeText(entry.confidence),
    standardsConfidence: metadata.standardsConfidence,
    conceptConfidence: metadata.conceptConfidence,
    possibleStandardsConfidence: normalizeText(entry.possibleStandardsConfidence),
    units: metadata.units,
    primaryStandards: metadata.standards.map((standard) => ({
      standardId: standard.standardId,
      label: standard.label,
      unit: standard.unit
    })),
    ...
    matchedConcepts: metadata.concepts.map((concept) => ({
      id: concept.id,
      title: concept.title,
      type: concept.type,
      unit: concept.unit
    })),
    standardsBankId: normalizeText(entry.standardsBankId),
    standardsError: normalizeText(entry.standardsError),
    archived,
    completed: archived,
    archiveCreatedAt: normalizeText(entry.archiveCreatedAt),
    archiveId,
    archiveCsvFilename
  };
}
