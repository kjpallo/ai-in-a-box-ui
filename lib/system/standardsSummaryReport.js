...
function buildStandardsSummaryReport(entries, options = {}) {
  ...
  const availableDates = Array.from(new Set(reportableEntries.map(dateKeyFromEntry).filter(Boolean)))
    .sort()
    .reverse();
  const validEntries = reportableEntries
    .filter((entry) => !selectedDate || dateKeyFromEntry(entry) === selectedDate);
