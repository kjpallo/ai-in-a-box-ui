...
const DEFAULT_CLASSROOM_CONTROLS = Object.freeze({
  ...
  questionsStandardsAutoArchiveEnabled: false,
  questionsStandardsAutoArchiveInactiveMinutes: 30
});
...
function sanitizeClassroomControls(value = {}, options = {}) {
  ...
  for (const key of allowedKeys) {
    ...
    } else if (key === 'questionsStandardsAutoArchiveInactiveMinutes') {
      controls[key] = validateAutoArchiveInactiveMinutes(source[key]);
    } else {
...
function validateAutoArchiveInactiveMinutes(value) {
  const number = Number(value);
  if (!Number.isInteger(number) || number < 1 || number > 1440) {
    const error = new Error('Auto-archive inactivity minutes must be a whole number from 1 to 1440.');
    error.statusCode = 400;
    throw error;
  }
  return number;
