...
function classifyMatterExample(text) {
  ...
  if (/\b(?:lemonade|sugar water|saltwater|salt water|black coffee)\b/.test(text)) {
    ...
    return concept('homogeneous_examples', `${capitalize(item)} is a homogeneous mixture, or solution, because it is evenly mixed and the same throughout.`);
  }
  if (/\b(?:trail mix|fruit salad|mixed nuts|vegetable soup|pizza|salad dressing)\b/.test(text)) {
    const item = matchLabel(text, [
      ['trail mix', 'trail mix'],
    ...
    return concept('heterogeneous_examples', `${capitalize(item)} is a heterogeneous mixture because the parts are unevenly distributed, visible, or able to separate.`);
  }
  ...
  if (/\b(?:milk|paint|mayonnaise|mayo)\b/.test(text)) {
    const item = /\bpaint\b/.test(text) ? 'Paint' : /\bmayo|mayonnaise\b/.test(text) ? 'Mayonnaise' : 'Milk';
    return concept('colloid_examples', `${item} is a colloid, which is a heterogeneous mixture with small dispersed particles. It is not a homogeneous solution.`);
  }
...
function asksHomogeneousHeterogeneousDifference(text) {
  return /\bhomogeneous|homogenous\b/.test(text) && /\bheterogeneous|heterogenous\b/.test(text) && /\bdifference|vs|compare\b/.test(text);
}
