...
function answerConceptQuestion(text) {
  ...
  if (has(text, 'friction') && hasAny(text, ['type', 'types'])) {
...
function answerConcept3Question(text) {
  ...
  if (asksFrictionTypes(text)) {
    return response('science_concept', 'friction_types', 'Static friction, sliding friction, and rolling friction are the main types of friction when referring to motion.');
  }
