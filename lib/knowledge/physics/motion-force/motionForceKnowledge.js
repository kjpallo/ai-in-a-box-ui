...
function tryMotionForceKnowledge(message, options = {}) {
  ...
  if (!text || (looksNumericOrFormulaBased(text) && !asksWhyFirstLawIsLawOfInertia(text))) return null;

  const clozeAnswer = answerClozeQuestion(text);
...
function answerClozeQuestion(text) {
...
function hasBalancedUnbalancedComparisonIntent(text) {
  return has(text, 'balanced') &&
    has(text, 'unbalanced') &&
    (hasAny(text, ['affect', 'effect', 'different', 'differently', 'compare', 'contrast', 'explain']) || has(text, 'motion'));
}
...
function buildMotionForceTutorMetadata(type, id, directAnswer) {
  const metadataById = {
    ...
    inertia_vocab: {
      ...
      guidingQuestions: [
        'Is inertia about an object changing motion easily or resisting a change?',
