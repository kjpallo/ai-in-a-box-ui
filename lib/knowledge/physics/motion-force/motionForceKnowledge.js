...
function answerConceptQuestion(text) {
  ...
  if (has(text, 'acceleration') && has(text, 'different from speed')) {
    return response('science_concept', 'acceleration_speed_comparison', 'Speed tells how fast an object moves. Acceleration means velocity changes, so an object accelerates when it speeds up, slows down, or changes direction.');
  }

  if ((has(text, 'acceleration') || /\baccelerat(?:e|es|ing)\b/.test(text)) && (hasDefinitionIntent(text) || has(text, 'concept'))) {
...
function answerVocabQuestion(text) {
  ...
  if (hasSimpleGeneralVocabQuestion(text, 'speed')) {
    return response('definition', 'speed_vocab', 'Speed tells how fast an object moves; speed = distance / time.');
  }
