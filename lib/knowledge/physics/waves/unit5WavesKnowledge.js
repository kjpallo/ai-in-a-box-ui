...
function answerWaveConcept(text) {
  ...
  if (asksObjectWaterRefraction(text)) {
    return concept('object_water_refraction', 'Objects can look bent or broken in water because of refraction. Light changes speed and bends as it moves between air and water, so the object appears shifted or bent.');
  }
...
function asksObjectWaterRefraction(text) {
  return /\bwater\b/.test(text) &&
    /\b(?:object|objects|thing|things|straw|straws|pencil|pencils)\b/.test(text) &&
    (
      /\b(?:look|looks|appear|appears)\s+(?:bent|broken|shifted)\b/.test(text) ||
      /\b(?:bent|broken|shifted)\s+in\s+water\b/.test(text)
    );
}
