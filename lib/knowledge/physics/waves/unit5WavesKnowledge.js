...
function answerWaveConcept(text) {
  ...
  if (asksPencilWater(text)) {
    return concept('pencil_water_refraction', 'A pencil looks broken in water because of refraction. Light changes speed and bends as it moves between air and water, so the pencil appears shifted.');
  }
...
function asksPencilWater(text) {
  return /\bpencil\b/.test(text) &&
    /\b(?:look|looks|appear|appears)\s+broken\b/.test(text) &&
    /\bwater\b/.test(text);
}
