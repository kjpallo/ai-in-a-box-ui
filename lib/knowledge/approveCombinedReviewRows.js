...
const { archiveAcceptedDraftKnowledgePack, archiveDraftKnowledgePackFromReviewQueue } = require('./archiveAcceptedDraftKnowledgePack');
  ...
  if (mode === 'final_publish') {
    ...
    Array.from(acceptedTargetsByPack.keys()).forEach((targetDraftPackId) => {
      ...
      if (countVisibleDraftRows(draftRecord.pack) === 0) {
        // Selected/all-valid approval only archives a source draft after the
        // approved combined pack has been written and enabled. Drafts with any
        // remaining visible review rows stay active for later teacher review.
        const archived = archiveAcceptedDraftKnowledgePack(targetDraftPackId, {
          ...options,
          draftPacksDir,
          approvedPacksDir
        });
