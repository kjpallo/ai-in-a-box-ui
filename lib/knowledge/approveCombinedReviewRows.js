...
function approveCombinedReviewRows(input = {}, options = {}) {
  ...
  selectedRows.forEach((row) => {
    ...
    const blockers = getApprovedItemPromotionBlockers(row.section, item);
    if (blockers.length) {
      skipped.blocked += 1;
      return;
    }
  ...
  if (!acceptedRows.length) {
    return blocked('No selected rows were valid for combined approval.', {
      skipped
    });
  }
  ...
  const existingCombined = findExistingCombinedPack(approvedPacksDir, combinedContext.reviewBatchKey);
  ...
  Array.from(acceptedTargetsByPack.keys()).forEach((draftPackId) => {
    const draftRecord = draftCache.get(draftPackId);
    if (!draftRecord || !draftRecord.success) return;
    const removed = removeAcceptedRowsFromDraftPack(draftRecord.pack, acceptedTargetsByPack.get(draftPackId));
    if (!removed) return;
    const draftPath = resolvePackFilePath(draftPacksDir, draftPackId);
    fs.writeFileSync(draftPath, `${JSON.stringify(draftRecord.pack, null, 2)}\n`);
    updatedDraftPackIds.push(draftPackId);

    if (mode === 'all_valid' && countVisibleDraftRows(draftRecord.pack) === 0) {
