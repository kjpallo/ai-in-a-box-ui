...
const { DEFAULT_DRAFT_PACKS_DIR, findKnowledgePackFiles } = require('./loadDraftKnowledgePacks');
...
function archiveAcceptedDraftKnowledgePack(packId, options = {}) {
  ...
  const approvedRecord = approved.packs.find((record) => record.packId === safePackId);
  if (!approvedRecord) {
...
function findDraftPackRecord(packId, draftPacksDir) {
  const matches = [];
  findKnowledgePackFiles(draftPacksDir).forEach((sourcePath) => {
    try {
