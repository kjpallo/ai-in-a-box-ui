...
const PACKETS = [
  {
    ...
    touchpointPatterns: [/packSchema|knowledge pack schema|validateKnowledgePack/i]
  },
  {
...
function buildInventoryItem(config, testFiles = []) {
  ...
  const directKnowledgeFunctionName = findDirectKnowledgeFunctionName(exported);
  const sourceMetadataPresent = hasSourceMetadata(exported, sourcePack, facts);
  const coverage = detectTestCoverage(config, testFiles);
  const touchpoints = detectTouchpoints(config);
  const unitTitle = readUnitTitle(config, exported, sourcePack);
