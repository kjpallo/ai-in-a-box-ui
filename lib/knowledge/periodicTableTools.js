...
function tryPeriodicTable(message) {
  ...
  if (/\bwhy\b/.test(lower) || /\breactive\b/.test(lower)) return null;
...
function buildAnswer(element, target, isotopeMassNumber) {
  ...
  if (target === 'protons') {
    return [
      ...
      `${name} has atomic number ${atomicNumber}.`,
  ...
  if (target === 'elementIdentity') {
    const note = name === 'Hydrogen'
      ? 'Note: hydrogen gas is usually H2, but hydrogen itself is the element H.'
      : '';
    return [
      `${name} is an element with symbol ${symbol} and atomic number ${atomicNumber}.`,
      note
    ].filter(Boolean).join('\n');
  }
...
function periodicTarget(lower) {
  ...
  if (/\b(?:what\s+is|what's|define|tell\s+me\s+about)\b/.test(lower)) return 'elementIdentity';
...
function hasElementSymbol(text, symbol) {
  ...
  const directDefinition = `\\b(?:[Ww]hat\\s+is|[Ww]hat's|[Dd]efine)\\s+${escaped}\\??$`;
  return new RegExp(`${contextBefore}|${contextAfter}|${directDefinition}`).test(text);
}
