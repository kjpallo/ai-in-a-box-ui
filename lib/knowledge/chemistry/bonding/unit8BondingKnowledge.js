...
const UNIT8_BONDING_CONCEPTS = Object.freeze([
  conceptGroup('unit8.bonding.stability', 'Stability and Bonding', [
    ...
    'unit8.magnesium_oxygen_ionic_bond',
    'unit8.water_lewis_structure'
  ]),
...
const UNIT8_BONDING_FACTS = Object.freeze([
  ...
  fact('unit8.magnesium_oxygen_ionic_bond', 'magnesium and oxygen bond', ['magnesium oxygen ionic bond'], 'Magnesium and oxygen form an ionic bond. Magnesium loses two electrons and becomes Mg2+. Oxygen gains two electrons and becomes O2-. The oppositely charged ions attract.'),
  fact('unit8.water_lewis_structure', 'water Lewis structure', ['H2O Lewis structure', 'water electron dot structure'], 'In the Lewis structure of H2O, oxygen is the central atom. Water has two single O-H bonds, and oxygen has two lone pairs.'),
...
function tryUnit8BondingKnowledge(message) {
  ...
  if (!text || (!hasBondingContext(text) && !asksForWaterLewisStructure(text))) return null;
...
function answerUnit8Bonding(text, original) {
  if (asksAboutMagnesiumOxygenBond(text)) {
    return factAnswer(findFact('unit8.magnesium_oxygen_ionic_bond'));
  }
  if (asksForWaterLewisStructure(text)) {
    return factAnswer(findFact('unit8.water_lewis_structure'));
  }
  ...
  if (asksAboutOxygenMoleculeBond(text)) {
...
function asksAboutMagnesiumOxygenBond(text) {
  const namesBothElements = /\b(?:magnesium|mg)\b/.test(text) && /\b(?:oxygen|o)\b/.test(text);
  const asksBondRelationship = /\bbonds?\b|\bbonding\b|\bionic\b|\belectron(?:s)?\s+transfer\b|\btransfer(?:s|red|ring)?\s+electron(?:s)?\b|\bforms?\s+between\b|\bcombin(?:e|es|ed|ing)\b/.test(text);
  return namesBothElements && asksBondRelationship;
}

function asksForWaterLewisStructure(text) {
  const namesWater = /\bwater\b|\bh2o\b/.test(text);
  const namesLewisRepresentation = /\blewis(?: dot)? (?:structure|diagram)\b|\belectron dot(?: structure| diagram)?\b|\bdot (?:structure|diagram)\b|\b(?:draw|show|sketch)\b.*\b(?:structure|diagram)\b/.test(text);
  return namesWater && namesLewisRepresentation;
}

function asksAboutOxygenMoleculeBond(text) {
  const namesOxygenMolecule = /\bo2\b|\boxygen molecules?\b|\b(?:the )?two oxygen atoms\b|\boxygen\b.*\bitself\b/.test(text);
  return namesOxygenMolecule && /\bdouble bond\b|\bsharing\b|\bshare\b|\bbonds?\b|\bbonding\b/.test(text);
}
