...
const { buildUnit7AtomicStructurePacket } = require('./unit7AtomicStructurePacketAdapter');
...
const periodicElementNameMatches = periodicElements
  .map((element) => [normalize(element.name), element])
  .sort((a, b) => b[0].length - a[0].length);
const periodicElementSymbolMatches = periodicElements
  .map((element) => [String(element.symbol || ''), element])
  .filter(([symbol]) => symbol)
  .sort((a, b) => b[0].length - a[0].length);
const periodicElementFormulaSymbols = [...new Set(periodicElementSymbolMatches.map(([symbol]) => symbol.toLowerCase()))]
...
const UNIT7_ATOMIC_STRUCTURE_PACKET = buildUnit7AtomicStructurePacket({
...
function tryUnit7AtomicStructureKnowledge(message) {
  ...
  if (!text) return null;
  if (looksLikeAtomicCalculation(text) &&
      !looksLikeMultiTargetAtomicShape(text) &&
      !looksLikeValenceElectronCountQuestion(text) &&
      !looksLikeInvalidAtomicMassNumberContext(text)) return null;
  if (looksLikeWavePeriodPrompt(text)) return null;
  if (looksLikeUnit6MatterClassificationPrompt(text) &&
      !looksLikeMultiTargetAtomicShape(text) &&
      !looksLikeMixedTargetValenceQuestion(text)) return null;
  if (looksLikeUnit7ConceptTutorPrompt(text) &&
      !looksLikeRelationshipQuestion(text) &&
      !looksLikeMixedTargetValenceQuestion(text)) return null;
  if (looksLikeCircuitElectronPrompt(text)) return null;

  const answer = answerUnit7AtomicStructure(text, originalText);
...
function answerUnit7AtomicStructure(text, originalText) {
  if (looksLikeInvalidAtomicMassNumberContext(text)) {
    return noMatchAnswer('unit7.mass_number.unsupported_context');
  }

  const unsupportedMolecularValenceAnswer = answerUnsupportedMolecularValenceQuestion(text, originalText);
  if (unsupportedMolecularValenceAnswer) return unsupportedMolecularValenceAnswer;

  if (looksLikeMixedTargetValenceQuestion(text)) {
    return noMatchAnswer('unit7.element.valence_electrons.unsupported_mixed_target');
  }

  if (looksLikeMultiTargetAtomicShape(text) && !looksLikeMultiTargetAtomicQuestion(text)) {
    return noMatchAnswer('unit7.atom.multiple_calculations.unsupported_context');
  }

  const multiTargetAnswer = answerMultiTargetAtomicQuestion(text);
  if (multiTargetAnswer) return multiTargetAnswer;
  ...
  if (
    ...
    /\bgroups?\b/.test(text) &&
    /\bperiods?\b/.test(text)
  ) {
    return concept(
      'unit7.periodic_table_groups_and_periods',
      'The periodic table is organized into groups and periods. Groups are vertical columns; elements in a group have similar properties and the same number of valence electrons. Periods are horizontal rows; elements in a period have the same number of occupied energy levels.'
    );
  }
  ...
  const particleQuestionAnswer = answerSubatomicParticleQuestion(text);
  if (particleQuestionAnswer) return particleQuestionAnswer;

  const valenceElectronAnswer = answerElementValenceElectrons(text, originalText);
  if (valenceElectronAnswer) return valenceElectronAnswer;
...
function answerUnit7Comparison(text) {
  ...
  if (/\bprotons?\b/.test(text) && /\b(?:neutrons?|nuetrons?|newtrons?)\b/.test(text) && /\belectrons?\b/.test(text)) {
    return {
      id: 'unit7.compare.subatomic_particles',
      answer: 'Protons, neutrons, and electrons are particles found in atoms. Proton: positive charge, in the nucleus, identifies the element. Neutron: neutral charge, in the nucleus, adds mass. Electron: negative charge, in the electron cloud outside the nucleus.',
      type: 'definition',
...
function answerSubatomicParticleQuestion(text) {
  if (/\bparticles?\b/.test(text) && /\b(?:inside|in|make up|part of)\b/.test(text) && /\batoms?\b/.test(text)) {
    return concept('unit7.subatomic_particles_in_atom', 'The main subatomic particles in an atom are protons, neutrons, and electrons. Protons and neutrons are in the nucleus, and electrons are in the electron cloud.');
  }

  if (/\b(?:calculate|find|determine)\b/.test(text) && /\b(?:neutrons?|nuetrons?|newtrons?)\b/.test(text) && !/\d/.test(text)) {
    return concept('unit7.calculate_neutrons_rule', 'To calculate neutrons, subtract atomic number from mass number: neutrons = mass number - atomic number.');
  }

  if (hasDefinitionIntent(text) && /\bvalence\s+electrons?\b/.test(text)) return factAnswer(findFact('unit7.valence_electrons'));
  const requestedParticles = [
    [/\bprotons?\b/, 'unit7.proton'],
    [/\b(?:neutrons?|nuetrons?|newtrons?)\b/, 'unit7.neutron'],
    [/\belectrons?\b/, 'unit7.electron']
  ].filter(([pattern]) => pattern.test(text));
  if (requestedParticles.length > 1 && (hasDefinitionIntent(text) || hasLocationIntent(text) || hasChargeIntent(text))) {
    return {
      id: 'unit7.subatomic_particles.multiple',
      answer: requestedParticles.map(([, id]) => findFact(id).definition).join('\n'),
      type: 'definition',
      sourceRefs: UNIT7_SOURCE_REFERENCES
    };
  }
  if (hasDefinitionIntent(text) && /\belectrons?\b/.test(text)) return factAnswer(findFact('unit7.electron'));
  if (hasDefinitionIntent(text) && /\bprotons?\b/.test(text)) return factAnswer(findFact('unit7.proton'));
  if (hasDefinitionIntent(text) && /\b(?:neutrons?|nuetrons?|newtrons?)\b/.test(text)) return factAnswer(findFact('unit7.neutron'));

  return null;
}

function answerElementValenceElectrons(text, originalText) {
  if (!looksLikeAtomicValenceElectronCountQuestion(text, originalText)) return null;
  const element = findPeriodicElement(text, originalText);
  if (!element) return null;
  const group = element.group;
  if (!Number.isInteger(group)) return noMatchAnswer('unit7.element.valence_electrons.unsupported_group');
  const valenceElectrons = element.name === 'Helium'
    ? 2
    : group <= 2
      ? group
      : group >= 13 && group <= 18
        ? group - 10
        : null;
  if (valenceElectrons == null) return noMatchAnswer('unit7.element.valence_electrons.unsupported_group');
  const groupDescription = group === 18 ? `a noble gas in Group ${group}` : `in Group ${group}`;
  return concept(
    'unit7.element.valence_electrons',
    `${element.name} is ${groupDescription}, so it has ${valenceElectrons} valence ${valenceElectrons === 1 ? 'electron' : 'electrons'}.`
  );
}

function answerUnsupportedMolecularValenceQuestion(text, originalText) {
  if (!looksLikeValenceElectronCountQuestion(text) || !hasMolecularValenceContext(text, originalText)) return null;
  return noMatchAnswer('unit7.element.valence_electrons.unsupported_molecular');
}

function answerMultiTargetAtomicQuestion(text) {
  if (!looksLikeMultiTargetAtomicQuestion(text)) return null;
  const protons = findParticleCount(text, ['proton', 'protons', 'p+']);
  const electrons = findParticleCount(text, ['electron', 'electrons', 'e-']);
  const neutrons = findParticleCount(text, ['neutron', 'neutrons', 'nuetron', 'nuetrons', 'newtron', 'newtrons', 'n0']);
  const wantsElement = /\belement\b/.test(text);
  const wantsCharge = /\bcharge\b/.test(text);
  const wantsMassNumber = /\bmass number\b/.test(text);
  const lines = [];

  if (wantsElement) {
    const element = periodicElements.find((entry) => Number(entry.atomicNumber) === protons);
    if (!element) return null;
    lines.push(`Element: ${element.name} (${element.symbol}), because ${protons} protons means atomic number ${protons}.`);
  }
  if (wantsCharge) {
    if (protons == null || electrons == null) return null;
    const charge = protons - electrons;
    lines.push(`Charge: ${formatCharge(charge)}, because charge = protons - electrons = ${protons} - ${electrons}.`);
  }
  if (wantsMassNumber) {
    if (protons == null || neutrons == null) return null;
    lines.push(`Mass number: ${protons + neutrons}, because mass number = protons + neutrons = ${protons} + ${neutrons}.`);
  }

  return concept('unit7.atom.multiple_calculations', lines.join('\n'));
}

function findParticleCount(text, labels) {
  const labelPattern = labels.map(escapeRegex).sort((a, b) => b.length - a.length).join('|');
  const patterns = [
    new RegExp(`\\b(\\d{1,3})\\s+(?:${labelPattern})\\b`),
    new RegExp(`\\b(?:${labelPattern})\\s*(?:is|are|=|:|of)?\\s*(\\d{1,3})\\b`)
  ];
...
function looksLikeMultiTargetAtomicQuestion(text) {
  return looksLikeMultiTargetAtomicShape(text) &&
    /\b(?:atoms?|atomic|ions?|ionic(?:\s+species)?)\b/.test(text) &&
    !/\b(?:molecules?|molecular|compounds?|formulas?|multi[ -]?atom)\b/.test(text);
}

function looksLikeInvalidAtomicMassNumberContext(text) {
  return /\bmass number\b/.test(text) &&
    /\bprotons?\b/.test(text) &&
    /\b(?:neutrons?|nuetrons?|newtrons?)\b/.test(text) &&
    /\d/.test(text) &&
    /\b(?:molecules?|molecular|compounds?|formulas?|multi[ -]?atom)\b/.test(text);
}

function looksLikeMultiTargetAtomicShape(text) {
  if (!/\b(?:what|find|calculate|determine|identify)\b/.test(text)) return false;
  const targetCount = [
    /\belement\b/.test(text),
    /\bcharge\b/.test(text),
    /\bmass number\b/.test(text)
  ].filter(Boolean).length;
  return targetCount > 1 && /\bprotons?\b/.test(text) && /\d/.test(text);
}

function looksLikeValenceElectronCountQuestion(text) {
  return /\b(?:how many|number of|count)\b/.test(text) && /\bvalence electrons?\b/.test(text);
}

function looksLikeAtomicValenceElectronCountQuestion(text, originalText = text) {
  return looksLikeValenceElectronCountQuestion(text) &&
    !looksLikeMixedTargetValenceQuestion(text) &&
    !hasBondingActionContext(text) &&
    !hasMolecularValenceContext(text, originalText);
}

function looksLikeMixedTargetValenceQuestion(text) {
  if (!looksLikeValenceElectronCountQuestion(text)) return false;
  const withoutValenceElectrons = text.replace(/\bvalence\s+electrons?\b/g, '');
  return /\b(?:protons?|neutrons?|nuetrons?|newtrons?|electrons?|atomic\s+number|mass\s+number|isotopes?|isotope\s+notation|periods?|groups?|charge)\b/.test(withoutValenceElectrons) ||
    /\b(?:what|which)\s+element\b|\bidentify\s+(?:the\s+)?element\b|\belement\s+(?:identity|name|symbol)\b|\bname\s+(?:the\s+)?element\b/.test(withoutValenceElectrons);
}

function hasBondingActionContext(text) {
  return /\b(?:gains?|gained|loses?|lost|needs?|needed|shares?|shared|transfers?|transferred|bonds?|bonding|donates?|donated|accepts?|accepted|become|becomes|became|stable|stability)\b/.test(text);
}

function hasMolecularValenceContext(text, originalText) {
  return /\b(?:molecules?|molecular|compounds?|(?:chemical\s+)?formulas?)\b/.test(text) ||
    (/\bgas\b/.test(text) && !/\bnoble\s+gas\b/.test(text)) ||
    containsMolecularFormula(originalText);
}

function containsMolecularFormula(text) {
  const source = String(text || '');
  const formulaTokens = source.match(/\b[A-Za-z][A-Za-z0-9]*\b/g) || [];
  if (formulaTokens.some((token) => {
    if (/\d/.test(token)) return isMolecularFormulaToken(token);
    return /^(?:[A-Z][a-z]?){2,}$/.test(token) && isMolecularFormulaToken(token);
  })) return true;

  const normalizedText = source.toLowerCase();
  const contextualTokens = [
    ...normalizedText.matchAll(/\b(?:in|for)\s+(?:the\s+)?([a-z][a-z0-9]*)\b/g),
    ...normalizedText.matchAll(/\bdoes\s+([a-z][a-z0-9]*)\s+have\b/g)
  ].map((match) => match[1]);
  return contextualTokens.some(isMolecularFormulaToken);
}

function isMolecularFormulaToken(token) {
  if (isPeriodicElementReferenceToken(token)) return false;
  const normalizedToken = String(token || '').toLowerCase();
  const memo = new Map();
  function parseAt(index) {
    if (index === normalizedToken.length) return 0;
    if (memo.has(index)) return memo.get(index);
    for (const symbol of periodicElementFormulaSymbols) {
      if (!normalizedToken.startsWith(symbol, index)) continue;
      let nextIndex = index + symbol.length;
      while (nextIndex < normalizedToken.length && /\d/.test(normalizedToken[nextIndex])) nextIndex += 1;
      const remainingSymbols = parseAt(nextIndex);
      if (remainingSymbols >= 0) {
        memo.set(index, remainingSymbols + 1);
        return remainingSymbols + 1;
      }
    }
    memo.set(index, -1);
    return -1;
  }
  const symbolCount = parseAt(0);
  return symbolCount > 0 && (/\d/.test(normalizedToken) || symbolCount > 1);
}

function isPeriodicElementReferenceToken(token) {
  const sourceToken = String(token || '');
  const normalizedToken = normalize(sourceToken);
  return periodicElementNameMatches.some(([name]) => name === normalizedToken) ||
    periodicElementSymbolMatches.some(([symbol]) => symbol === sourceToken);
}

function shouldPreferUnit7AtomicStructureKnowledge(message) {
  const text = normalize(message);
  return looksLikeInvalidAtomicMassNumberContext(text) ||
    looksLikeMultiTargetAtomicShape(text) ||
    (looksLikeValenceElectronCountQuestion(text) &&
      !looksLikeMixedTargetValenceQuestion(text) &&
      !hasBondingActionContext(text));
...
function shouldBlockMixedTargetValenceQuestion(message) {
  return looksLikeMixedTargetValenceQuestion(normalize(message));
}

function shouldPreferUnit8BondingForValenceAction(message) {
  const text = normalize(message);
  return looksLikeValenceElectronCountQuestion(text) && hasBondingActionContext(text);
}

function looksLikeComparisonQuestion(text) {
  if (/\b(?:compare|comparison|difference|diffrence|difrence|different|alike|vs|versus|same and different|similarities?|similarities and differences)\b/.test(text)) {
    return true;
  }
  if (/\bisotopes?\b/.test(text) && /\bions?\b/.test(text) && /\bdiffers?\s+from\b/.test(text)) {
    return true;
  }
...
function looksLikeWavePeriodPrompt(text) {
  return /\bfrequency\b/.test(text) &&
    /\bperiod\b/.test(text) &&
    !/\bperiodic\s+table\b/.test(text) ||
    /\bperiod\b/.test(text) &&
    /\bwaves?\b/.test(text);
}

function looksLikeUnit6MatterClassificationPrompt(text) {
  if (/\b(?:compound|mixture)\b/.test(text) && /\belements?\b/.test(text)) return true;
  return /\b(?:salt|hcl|hydrogen chloride|hydrochloric|oxygen|chlorine)\b/.test(text) &&
    /\b(?:elements?|compounds?|mixtures?)\b/.test(text) &&
    !/\b(?:periodic table|atomic|atom|atoms|atomic number|group|period)\b/.test(text);
}

function looksLikeCircuitElectronPrompt(text) {
  if (!/\b(?:electrons?|eletrons?|electons?)\b/.test(text)) return false;
  if (hasGeneralUnit7Context(text)) return false;
  const hasCircuitContext = /\b(?:batter(?:y|ies)|circuits?|voltage|current|terminals?)\b/.test(text) ||
    /\bnegative\s+to\s+positive\b/.test(text) ||
    /\belectrons?\s+flow\b|\belectron\s+flow\b/.test(text);
  const asksAboutFlow = /\b(?:flow|flows|move|moves|moving|travel|travels|traveling|push|pushes|cause|causes)\b/.test(text) ||
    /\bnegative\b.*\bpositive\b|\bpositive\b.*\bnegative\b/.test(text);
  return hasCircuitContext && asksAboutFlow;
}
...
function findPeriodicElement(text, originalText = text) {
  const normalizedText = normalize(text);
  for (const [name, element] of periodicElementNameMatches) {
    if (new RegExp(`(^|[^a-z0-9])${escapeRegex(name)}(?=$|[^a-z0-9])`).test(normalizedText)) return element;
  }
  for (const [symbol, element] of periodicElementSymbolMatches) {
    if (new RegExp(`\\b${escapeRegex(symbol)}\\b`).test(String(originalText || ''))) return element;
  }
  return null;
}

function noMatchAnswer(id) {
  return {
    id,
    answer: 'I do not have a trusted local science fact for that yet. Please reword the question with the vocabulary word, formula, or numbers you are asking about, or ask your teacher.',
    type: 'no_match',
    sourceRefs: []
  };
}
