...
const UNIT9_REACTIONS_METADATA = Object.freeze({
  ...
  sourceNote: 'Built from the grounded Reactions synthesis supplied by the user. Raw PDFs/PPTX files are not required for this patch.',
...
function answerUnit9Reactions(text, original) {
  ...
  if (/\bexergonic\b|\bexothermic\b/.test(text)) return factAnswer(findFact('unit9.exergonic_exothermic'));
  if (/\bendergonic\b|\bendothermic\b/.test(text)) return factAnswer(findFact('unit9.endergonic_endothermic'));
...
function conceptAnswer(id, answer) {
