...
} = require('./graphSeeds/motionForceGraphSeed');
...
function addMotionForceSeedRelationships(builder) {
  MOTION_FORCE_SEED_RELATIONSHIPS.forEach(([fromLabel, toLabel, edgeType, relationshipLabel]) => {
    ...
    addEdge(builder, from.id, to.id, edgeType, {
      label: relationshipLabel || edgeType.replace(/_/g, ' '),
      confidence: 'high',
      source: MOTION_FORCE_SEED_SOURCE
    });
  });
}
...
function findOrCreateSeedNode(builder, label) {
  ...
  return addNode(builder, {
    id: makeNodeId(MOTION_FORCE_SEED_NODE_TYPES[normalized] || 'concept', label, MOTION_FORCE_SEED_PACK_ID),
    type: MOTION_FORCE_SEED_NODE_TYPES[normalized] || 'concept',
    label,
    aliases: [],
    source: MOTION_FORCE_SEED_SOURCE,
    packId: MOTION_FORCE_SEED_PACK_ID,
    standards: [],
    metadata: {
      seed: MOTION_FORCE_SEED_NAME
    }
  });
}
...
function isApprovedKnowledgeItem(item) {
  ...
  const reviewStatus = String(
    item.reviewStatus || item.reviewState || ''
  ).trim().toLowerCase();
  return reviewStatus === APPROVED_STATUS;
}
