...
const SOURCE_TRACKING_REQUIRED_SECTIONS = new Set([
  'vocabulary',
  'concepts',
  'referenceFormulas',
  'problemBank'
...
function promoteDraftKnowledgePack(draftPackInput, options = {}) {
  ...
  let activation = null;
  try {
    activation = setApprovedPackActivation(safePackId, true, {
      ...options,
      approvedPacksDir
    });
  } catch (error) {
    return blocked({
      ...
      outputPath,
      warnings,
      errors: [`Approved pack was created, but it could not be activated: ${error instanceof Error ? error.message : String(error)}`],
      validationPassed: true,
      promotionMode: promotionScope.mode
    });
  }
...
function getApprovedItemPromotionBlockers(sectionName, item, label) {
  ...
  if (SOURCE_TRACKING_REQUIRED_SECTIONS.has(sectionName)) {
