...
function validatePromotionReadiness(pack) {
  ...
  PROMOTABLE_SECTIONS.forEach((sectionName) => {
    ...
    items.forEach((item, index) => {
      ...
      if (item.reviewStatus === 'pending' || item.reviewStatus === 'needs_review') {
        errors.push(`${label} is still pending teacher review.`);
      }

      if (item.reviewStatus === 'approved') {
        if (!isApprovedItemSourceGrounded(item)) {
          errors.push(`${label} is approved but does not have valid source-grounding evidence.`);
        } else if (isLowConfidenceValue(item.confidence) && !hasTeacherLowConfidenceOverride(item)) {
          errors.push(`${label} is approved but still has low confidence.`);
        } else {
          approvedCount += 1;
        }
      }
...
function buildApprovedKnowledgePack(pack) {
  ...
  PROMOTABLE_SECTIONS.forEach((sectionName) => {
    ...
    approvedPack[sectionName] = approvedPack[sectionName].filter((item) => {
      return item
        && item.reviewStatus === 'approved'
        && isApprovedItemSourceGrounded(item)
        && (!isLowConfidenceValue(item.confidence) || hasTeacherLowConfidenceOverride(item));
    });
