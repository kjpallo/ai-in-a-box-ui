...
function buildFlashcardDeck(message, options = {}) {
  const text = normalize(message);
  const topic = selectTopic(text, options.topics || []);
  if (!topic) return null;

  const cards = normalizeFlashcards(topic);
  if (cards.length === 0) return null;

  return {
    type: 'flashcards',
    topicId: topic.id || '',
    title: clean(topic.displayTitle || topic.title) || 'Flashcards',
    cards,
    currentCardIndex: 0,
    showingBack: false
  };
...
function normalizeFlashcards(topic) {
  const cards = Array.isArray(topic.flashcards) ? topic.flashcards : [];
  return cards.map((card) => {
    const front = clean(card.front || card.question || buildQuestionFront(card));
    const back = clean(card.back || card.answer || card.term);
    if (!front || !back) return null;
    return {
      front,
      back: sentenceCaseAnswer(back)
    };
  }).filter(Boolean);
}

function buildQuestionFront(card = {}) {
  const term = clean(card.term);
  const definition = clean(card.definition);
  if (!term || !definition) return '';
  return `Which term means: ${definition}`;
}

function sentenceCaseAnswer(value) {
