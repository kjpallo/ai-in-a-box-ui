...
function detectGraphSupportIntent(message, options = {}) {
  ...
  if (/\b(?:mixing\s+up|mixed\s+up|confus(?:e|ed|ing)|same|vs|versus)\b/.test(normalized) ||
...
function formatGraphPath(path) {
  ...
  const parts = [displayLabel(nodes[0]?.label)];
  edges.forEach((edge, index) => {
    const nextNode = nodes[index + 1];
    parts.push(cleanLabel(edge.label || edge.type || 'related to').replace(/_/g, ' '), displayLabel(nextNode?.label));
  });
...
function formatList(labels) {
  const clean = uniqueLabels(labels).map(displayLabel);
...
function cleanSentence(value) {
  return String(value || '').replace(/\s+/g, ' ').trim().replace(/[;:]\s*$/g, '').slice(0, 220);
}
