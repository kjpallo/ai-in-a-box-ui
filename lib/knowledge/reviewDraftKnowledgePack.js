...
function getDraftItemApprovalBlockers(item) {
  ...
  if (item.confidence === 'low') {
    blockers.push('Low-confidence generated items cannot be approved automatically. Review or repair the source evidence first.');
  }
  ...
  if (grounding && grounding.status !== 'supported') {
    blockers.push('Generated wording was not strongly supported by the extracted source text.');
  }
  const unsafeText = [
  ...
  if (/\b(rejected|reject|repair[_ -]?needed|needs? repair|quarantine|quarantined|invalid|failed|failure|error)\b/.test(unsafeText)) {
    blockers.push('Rejected, repair-needed, quarantined, invalid, or failed draft items cannot be approved.');
  }
