...
const MEASUREMENT_FACTS = [
  ...
  quantityFact({
    ...
    canonicalTerm: 'mass',
    definition: 'Mass is the amount of matter in an object. Formula: mass can be found from m = F/a when force and acceleration are known, or m = density × volume when density and volume are known.',
  ...
  quantityFact({
    ...
    definition: 'Volume is the amount of space an object or substance takes up. Common units include mL, L, cm³, or m³. Volume is also used in density: density = mass / volume.',
  ...
  quantityFact({
    ...
    definition: 'Temperature measures how hot or cold something is and measures the average kinetic energy of particles.',
...
function tryUnit1MeasurementKnowledge(message) {
  ...
  return {
    type: fact.type === 'measurement_concept' || fact.type === 'quantity' ? 'definition' : 'science_concept',
    confidence: 'strong',
...
function matchUnitQuestion(text) {
  if (!/\b(unit|si|measured|mesured)\b/.test(text)) return null;
