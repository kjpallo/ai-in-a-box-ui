...
function findMatchingFact(text) {
  ...
  if (/\bmeasurement\b/.test(text) && /\b(include|number|unit)\b/.test(text)) {
...
function looksLikeCalculationOrChunk3Prompt(text) {
  ...
  if (/\b(convert|conversion|dimensional analysis|picket fence|scientific notation|standard notation)\b/.test(text)) return true;
