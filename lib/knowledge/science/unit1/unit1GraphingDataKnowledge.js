...
const { buildUnit1GraphingDataPacket } = require('./unit1GraphingDataPacketAdapter');
...
const UNIT1_GRAPHING_DATA_PACKET = buildUnit1GraphingDataPacket({
  facts: FACTS,
...
function tryUnit1GraphingDataKnowledge(message) {
  ...
  if (looksLikeMotionGraphPrompt(text)) return null;
...
function looksLikeMotionGraphPrompt(text) {
  const hasMotionAxis = /\b(?:distance|position|speed|velocity)\s+(?:vs|versus|over|against)\s+time\b/.test(text) ||
    /\b(?:distance|position|speed|velocity)\s+time\s+graph\b/.test(text);
  const hasGraphInterpretationCue = /\b(?:constant\s+slope|upward\s+curve|curves?\s+upward|getting\s+steeper|gets?\s+steeper|steepest|rate\s+of\s+change|horizontal\s+flat\s+line|flat\s+line|sloping\s+downward|sloping\s+upward)\b/.test(text);
  if (hasMotionAxis && /\bgraph\b/.test(text)) return true;
  return /\bgraph\b/.test(text) && /\b(?:distance|position|speed|velocity)\b/.test(text) && hasGraphInterpretationCue;
}
