...
const SAFETY_FACTS = [
  {
    ...
    examples: ['wearing goggles while using chemicals or heating glassware'],
  ...
  },
  {
    ...
    examples: ['gently waving fumes toward your nose instead of smelling a chemical directly'],
  ...
  },
  {
    ...
    examples: ['telling the teacher after a test tube breaks instead of picking it up with bare hands'],
...
const EQUIPMENT_FACTS = [
  equipment('unit1.equipment.graduated_cylinder', 'graduated cylinder', ['grad cylinder'], ['graduated cylender'], 'A graduated cylinder measures liquid volume more precisely.', 'unit1.corpus.0012', ['measuring a specific liquid volume in milliliters']),
  ...
  equipment('unit1.equipment.beaker', 'beaker', [], ['beeker'], 'A beaker is used for holding, mixing, heating, and roughly measuring liquids or solids.', 'unit1.corpus.0011', ['holding or mixing a liquid during a lab'])
];
...
const PACKET_FACTS = UNIT1_SAFETY_EQUIPMENT_PACKET.canonicalFacts;
const PACKET_FACTS_BY_ID = new Map(PACKET_FACTS.map((fact) => [fact.id, fact]));

function tryUnit1SafetyEquipmentKnowledge(message) {
  const text = normalize(message);
  if (!text) return null;
  if (looksLikePhysicalChemicalChangePrompt(text)) return null;
...
function findMatchingFact(text) {
  const passFact = packetFactById('unit1.safety.fire_extinguisher');
  if (isPassFireExtinguisherQuestion(text)) return passFact;

  const clothesFireFact = packetFactById('unit1.safety.clothing_fire');
  ...
  const spillFact = packetFactById('unit1.safety.chemical_spill_exposure');
  ...
  const noTasteFact = packetFactById('unit1.safety.no_eating_drinking_tasting');
  if (/\b(taste|eat|drink|food)\b/.test(text) && /\b(lab|chemical|chemicals?)\b/.test(text)) return noTasteFact;

  const disposalFact = packetFactById('unit1.safety.chemical_disposal');
  if (/\b(pour|dump|dispose|disposal)\b/.test(text) && /\b(chemical|chemicals?|sink)\b/.test(text)) return disposalFact;

  const brokenGlassFact = packetFactById('unit1.safety.broken_glass');
  ...
  const hairFact = packetFactById('unit1.safety.hair_clothing_jewelry');
  ...
  for (const fact of PACKET_FACTS) {
...
function looksLikePhysicalChemicalChangePrompt(text) {
  return /\b(?:physical|chemical)\b/.test(text) &&
...
function packetFactById(id) {
  return PACKET_FACTS_BY_ID.get(id);
}

function equipment(id, canonicalTerm, aliases, typoAliases, answerTemplate, sourceRef, examples = []) {
  return {
    id,
    unit: UNIT,
    chunk: CHUNK,
...
module.exports = {
  ALL_UNIT1_SAFETY_EQUIPMENT_FACTS: PACKET_FACTS,
