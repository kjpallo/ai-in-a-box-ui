...
const DEFAULT_DRAFT_PACKS_DIR = path.join(__dirname, '..', '..', 'knowledge', 'draft-packs');

function loadDraftKnowledgePacks(options = {}) {
  const draftPacksDir = options.draftPacksDir || DEFAULT_DRAFT_PACKS_DIR;
  const packFilePaths = findKnowledgePackFiles(draftPacksDir);
  const packs = [];
  const errors = [];
