...
function validateKnowledgePack(pack, options = {}) {
  const errors = [];
  const warnings = [];
  const standardsPaused = options.standardsPaused === true;
  ...
  validateStandardsMap(pack.standardsMap, errors, warnings, { standardsPaused });
  validateSmokeTests(pack.smokeTests, errors, warnings, { standardsPaused });
...
function validateStandardsMap(standardsMap, errors, warnings, options = {}) {
  ...
  if (standardsMap.length === 0 && options.standardsPaused !== true) {
    warnings.push('standardsMap is empty; approved packets should normally map content to standards.');
  }
}

function validateSmokeTests(smokeTests, errors, warnings, options = {}) {
  ...
  if (smokeTests.length === 0 && options.standardsPaused !== true) {
