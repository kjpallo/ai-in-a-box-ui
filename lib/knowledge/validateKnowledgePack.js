...
function validateVocabulary(vocabulary, errors, warnings) {
  ...
  vocabulary.forEach((item, index) => {
    ...
    requireArray(item, 'standards', `${label}.standards`, errors);
...
function validateProblemBank(problemBank, errors, warnings) {
  ...
  problemBank.forEach((item, index) => {
    ...
    if ('standards' in item) requireArray(item, 'standards', `${label}.standards`, errors);
    validateReviewAndConfidence(item, label, errors);
    validateSourceTracking(item, label, errors, warnings);
  });
}
...
function validateStandardReferences(pack, approvedStandardIds, errors) {
  ...
  validateStandardArrayReferences(pack.vocabulary, 'vocabulary', approvedStandardIds, errors);
  validateStandardArrayReferences(pack.concepts, 'concepts', approvedStandardIds, errors);
  validateStandardArrayReferences(pack.problemBank, 'problemBank', approvedStandardIds, errors);
...
function validateStandardArrayReferences(items, fieldName, approvedStandardIds, errors) {
  if (!Array.isArray(items)) return;

  items.forEach((item, itemIndex) => {
    if (!item || typeof item !== 'object' || Array.isArray(item) || !Array.isArray(item.standards)) return;

    item.standards.forEach((standardId, standardIndex) => {
      if (typeof standardId !== 'string') return;
      if (!approvedStandardIds.has(standardId)) {
        errors.push(`${fieldName}[${itemIndex}].standards[${standardIndex}] has unknown standard reference: ${standardId}`);
      }
    });
  });
}
