...
function exportQuestionsStandardsCsv(options = {}) {
  return buildQuestionsStandardsCsvExport(readStudentInteractions(options.logFilePath), options);
}
...
function buildQuestionsStandardsCsvExport(records, options = {}) {
  ...
  const rows = (Array.isArray(records) ? records : [])
    ...
    .filter((record) => !selectedDate || dateKeyFromEntry(record) === selectedDate)
