...
function readArchivedCsvRecords(archiveFile, { verifyChecksums = true } = {}) {
  ...
  return rows
    ...
    .map((row, rowIndex) => archivedCsvRowToInteraction(row, archiveFile, rowIndex))
    .filter(Boolean);
}

function archivedCsvRowToInteraction(row, archiveFile, rowIndex = 0) {
  const timestamp = safeText(row.timestamp || row.time) || safeText(row.date) || safeText(archiveFile.manifest?.createdAt);
  const question = safeText(row.question);
  if (!question) return null;
  const id = safeText(row.id) || legacyArchiveRecordId({
    archiveId: archiveFile.manifest?.archiveId || archiveFile.manifest?.exportId,
  ...
  const record = {
    ...
    sessionId: safeText(row.sessionId || archiveFile.manifest?.sessionId || archiveFile.manifest?.classSessionId),
    className: safeText(archiveFile.manifest?.className || archiveFile.manifest?.sessionLabel),
    sessionLabel: safeText(archiveFile.manifest?.sessionLabel || archiveFile.manifest?.className),
    ...
    category: safeText(row['topic/category'] || row.topic),
    topic: safeText(row['topic/category'] || row.topic),
    ...
    primaryStandards: parseStandardsCell(row.primaryStandards || row.standard),
    possibleStandards: parseStandardsCell(row.possibleStandards),
    standards: parseStandardsCell(row.standards || row.standard),
...
function legacyArchiveRecordId({
  archiveId,
  archiveCsvFilename,
  rowIndex
} = {}) {
  const hash = crypto.createHash('sha256')
    .update([
      safeText(archiveId),
      safeText(archiveCsvFilename),
      `row:${normalizeRowIndex(rowIndex)}`
    ].join('|'))
    .digest('hex')
    .slice(0, 20);
  return `archived_${hash}`;
}
