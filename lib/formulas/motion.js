...
function buildMotionValueEntryDisplayEquation(values, requestedValue) {
  const displaySymbol = (key) => key === requestedValue
    ? '{{blank}}'
    : key === 'distance'
      ? 'd'
      : key === 'time'
  ...
  if (values.solveFor === 'distance') {
    return `d = ${displaySymbol('speed')} × ${displaySymbol('time')}`;
  }
  if (values.solveFor === 'time') {
    return `t = ${displaySymbol('distance')} / ${displaySymbol('speed')}`;
  }
  return `v = ${displaySymbol('distance')} / ${displaySymbol('time')}`;
}
