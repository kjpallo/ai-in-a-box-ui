function tryMotion(text, lower, ctx) {
  if (ctx.asksForAcceleration(lower)) return null;
  ...
  const segmentedAverageSpeed = trySegmentedAverageSpeed(text, lower, target, time, ctx);
  if (segmentedAverageSpeed) return segmentedAverageSpeed;
  ...
  if (target === 'distance' && speed && time) {
    ...
    const formulaWork = buildMotionFormulaWork({
      ...
      timeOriginalValue: time.value,
      timeOriginalUnit: time.unit,
    ...
    const lines = [
      ...
      `distance = ${formatRateAnswer(speed.value, speed.unit, ctx)} × ${formatTimeAnswer(timeInRateUnit, speed.perTimeUnit, ctx)}`,
  ...
  if (target === 'speed' && distance && time && time.value !== 0) {
    ...
    const formulaWork = buildMotionFormulaWork({
      solveFor: answerKind === 'velocity' ? 'velocity' : 'speed',
    ...
    const lines = [
      ...
      `${finalLineLabel} = ${ctx.cleanNumber(distance.value)} ${ctx.plural(distance.unit, distance.value)} / ${formatTimeAnswer(time.value, time.unit, ctx)}`,
      ...extraLines,
  ...
  if (target === 'time' && distance && speed && speed.value !== 0) {
    ...
    const lines = [
      ...
      `time = ${ctx.cleanNumber(distanceInRateUnit)} ${ctx.plural(speed.distanceUnit, distanceInRateUnit)} / ${formatRateAnswer(speed.value, speed.unit, ctx)}`,
      `time = ${formatFinalTimeAnswer(value, speed.perTimeUnit, ctx)}`
    ];
...
function trySegmentedAverageSpeed(text, lower, target, totalTime, ctx) {
  if (target !== 'speed' || !totalTime || totalTime.value === 0) return null;
  if (!/\baverage\s+speed\b/.test(lower)) return null;
  if (!/\b(?:entire|whole|total)\s+(?:trip|time|journey|run|ride)\b/.test(lower)) return null;

  const distances = ctx.findAllNumbersWithUnits(text, ctx.DISTANCE_UNITS);
  if (distances.length < 2) return null;

  const unit = distances[0].unit;
  if (distances.some((distance) => distance.unit !== unit)) return null;

  const totalDistance = distances.reduce((sum, distance) => sum + distance.value, 0);
  const speedValue = totalDistance / totalTime.value;
  const speedUnit = `${unit}/${totalTime.unit}`;
  const distanceExpression = distances
    .map((distance) => `${ctx.cleanNumber(distance.value)} ${ctx.plural(distance.unit, distance.value)}`)
    .join(' + ');
  const finalDisplay = formatRateAnswer(speedValue, speedUnit, ctx, { compact: true, final: true });
  const formulaWork = buildMotionFormulaWork({
    solveFor: 'speed',
    distanceValue: totalDistance,
    distanceUnit: unit,
    distanceSourceDisplay: `${distanceExpression} = ${ctx.cleanNumber(totalDistance)} ${ctx.plural(unit, totalDistance)}`,
    timeValue: totalTime.value,
    timeUnit: totalTime.unit,
    speedValue,
    speedUnit,
    finalDisplay,
    ctx
  ...
  return ctx.answer('Recognized average speed problem: adding trip segments before dividing by total time.', [
    'Use the motion formula: average speed = total distance / total time.',
    `total distance = ${distanceExpression} = ${ctx.cleanNumber(totalDistance)} ${ctx.plural(unit, totalDistance)}`,
    `total time = ${formatTimeAnswer(totalTime.value, totalTime.unit, ctx)}`,
    `average speed = ${ctx.cleanNumber(totalDistance)} ${ctx.plural(unit, totalDistance)} / ${formatTimeAnswer(totalTime.value, totalTime.unit, ctx)}`,
    `average speed = ${finalDisplay}`
  ], formulaWork);
}
...
function buildMotionFormulaWork({
  ...
  const calculationTarget = solveFor === 'velocity' ? 'speed' : solveFor;
  const solveTargetChoices = solveFor === 'velocity' ? ['velocity', 'distance', 'time'] : ['speed', 'distance', 'time'];
  ...
  const timeOriginalDisplay = formatTimeAnswer(timeOriginalValue, timeOriginalUnit, ctx);
  ...
  const combinedFinalDisplay = extraFinalAnswers.length > 0
    ? `${finalSpeedDisplay} and about ${extraFinalAnswers[0].display}`
    : finalSpeedDisplay;
  const finalByTarget = {
    speed: {
      value: speedValue,
      unit: speedUnit,
      display: combinedFinalDisplay,
  ...
  return {
    ...
    family: 'motion',
    solveFor,
    formula: formulaByTarget[solveFor] || formulaByTarget[calculationTarget],
    finalAnswer: finalByTarget[solveFor] || finalByTarget[calculationTarget],
    variables: {
      distance: {
        symbol: 'd',
        value: distanceValue,
    ...
    steps: [
      buildSolveTargetStep(solveFor, solveTargetChoices),
      buildFormulaChoiceStep(formulaByTarget[solveFor] || formulaByTarget[calculationTarget], solveFor),
      ...buildMotionQuantitySteps({
        solveFor: calculationTarget,
        solveForLabel: solveFor,
        distanceValue,
        distanceUnit,
        distanceDisplay,
        distanceSourceDisplay,
      ...
      buildMotionCalculationStep({ solveFor: calculationTarget, solveForLabel: solveFor, distanceValue, timeValue, speedValue, distanceDisplay, timeDisplay, speedDisplay: finalSpeedDisplay, ctx }),
      ...buildMotionAlternateUnitSteps({
        solveFor,
        speedValue,
        speedDisplay: finalSpeedDisplay,
        extraFinalAnswers,
        finalDisplay: combinedFinalDisplay,
        ctx
      })
    ]
  };
...
function buildFormulaAliases(solveFor, formula) {
  ...
  if (solveFor === 'velocity') aliases.push('v = d / t', 'speed = distance / time');
...
function buildMotionQuantitySteps(values) {
  ...
  if (values.solveFor !== 'distance') {
    const distanceStep = {
      ...
      displayEquation: buildMotionValueEntryDisplayEquation(values, 'distance'),
      expectedValue: values.distanceValue,
      expectedUnit: values.distanceUnit,
      expectedDisplay: values.distanceDisplay,
      hints: values.distanceSourceDisplay
        ? [`For the whole trip, use total distance: ${values.distanceSourceDisplay}.`]
        : [`Look for the number with ${values.distanceUnit}.`]
    };

    if (values.distanceSourceDisplay) {
      distanceStep.sourceDisplay = values.distanceSourceDisplay;
    }
  ...
  if (values.solveFor !== 'time') {
    const timeStep = {
      ...
      displayEquation: buildMotionValueEntryDisplayEquation(values, 'time'),
    ...
    if (values.timeOriginalUnit && values.timeOriginalUnit !== values.timeUnit) {
      const conversionStepId = 'convert_time';
      timeStep.originalValue = values.timeOriginalValue;
      timeStep.originalUnit = values.timeOriginalUnit;
      timeStep.originalDisplay = values.timeOriginalDisplay;
      timeStep.conversionDisplay = `${values.timeOriginalDisplay} = ${values.timeDisplay}`;
      timeStep.conversionReminder = `Because speed is in ${formatRateUnitForSentence(values.speedUnit)}, we will convert that to ${values.timeDisplay} before multiplying.`;
      timeStep.conversionStepId = conversionStepId;
      timeStep.skipConversionStepOnConvertedAnswer = true;
      timeStep.hints = [
        `The problem gives ${values.timeOriginalDisplay}. Because speed is in ${formatRateUnitForSentence(values.speedUnit)}, we will convert that to ${values.timeDisplay} before multiplying.`
      ];
      steps.push(timeStep, buildTimeConversionStep({
        id: conversionStepId,
        rateKind: 'speed',
        rateUnit: values.speedUnit,
        originalValue: values.timeOriginalValue,
        originalUnit: values.timeOriginalUnit,
        originalDisplay: values.timeOriginalDisplay,
        convertedValue: values.timeValue,
        convertedUnit: values.timeUnit,
        convertedDisplay: values.timeDisplay,
  ...
  if (values.solveFor !== 'speed') {
    steps.push({
      ...
      displayEquation: buildMotionValueEntryDisplayEquation(values, 'speed'),
...
function buildMotionValueEntryDisplayEquation(values, requestedValue) {
  const displayValue = (key) => key === requestedValue
    ? '{{blank}}'
    : values.ctx.cleanNumber(values[`${key}Value`]);
  const solveForLabel = values.solveForLabel || values.solveFor;

  if (values.solveFor === 'distance') {
    return `distance = ${displayValue('speed')} × ${displayValue('time')}`;
  }
  if (values.solveFor === 'time') {
    return `time = ${displayValue('distance')} ÷ ${displayValue('speed')}`;
  }
  return `${solveForLabel} = ${displayValue('distance')} ÷ ${displayValue('time')}`;
}
...
function buildTimeConversionStep({
  ...
  const prompt = divisor
    ? `The ${rateKind} is in ${formatRateUnitForSentence(rateUnit)}, so convert ${originalDisplay} to ${formatTimeUnitForSentence(convertedUnit)}. What is ${ctx.cleanNumber(originalValue)} ÷ ${ctx.cleanNumber(divisor)}?`
    : `The ${rateKind} is in ${formatRateUnitForSentence(rateUnit)}, so convert ${originalDisplay} to ${formatTimeUnitForSentence(convertedUnit)}. What is the converted time?`;
  ...
  return {
    ...
    prompt,
    ...(divisor
      ? { displayEquation: `${ctx.cleanNumber(originalValue)} ÷ ${ctx.cleanNumber(divisor)} = {{blank}}` }
      : {}),
    expectedValue: convertedValue,
    expectedUnit: convertedUnit,
    expectedDisplay: convertedDisplay,
    calculationExpression: divisor ? `${ctx.cleanNumber(originalValue)} / ${ctx.cleanNumber(divisor)}` : '',
    hints: [
      `${originalDisplay} = ${convertedDisplay}. Use ${convertedDisplay} before ${operation}.`
    ]
  };
}
...
function buildMotionCalculationStep({ solveFor, solveForLabel = solveFor, distanceValue, timeValue, speedValue, distanceDisplay, timeDisplay, speedDisplay, ctx }) {
  if (solveFor === 'distance') {
    return {
      ...
      displayEquation: `distance = ${ctx.cleanNumber(speedValue)} × ${ctx.cleanNumber(timeValue)} = {{blank}}`,
  ...
  if (solveFor === 'time') {
    return {
      ...
      displayEquation: `time = ${ctx.cleanNumber(distanceValue)} ÷ ${ctx.cleanNumber(speedValue)} = {{blank}}`,
  ...
  return {
    id: 'calculate',
    type: 'calculation',
    prompt: `Now substitute: ${solveForLabel} = ${ctx.cleanNumber(distanceValue)} / ${ctx.cleanNumber(timeValue)}. What is ${ctx.cleanNumber(distanceValue)} / ${ctx.cleanNumber(timeValue)}?`,
    displayEquation: `${solveForLabel} = ${ctx.cleanNumber(distanceValue)} ÷ ${ctx.cleanNumber(timeValue)} = {{blank}}`,
    expectedValue: speedValue,
    expectedUnit: solveForLabel,
    expectedDisplay: speedDisplay,
    hints: ['Divide distance by time.']
  };
}

function buildMotionAlternateUnitSteps({ solveFor, speedValue, speedDisplay, extraFinalAnswers, finalDisplay, ctx }) {
  if (solveFor !== 'velocity' || !Array.isArray(extraFinalAnswers) || extraFinalAnswers.length === 0) return [];

  const alternate = extraFinalAnswers[0];
  if (!Number.isFinite(Number(alternate.value)) || alternate.unit !== 'm/s') return [];

  const acceptedAlternateValues = Math.abs(alternate.value - 35.7632) < 0.001 ? [35.7] : [];
  const combinedFinalAnswer = {
    response: `Correct. velocity = ${finalDisplay}.`,
    values: [
      { value: speedValue, label: speedDisplay },
      { value: alternate.value, label: alternate.display }
    ]
  };

  return [
    {
      id: 'convert_velocity_to_mps',
      type: 'calculation',
      prompt: `Convert ${speedDisplay} to m/s. What is ${ctx.cleanNumber(speedValue)} × 0.44704?`,
      displayEquation: `${ctx.cleanNumber(speedValue)} × 0.44704 = {{blank}}`,
      substitution: `${ctx.cleanNumber(speedValue)} mi/hr × 0.44704`,
      calculationExpression: `${ctx.cleanNumber(speedValue)} * 0.44704`,
      expectedValue: alternate.value,
      expectedUnit: 'm/s',
      expectedDisplay: alternate.display,
      acceptedAlternateValues,
      correctResponse: `Correct. velocity = ${finalDisplay}.`,
      skipNextStepOnCorrect: true,
      skipStepIdOnCorrect: 'state_both_velocity_units',
      acceptCombinedFinalAnswer: true,
      combinedFinalAnswer,
      skipNextStepOnCombinedFinalAnswer: true,
      skipStepIdOnCombinedFinalAnswer: 'state_both_velocity_units',
      hints: ['Multiply miles per hour by 0.44704 to convert to meters per second.']
    },
    {
      id: 'state_both_velocity_units',
      type: 'final_answer',
      prompt: 'What is the final velocity in both units?',
      expected: finalDisplay,
      combinedFinalAnswer,
      acceptedAnswers: [finalDisplay],
      hints: ['Include the mi/hr answer and the m/s conversion.']
    }
  ];
}
...
function formatRateUnitForSentence(unit) {
  if (unit === 'mile/hr') return 'miles per hour';
  if (unit === 'km/hr') return 'km/hr';
  if (unit === 'm/s') return 'm/s';
  if (unit === 'ft/s') return 'ft/s';
  return unit;
}

function formatFinalTimeAnswer(value, unit, ctx) {
  const display = formatTimeAnswer(isRoundedApproximation(value) ? Number(formatFinalNumber(value)) : value, unit, ctx);
  const primary = formatMaybeApprox(value, display);
  const minutes = usefulMinutesConversion(value, unit, ctx);
  return minutes ? `${primary} (${minutes})` : primary;
}

function usefulMinutesConversion(value, unit, ctx) {
  if (unit !== 'hr' || !Number.isFinite(value) || value <= 0 || value >= 2) return '';
  const minutes = value * 60;
  if (!Number.isFinite(minutes) || minutes < 1) return '';
  const minuteNumber = Math.round(minutes);
  return `about ${formatTimeAnswer(minuteNumber, 'min', ctx)}`;
}
