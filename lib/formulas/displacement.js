...
function tryTwoDimensionalMovementDisplacement(moves, ctx, intent = {}) {
  ...
  if (Number.isFinite(displacementRadicand)) {
    lines.push(`displacement = √${ctx.cleanNumber(displacementRadicand)} ${displayDistanceUnit(unit)}`);
  }
...
function findDirectedMoves(text, ctx) {
  ...
  return moves
    ...
    .map(({ value, unit, direction }) => ({ value, unit, direction }));
}
...
function buildMovementVariables(moves, ctx) {
  const variables = {
    movements: moves.map((move) => ({
      ...
      display: `${ctx.cleanNumber(move.value)} ${displayDistanceUnit(move.unit)} ${move.direction}`
    }))
  };
...
function buildMovementQuantityStep(move, index, ctx) {
  const display = `${ctx.cleanNumber(move.value)} ${displayDistanceUnit(move.unit)} ${move.direction}`;
  return {
