...
function tryDisplacement(text, lower, ctx) {
  if (isDistanceDisplacementConceptIdentification(lower)) return null;
...
function isDistanceDisplacementConceptIdentification(lower) {
  return /\b(?:distance\s+or\s+displacement|displacement\s+or\s+distance)\b/.test(lower) ||
    /\bwhich\s+is\s+(?:the\s+)?(?:distance|displacement)\b/.test(lower);
}
...
function tryPoolLengthDistanceDisplacement(text, lower, ctx) {
  ...
  const displacementDisplay = displacement === 0
    ...
    : `${formatApproxNumber(displacement)} ${displayDistanceUnit(length.unit)}`;
...
function tryClosedLoopDistanceDisplacement(text, lower, ctx) {
  ...
  const closedLoopContext = /\btrack\b|\blap\b/.test(lower) ? 'track' : 'block';
...
function looksLikeClosedLoopCase(lower) {
  return (
    /\baround\s+(?:the\s+)?block\b/.test(lower) ||
    /\baround\s+(?:a|an|the)?\s*(?:\d+(?:\.\d+)?\s*)?(?:mile|mi|kilometer|km|meter|m)?\s*track\b/.test(lower) ||
    /\b(?:one|1)\s+(?:full\s+)?lap\b/.test(lower)
  ) &&
    /\b(?:back|returns?|returned|ends?|ended|end\s+up|ended\s+up)\b/.test(lower) &&
    /\b(?:home|start|starting|started|doorstep|starting\s+position|where\s+(?:he|she|they|it)\s+started)\b/.test(lower);
}
...
function buildClosedLoopDistanceDisplacementSteps({ distance, ctx, closedLoopContext }) {
  if (!distance || !ctx) return [];
  const distanceDisplay = formatDistanceValue(distance.value, distance.unit, ctx);
  const displacementDisplay = `0 ${displayDistanceUnit(distance.unit)}`;
  if (closedLoopContext === 'track') {
    return [
      buildTargetStep(),
      buildConceptStep(
        'distance = total path; displacement = start-to-finish change',
        'Distance is the full path around the track. Displacement only compares the start and finish.'
      ),
      {
        id: 'calculate_distance',
        type: 'calculation',
        prompt: 'What is the total path distance around the track?',
        ...
        hints: [`The problem gives the path around the track as ${distanceDisplay}.`]
      },
      {
        id: 'identify_finish_location',
        type: 'multiple_choice',
        prompt: 'Where does the runner finish compared to where they started?',
        choices: [
          { number: 1, label: 'Same place / back where they started', correct: true },
          { number: 2, label: 'Opposite side / away from where they started', correct: false },
          { number: 3, label: `${distanceDisplay} away from the starting position`, correct: false }
        ],
        expected: 'the starting position',
        allowAcceptedAnswerContains: true,
        acceptedAnswers: [
          'same',
          'same place',
          'the same place',
        ...
        hints: ['The runner ends at the starting position.']
      },
      {
        id: 'calculate_displacement',
        type: 'multiple_choice',
        prompt: 'What is the runner’s displacement?',
        choices: [
          { number: 1, label: displacementDisplay, correct: true },
          { number: 2, label: `${distanceDisplay} west`, correct: false },
          { number: 3, label: `${distanceDisplay} north`, correct: false }
        ],
        expected: displacementDisplay,
        expectedDisplay: displacementDisplay,
        acceptedAnswers: ['0', displacementDisplay],
        hints: ['Displacement is 0 because the start and end positions are the same.']
      }
    ];
  }
  ...
  return [
    ...
    },
    {
      ...
      choices: [
        { number: 1, label: 'Same place / back where he started', correct: true },
        { number: 2, label: 'Opposite side / away from where he started', correct: false },
        { number: 3, label: '0.35 miles north', correct: false }
      ],
...
function buildSpinInPlaceDistanceDisplacementSteps() {
  return [
    ...
    ),
    {
      ...
      choices: [
        { number: 1, label: 'She spun/stayed in place', correct: true },
        { number: 2, label: 'She moved to a different location', correct: false },
        { number: 3, label: 'She traveled 7 meters', correct: false }
      ],
    ...
    },
    {
      ...
      choices: [
        { number: 1, label: '0', correct: true },
        { number: 2, label: '7 times', correct: false },
        { number: 3, label: '7 meters', correct: false }
      ],
      expected: '0',
      expectedDisplay: '0',
...
function buildPoolLengthDistanceDisplacementSteps({ length, times, distance, displacement, ctx }) {
  ...
  return [
    ...
    },
    {
      ...
      choices: [
        { number: 1, label: 'Back where he started', correct: false },
        { number: 2, label: 'Opposite side of the pool', correct: true },
        { number: 3, label: '150 m away from the start', correct: false }
      ],
      expected: 'opposite side',
      allowAcceptedAnswerContains: true,
      retryMessage: 'Choose 2, or click “2. Opposite side of the pool.”',
      acceptedAnswers: [
        'opposite',
        'opposite side',
        'the opposite side',
    ...
    },
    {
      ...
      choices: [
        { number: 1, label: displacementDisplay, correct: true },
        { number: 2, label: distanceDisplay, correct: false },
        { number: 3, label: `0 ${displayDistanceUnit(length.unit)}`, correct: false }
      ],
      expected: displacementDisplay,
      expectedDisplay: displacementDisplay,
      acceptedAnswers: [ctx.cleanNumber(displacement), displacementDisplay],
      conceptualNudges: [
        {
          answers: ['opposite', 'opposite side', 'the opposite side', 'other side', 'the other side', 'different side', 'difrent', 'difrent side', 'opposition side'],
          response: 'Yes, he is on the opposite side. What distance is that from where he started?'
        }
      ],
