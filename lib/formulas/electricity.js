...
function tryCircuitResistance(text, lower) {
  ...
  if (isExactSimpleCurrentAfterResistanceCase(circuitType, lower, resistances, voltage, wantsResistance, wantsCurrent, wantsVoltage)) {
...
function isExactSimpleCurrentAfterResistanceCase(circuitType, lower, resistances, voltage, wantsResistance, wantsCurrent, wantsVoltage) {
  if (!wantsResistance || !wantsCurrent || wantsVoltage || !voltage) return false;
  if (!Array.isArray(resistances) || resistances.length !== 3) return false;
  if (!resistances.every((value) => value === 20) || voltage.value !== 120) return false;

  const normalized = lower.replace(/[?.!,]+/g, ' ').replace(/\s+/g, ' ').trim();
  const pattern = new RegExp(`\\b(?:three|3) 20 ohm resistors are connected in ${circuitType}\\b`);
  return pattern.test(normalized) &&
    /\bacross a 120 v generator\b/.test(normalized) &&
    /\bfind the total resistance and current\b/.test(normalized);
}
...
function isExactSimpleParallelTotalResistanceCase(lower, resistances, wantsResistance, wantsCurrent, wantsVoltage) {
  ...
  if (!Array.isArray(resistances) || resistances.length !== 3) return false;
  const normalized = lower.replace(/[?.!,]+/g, ' ').replace(/\s+/g, ' ').trim();
  const isThree20OhmCase = resistances.every((value) => value === 20) &&
    /\b(?:three|3) 20 ohm resistors are connected in parallel\b/.test(normalized) &&
    /\bfind the total resistance\b/.test(normalized);
  const isUnequal5_10_20Case = resistances[0] === 5 &&
    resistances[1] === 10 &&
    resistances[2] === 20 &&
    normalized === 'find total resistance in a parallel circuit with 5 ohms 10 ohms and 20 ohms';
  return isThree20OhmCase || isUnequal5_10_20Case;
}
