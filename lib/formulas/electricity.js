...
function tryCircuitResistance(text, lower) {
  ...
  if (circuitType === 'series') {
    lines.push(`Rt = ${resistances.map((value) => `${cleanOhmsNumber(value)} ohms`).join(' + ')}`);
  ...
  if (voltage && (wantsCurrent || /\bcurrent\b/.test(lower))) {
    ...
    lines.push('Use Ohm’s law with total resistance: I = V / Rt.');
    lines.push(`I = ${cleanOhmsNumber(voltage.value)} V / ${totalResistanceDisplay} ohms`);
  ...
  if (current && wantsVoltage) {
    ...
    lines.push('Use Ohm’s law with total resistance: V = I × Rt.');
    lines.push(`V = ${cleanOhmsNumber(current.value)} amps × ${cleanOhmsNumber(totalResistance)} ohms`);
    lines.push(`V = ${cleanOhmsNumber(voltageValue)} volts`);
  }

  return answer(`Recognized ${circuitType} circuit resistance problem.`, lines);
}
