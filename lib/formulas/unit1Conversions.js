...
function parseMetricConversion(text) {
  ...
  const patterns = [
    ...
    new RegExp(`\\bhow\\s+many\\s+(${unitPattern})\\s+(?:is|are)?\\s*in\\s+(${NUMBER_PATTERN})\\s*(${unitPattern})\\b`, 'i'),
    new RegExp(`\\b(${NUMBER_PATTERN})\\s*(${unitPattern})[?.!,;:\\s]+(?:is\\s+)?how\\s+many\\s+(${unitPattern})\\b`, 'i')
  ];
