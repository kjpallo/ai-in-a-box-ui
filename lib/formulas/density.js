function tryDensity(text, lower, ctx) {
  ...
  if (target === 'density' && mass && volume && volume.value !== 0) {
    ...
    const formulaWork = !derivedVolume
      ? buildDensityFormulaWork(m, massUnit, v, volumeUnit, value, resultUnit, ctx)
      : null;
...
function buildDensityFormulaWork(massValue, massUnit, volumeValue, volumeUnit, densityValue, densityUnit, ctx) {
  ...
  return {
    ...
    steps: [
      ...
      },
      {
        ...
        expectedUnit: massUnit,
        expectedDisplay: massDisplay,
        hints: [`Look for the number with ${massUnit}.`]
      },
      {
        id: 'identify_volume',
        type: 'quantity',
        ...
        hints: [`Look for the number with ${volumeUnit}.`]
      },
      {
        id: 'calculate',
        type: 'calculation',
        ...
        expectedUnit: densityUnit,
        expectedDisplay: densityDisplay,
