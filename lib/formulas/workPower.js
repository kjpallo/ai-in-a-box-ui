...
function tryWorkPowerTime(text, lower) {
  ...
  if (/\b(?:potential energy|gravitational potential energy|gpe)\b/.test(lower)) return null;
...
function buildWorkPowerFormulaWork({ solveFor, workValue, powerValue, timeValue }) {
  ...
  const finalAnswerByTarget = {
    ...
    work: { value: workValue, unit: 'J', display: workDisplay },
    time: { value: timeValue, unit: 's', display: timeDisplay }
  };
  const formulaByTarget = {
    power: 'P = W / t',
    work: 'W = P × t',
  ...
  const calculationByTarget = {
    power: {
      prompt: `Now substitute: P = ${cleanNumber(workValue)} / ${cleanNumber(timeValue)}. What is ${cleanNumber(workValue)} / ${cleanNumber(timeValue)}?`,
      hint: 'Divide work by time.'
    },
    work: {
      prompt: `Now substitute: W = ${cleanNumber(powerValue)} × ${cleanNumber(timeValue)}. What is ${cleanNumber(powerValue)} × ${cleanNumber(timeValue)}?`,
      hint: 'Multiply power by time.'
    },
    time: {
      prompt: `Now substitute: t = ${cleanNumber(workValue)} / ${cleanNumber(powerValue)}. What is ${cleanNumber(workValue)} / ${cleanNumber(powerValue)}?`,
      hint: 'Divide work by power.'
    }
  };
  const finalAnswer = finalAnswerByTarget[solveFor];
  const calculation = calculationByTarget[solveFor];
