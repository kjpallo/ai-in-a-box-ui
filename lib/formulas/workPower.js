...
function buildWorkForceFormulaWork({ solveFor, workValue, forceValue, distanceValue }) {
  ...
  const finalAnswer = solveFor === 'work'
    ? { value: workValue, unit: 'J', display: workDisplay }
    : { value: distanceValue, unit: 'm', display: distanceDisplay };

  return buildFormulaWork({
    formulaId: 'work_force_distance',
    family: 'work',
    solveFor,
    formula: solveFor === 'work' ? 'W = F × d' : 'd = W / F',
    finalAnswer,
    ...
    calculation: {
      prompt: solveFor === 'work'
        ? `Now substitute: W = ${cleanNumber(forceValue)} × ${cleanNumber(distanceValue)}. What is ${cleanNumber(forceValue)} × ${cleanNumber(distanceValue)}?`
        : `Now substitute: d = ${cleanNumber(workValue)} / ${cleanNumber(forceValue)}. What is ${cleanNumber(workValue)} / ${cleanNumber(forceValue)}?`,
      expectedValue: finalAnswer.value,
      hints: [solveFor === 'work' ? 'Multiply force by distance.' : 'Divide work by force.']
    }
  });
}
