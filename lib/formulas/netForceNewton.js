...
function netForceStepLines(netForce) {
  const lines = [
    ...
    'Forces in the same direction are added. Forces in opposite directions are subtracted or cancel out.'
  ];
  const cancelLine = netForce.lines.find((line) => /\bcancel\s+out\b/i.test(line));
  if (cancelLine) lines.push(cancelLine);
  const workLines = netForce.lines.filter((line) => /=/.test(line) && /\bN\b/.test(line) && !/^net force\s*=/i.test(line));
...
function buildTwoStepFormulaWork({ solveFor, forceValue, massValue, accelerationValue, direction, ctx }) {
  ...
  formulaWork.variables.netForce = {
    symbol: 'Fnet',
    value: forceValue,
    unit: 'N',
    display: direction ? `${ctx.cleanNumber(forceValue)} N ${direction}` : `${ctx.cleanNumber(forceValue)} N`
  };
  formulaWork.steps = buildTwoStepTutorSteps({
    solveFor,
    forceValue,
    massValue,
    accelerationValue,
    ...
    finalAnswer: formulaWork.finalAnswer,
    ctx
  });
...
function buildTwoStepTutorSteps({ solveFor, forceValue, massValue, accelerationValue, direction, formula, finalAnswer, ctx }) {
  const forceDisplay = direction ? `${ctx.cleanNumber(forceValue)} N ${direction}` : `${ctx.cleanNumber(forceValue)} N`;
  const massDisplay = `${ctx.cleanNumber(massValue)} kg`;
  const accelerationDisplay = direction ? `${ctx.cleanNumber(accelerationValue)} m/s² ${direction}` : `${ctx.cleanNumber(accelerationValue)} m/s²`;
  const isAcceleration = solveFor === 'acceleration';
  const isMass = solveFor === 'mass';
  ...
  const steps = [
    {
      ...
      choices: [
        ...
        { number: 2, label: 'acceleration', correct: isAcceleration },
        { number: 3, label: 'mass', correct: isMass }
      ],
      expected: solveFor,
    ...
    },
    {
      ...
      type: 'calculation',
      prompt: 'Forces in the same direction are added. Forces in opposite directions are subtracted or cancel out. After combining the forces, what is the net force?',
      expectedValue: forceValue,
      expectedUnit: 'N',
      expectedDisplay: forceDisplay,
      acceptFinalAnswer: isAcceleration,
      hints: ['Add forces in the same direction and subtract forces in opposite directions.']
    },
    {
      id: 'choose_formula',
      type: 'multiple_choice',
      prompt: 'Which formula should we use with net force?',
      choices: [
      ...
      expected: formula,
      hints: ['Newton’s second law connects net force, mass, and acceleration.']
    }
  ];
  ...
  if (isAcceleration) {
    steps.push(
      ...
      },
      {
        ...
        calculationExpression: `${ctx.cleanNumber(forceValue)} / ${ctx.cleanNumber(massValue)}`,
        expectedValue: accelerationValue,
        expectedUnit: 'm/s²',
        expectedDisplay: finalAnswer.display || accelerationDisplay,
        hints: ['Divide net force by mass.']
      }
    );
  } else if (isMass) {
    steps.push(
      ...
      },
      {
        ...
        calculationExpression: `${ctx.cleanNumber(forceValue)} / ${ctx.cleanNumber(accelerationValue)}`,
        expectedValue: massValue,
        expectedUnit: 'kg',
        expectedDisplay: finalAnswer.display || massDisplay,
        hints: ['Divide net force by acceleration.']
      }
    );
  } else {
    steps.push(
      ...
      },
      {
        id: 'calculate',
        type: 'calculation',
        prompt: `Now substitute: Fnet = ${ctx.cleanNumber(massValue)} × ${ctx.cleanNumber(accelerationValue)}. What is ${ctx.cleanNumber(massValue)} × ${ctx.cleanNumber(accelerationValue)}?`,
        calculationExpression: `${ctx.cleanNumber(massValue)} * ${ctx.cleanNumber(accelerationValue)}`,
        expectedValue: forceValue,
        expectedUnit: 'N',
        expectedDisplay: finalAnswer.display || forceDisplay,
