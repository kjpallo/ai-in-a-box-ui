  ...
  const knownValueSteps = variableEntries
    ...
    .flatMap((variable) => {
      ...
      for (const field of [
        ...
        'conversionReminder',
        'conversionStepId',
        'skipConversionStepOnConvertedAnswer'
      ]) {
        if (Object.prototype.hasOwnProperty.call(variable, field)) {
          quantityStep[field] = variable[field];
        }
      }

      return variable.conversionStep
        ? [quantityStep, variable.conversionStep]
        : [quantityStep];
    });
  ...
  return {
    ...
    steps: [
      ...
      ...knownValueSteps,
      {
        ...
        calculationExpression: calculation.calculationExpression || '',
        expectedUnit: finalAnswer.unit,
        expectedDisplay: finalAnswer.display,
        acceptedAlternateValues: calculation.acceptedAlternateValues || [],
