...
function findSpecificHeatQuantity(text) {
  const number = '(-?\\d+(?:\\.\\d+)?)';
  const unitPattern = '(j\\s*\\/\\s*(g|kg)\\s*°?\\s*c|j\\s*\\/\\s*(g|kg)\\s*degrees?\\s*celsius|joules?\\s+per\\s+(gram|kilogram)\\s+degree\\s+celsius)';
  const patterns = [
...
function buildSpecificHeatFormulaWork({
  ...
  const finalAnswer = solveFor === 'heat energy'
    ? { value: heatEnergyValue, unit: 'J', display: heatDisplay }
    : { value: specificHeatValue, unit: specificHeatUnit, display: specificHeatDisplay };

  return buildFormulaWork({
    formulaId: 'specific_heat',
    family: 'specific_heat',
    solveFor,
    formula: solveFor === 'heat energy' ? 'q = m × c × ΔT' : 'c = q / (m × ΔT)',
    finalAnswer,
    ...
    calculation: {
      prompt: solveFor === 'heat energy'
        ? `Now substitute: q = ${ctx.cleanNumber(massValue)} × ${ctx.cleanNumber(specificHeatValue)} × ${ctx.cleanNumber(temperatureChangeValue)}. What is ${ctx.cleanNumber(massValue)} × ${ctx.cleanNumber(specificHeatValue)} × ${ctx.cleanNumber(temperatureChangeValue)}?`
        : `Now substitute: c = ${ctx.cleanNumber(heatEnergyValue)} / (${ctx.cleanNumber(massValue)} × ${ctx.cleanNumber(temperatureChangeValue)}). What is ${ctx.cleanNumber(heatEnergyValue)} / (${ctx.cleanNumber(massValue)} × ${ctx.cleanNumber(temperatureChangeValue)})?`,
      expectedValue: finalAnswer.value,
      hints: [solveFor === 'heat energy' ? 'Multiply mass, specific heat, and temperature change.' : 'Multiply mass by temperature change, then divide heat energy by that product.']
    }
  });
}
