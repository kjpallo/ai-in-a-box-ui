...
function buildTimeConversionStep({
  ...
  const prompt = divisor
    ? `The velocity is in ${velocityUnit}, so convert ${originalDisplay} to ${formatTimeUnitForSentence(convertedUnit)}. What is ${ctx.cleanNumber(originalValue)} ÷ ${ctx.cleanNumber(divisor)}?`
