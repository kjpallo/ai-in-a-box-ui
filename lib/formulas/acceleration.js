...
function tryAccelerationFromVelocity(text, lower, ctx) {
  ...
  const inferredFinalVelocity = inferFinalVelocityFromExplicitAccelerationTarget(text, lower, { vi, acceleration }, ctx);

  const vf = explicitVf || (velocityChange && velocityChange.vf) ||
    inferredFinalVelocity ||
  ...
  const viForCalculation = vi || (acceleration && time && asksForFinalVelocityTarget(lower)
    ? firstVelocityQuantity(text, ctx)
    : null) || (hasInitialRestPhrase(lower) || inferredFinalVelocity
  ...
  if (target === 'acceleration' && isAmbiguousOneSpeedAccelerationPrompt(text, { vi: viForCalculation, vf, time, acceleration }, ctx)) {
    const speed = firstVelocityQuantity(text, ctx);
    const speedDisplay = speed ? `${ctx.cleanNumber(speed.value)} ${speed.unit}` : 'that speed';
    const timeDisplay = time ? `${ctx.cleanNumber(time.value)} ${formatTimeUnitForSentence(time.unit)}` : 'that time';
    return ctx.answer('Recognized acceleration question, but starting and ending velocities need clarification.', [
      'To calculate acceleration, I need starting velocity and ending velocity.',
      `Did it start from rest and reach ${speedDisplay} in ${timeDisplay}, or start at ${speedDisplay} and change to another speed?`
    ]);
  }
  ...
  if (target === 'acceleration' && viForCalculation && vf && time && time.value !== 0) {
    ...
    const formulaWork = buildAccelerationFormulaWork({
      ...
      initialVelocityOriginalValue: viForCalculation.value,
      initialVelocityOriginalUnit: viForCalculation.unit,
      finalVelocityOriginalValue: vf.value,
      finalVelocityOriginalUnit: vf.unit,
      ...
      timeOriginalValue: calculation.originalTimeValue,
      timeOriginalUnit: calculation.originalTimeUnit,
      timeOriginalDisplay: calculation.originalTimeDisplay,
      ctx
    });
    const lines = [
  ...
  if (target === 'time' && vi && vf && acceleration && acceleration.value !== 0) {
    const calculation = timeFromAccelerationCalculation(vi, vf, acceleration, ctx);
    if (!calculation) return null;

    return ctx.answer('Recognized acceleration problem: solving for time.', [
      'Use the formula: t = (vf - vi) / a.',
      `t = (${ctx.cleanNumber(calculation.finalVelocity)} ${calculation.velocityUnit} - ${ctx.cleanNumber(calculation.initialVelocity)} ${calculation.velocityUnit}) / ${ctx.cleanNumber(acceleration.value)} ${displayUnit(acceleration.unit)}`,
      `t = ${formatTimeDisplay(calculation.value, calculation.timeUnit, ctx)}`
    ]);
  }
...
function findAccelerationQuantity(text, ctx) {
  ...
  const patterns = [
    ...
    new RegExp(`\\b(?:slows?|slowed|slow(?:ing)?\\s+down|speeds?|speeding\\s+up)\\s+(?:at|by)?\\s*(?:a\\s+)?rate\\s+of\\s*${number}\\s*(${accelUnitPattern})(?=$|[^A-Za-z0-9/²^])`, 'i'),
    new RegExp(`\\b(?:rate)\\s+of\\s*${number}\\s*(${accelUnitPattern})(?=$|[^A-Za-z0-9/²^])`, 'i'),
...
function timeFromAccelerationCalculation(vi, vf, acceleration, ctx) {
  if (vi.unit === vf.unit && vi.distanceUnit === acceleration.distanceUnit && vi.perTimeUnit === acceleration.perTimeUnit) {
    return {
      initialVelocity: vi.value,
      finalVelocity: vf.value,
      velocityUnit: vi.unit,
      value: (vf.value - vi.value) / acceleration.value,
      timeUnit: acceleration.perTimeUnit
    };
  }

  const viMS = ctx.velocityToMS(vi);
  const vfMS = ctx.velocityToMS(vf);
  const accelerationMS2 = accelerationToMS2(acceleration, ctx);
  if (viMS == null || vfMS == null || accelerationMS2 == null || accelerationMS2 === 0) return null;

  return {
    initialVelocity: viMS,
    finalVelocity: vfMS,
    velocityUnit: 'm/s',
    value: (vfMS - viMS) / accelerationMS2,
    timeUnit: 's'
  };
}

function accelerationToMS2(acceleration, ctx) {
  if (acceleration.unit === 'm/s^2') return acceleration.value;
  if (!acceleration.distanceUnit || !acceleration.perTimeUnit) return null;

  const distanceInMeters = ctx.convertDistance(acceleration.value, acceleration.distanceUnit, 'm');
  const secondsPerTimeUnit = ctx.convertTime(1, acceleration.perTimeUnit, 's');
  if (distanceInMeters == null || secondsPerTimeUnit == null || secondsPerTimeUnit === 0) return null;
  return distanceInMeters / (secondsPerTimeUnit ** 2);
}
...
function accelerationCalculation(vi, vf, time, lower, ctx) {
  ...
  return {
    ...
    originalTimeValue: time.value,
    originalTimeUnit: time.unit,
    originalTimeDisplay: formatTimeDisplay(time.value, time.unit, ctx),
...
function worksheetAccelerationPolicy(vi, vf, time, lower, ctx) {
  ...
  if (vi.perTimeUnit === 'hr' && time.unit === 's' && asksToConvertSecondsToHours(lower)) {
    ...
    const exactHours = `${ctx.cleanNumber(time.value)} / 3600`;
    return {
      velocityUnit: vi.unit,
      timeUnit: 'hr',
      accelerationUnit: accelerationUnitFor(vi.unit, 'hr'),
      timeDisplay: `(${exactHours})`,
      conversionLines: [`Convert: ${ctx.cleanNumber(time.value)} s = ${exactHours} hr ≈ ${displaySmallTimeConversion(hours, ctx)} hr`],
      isApproximate: false
    };
  }
...
function formatTimeDisplay(value, unit, ctx) {
...
function formatTimeUnitForSentence(unit) {
  if (unit === 'hr') return 'hours';
  if (unit === 'min') return 'minutes';
  if (unit === 's') return 'seconds';
  return unit;
}

function buildTimeConversionStep({
  id,
  velocityUnit,
  originalValue,
  originalUnit,
  ...
  const divisor = timeConversionDivisor(originalUnit, convertedUnit);
  const conversionExpression = divisor ? `${ctx.cleanNumber(originalValue)} / ${ctx.cleanNumber(divisor)}` : '';
  const prompt = divisor
    ? 'Which time value should we use before dividing?'
    : `The velocity is in ${velocityUnit}, so convert ${originalDisplay} to ${formatTimeUnitForSentence(convertedUnit)}. What is the converted time?`;
  ...
  if (divisor) {
    return {
      ...
      prompt,
      choices: [
        { number: 1, label: `${ctx.cleanNumber(originalValue)} ${convertedUnit}`, correct: false },
        { number: 2, label: `${conversionExpression} ${convertedUnit} ≈ ${displaySmallTimeConversion(convertedValue, ctx)} ${convertedUnit}`, correct: true },
        { number: 3, label: `${ctx.cleanNumber(divisor)} / ${ctx.cleanNumber(originalValue)} ${convertedUnit}`, correct: false }
      ],
      expected: `${conversionExpression} ${convertedUnit}`,
      expectedValue: convertedValue,
      expectedUnit: convertedUnit,
      expectedDisplay: convertedDisplay,
      calculationExpression: conversionExpression,
      acceptNumericAnswer: true,
      acceptedAnswers: [
        `${conversionExpression}`,
        `${conversionExpression} ${convertedUnit}`,
        `${displaySmallTimeConversion(convertedValue, ctx)}`,
        `${displaySmallTimeConversion(convertedValue, ctx)} ${convertedUnit}`
      ],
      acceptFinalAnswer: Array.isArray(earlyFinalValues) && earlyFinalValues.length > 0,
      acceptedFinalValues: earlyFinalValues || [],
      hints: [
        `${originalDisplay} = ${convertedDisplay}. Use ${convertedDisplay} before dividing.`
      ]
    };
  }
...
function timeConversionDivisor(fromUnit, toUnit) {
  if (fromUnit === 'min' && toUnit === 'hr') return 60;
  if (fromUnit === 's' && toUnit === 'hr') return 3600;
  if (fromUnit === 's' && toUnit === 'min') return 60;
  return null;
}
...
function findAccelerationVelocityChange(text, lower, ctx) {
  ...
  if (speeds.length >= 2 && hasVelocityChangeCue(lower)) {
    return {
      vi: speeds[0],
      vf: speeds[1],
      time
...
function inferFinalVelocityFromExplicitAccelerationTarget(text, lower, values, ctx) {
  if (!asksForAcceleration(lower)) return null;
  if (values.vi || values.acceleration) return null;
  if (!hasInitialRestPhrase(lower)) return null;

  const speeds = ctx.findAllNumbersWithUnits(text, ctx.VELOCITY_UNITS);
  const times = ctx.findAllNumbersWithUnits(text, ctx.TIME_UNITS);
  if (speeds.length !== 1 || times.length !== 1) return null;

  return speeds[0];
}

function isAmbiguousOneSpeedAccelerationPrompt(text, values, ctx) {
  if (values.acceleration || values.vi || values.vf || !values.time) return false;

  const speeds = ctx.findAllNumbersWithUnits(text, ctx.VELOCITY_UNITS);
  const times = ctx.findAllNumbersWithUnits(text, ctx.TIME_UNITS);
  return speeds.length === 1 && times.length === 1;
}
...
function hasVelocityFormulaCue(lower) {
  return /\b(initial velocity|starting velocity|start velocity|initial speed|starting speed|start speed|final velocity|final speed|ending velocity|ending speed|end velocity|end speed|velocity|vf|vi)\b/.test(lower);
}
...
function asksForFinalVelocityTarget(lower) {
  return questionTargetRegex('final velocity|final speed|ending velocity|end velocity|vf').test(lower) ||
    asksForFinalVelocity(lower);
}
...
function questionTargetRegex(targetPattern) {
  return new RegExp(`\\b(?:what is|what's|find|calculate|alculate|solve for|determine)\\s+(?:(?:my|the)\\s+)?(?:[a-z0-9-]+(?:['’]s)?\\s+)?(?:${targetPattern})\\b`);
}

function displayUnit(unit) {
  if (unit === 'm/s^2') return 'm/s²';
  if (unit === 'ft/s^2') return 'ft/s²';
  if (unit === 'km/hr^2') return 'km/hr²';
  return unit;
}
...
function buildAccelerationFormulaWork({
  ...
  finalVelocityValue,
  initialVelocityOriginalValue = initialVelocityValue,
  initialVelocityOriginalUnit = '',
  finalVelocityOriginalValue = finalVelocityValue,
  finalVelocityOriginalUnit = '',
  timeValue,
  timeUnit = 's',
  timeOriginalValue = timeValue,
  timeOriginalUnit = timeUnit,
  timeOriginalDisplay = '',
  accelerationValue,
  velocityUnit = 'm/s',
  accelerationUnit = 'm/s²',
  ...
  const initialOriginalFields = buildOriginalVelocityFields({
    originalValue: initialVelocityOriginalValue,
    originalUnit: initialVelocityOriginalUnit,
    convertedValue: initialVelocityValue,
    convertedDisplay: initialDisplay,
  ...
  const finalOriginalFields = buildOriginalVelocityFields({
    originalValue: finalVelocityOriginalValue,
    originalUnit: finalVelocityOriginalUnit,
    convertedValue: finalVelocityValue,
    convertedDisplay: finalDisplay,
    targetUnit: velocityUnit,
  ...
  const timeExpression = buildConvertedTimeExpression({
    originalValue: timeOriginalValue,
    originalUnit: timeOriginalUnit,
    convertedUnit: timeUnit,
    ctx
  });
  const convertedTimeNumber = displaySmallTimeConversion(timeValue, ctx);
  const timeDisplay = timeExpression
    ? `${timeExpression} ${timeUnit} ≈ ${convertedTimeNumber} ${timeUnit}`
    : `${ctx.cleanNumber(timeValue)} ${timeUnit}`;
  const calculationTimeTerm = timeExpression ? `(${timeExpression})` : ctx.cleanNumber(timeValue);
  const originalTimeDisplay = timeOriginalDisplay || formatTimeDisplay(timeOriginalValue, timeOriginalUnit, ctx);
  const classroomRoundedAccelerationValue = getClassroomRoundedAccelerationValue({
    solveFor,
    initialVelocityValue,
    finalVelocityValue,
    timeOriginalValue,
  ...
  const accelerationDisplay = formatAccelerationDisplay({
    value: accelerationValue,
    unit: accelerationUnit,
    classroomRoundedValue: classroomRoundedAccelerationValue,
    ctx
  });
  ...
  const calculationPrompt = isAcceleration
    ? `Now substitute: a = (${ctx.cleanNumber(finalVelocityValue)} - ${ctx.cleanNumber(initialVelocityValue)}) / ${calculationTimeTerm}. What is (${ctx.cleanNumber(finalVelocityValue)} - ${ctx.cleanNumber(initialVelocityValue)}) / ${calculationTimeTerm}?`
  ...
  return buildFormulaWork({
    ...
    variables: [
      ...
      },
      {
        ...
        unit: timeUnit,
        display: timeDisplay,
        hints: timeOriginalUnit && timeOriginalUnit !== timeUnit
          ? [`The problem gives ${originalTimeDisplay}. Because velocity is in ${velocityUnit}, we will convert that to ${timeDisplay} before dividing.`]
          : ['Look for how long the change takes.'],
        ...(timeOriginalUnit && timeOriginalUnit !== timeUnit ? {
          originalValue: timeOriginalValue,
          originalUnit: timeOriginalUnit,
          originalDisplay: originalTimeDisplay,
          conversionDisplay: `${originalTimeDisplay} = ${timeDisplay}`,
          conversionReminder: `Because velocity is in ${velocityUnit}, we will convert that to ${timeDisplay} before dividing.`,
          conversionStepId: 'convert_time',
          skipConversionStepOnConvertedAnswer: true,
          conversionStep: buildTimeConversionStep({
            id: 'convert_time',
            velocityUnit,
            originalValue: timeOriginalValue,
            originalUnit: timeOriginalUnit,
            ...
            convertedUnit: timeUnit,
            convertedDisplay: timeDisplay,
            earlyFinalValues: isAcceleration
              ? [accelerationValue, classroomRoundedAccelerationValue].filter((value) => Number.isFinite(Number(value)))
              : [],
            ctx
          })
        } : {})
    ...
    calculation: {
      ...
      calculationExpression: isAcceleration
        ? `(${ctx.cleanNumber(finalVelocityValue)} - ${ctx.cleanNumber(initialVelocityValue)}) / ${calculationTimeTerm}`
        : `${ctx.cleanNumber(initialVelocityValue)} + ${ctx.cleanNumber(accelerationValue)} * ${ctx.cleanNumber(timeValue)}`,
      acceptedAlternateValues: isAcceleration && Number.isFinite(classroomRoundedAccelerationValue)
        ? [{ value: classroomRoundedAccelerationValue }]
        : [],
      hints: [isAcceleration && Number.isFinite(classroomRoundedAccelerationValue)
        ? `Subtract initial velocity from final velocity, then divide by time. Exact conversion gives ${ctx.cleanNumber(accelerationValue)} ${accelerationUnit}; using 0.0019 hr gives about ${formatRoundedWithCommas(classroomRoundedAccelerationValue)} ${accelerationUnit}.`
        : isAcceleration
        ? 'Subtract initial velocity from final velocity, then divide by time.'
        : 'Multiply acceleration by time, then add the initial velocity.']
    }
  });
}

function getClassroomRoundedAccelerationValue({
  solveFor,
  initialVelocityValue,
  finalVelocityValue,
  timeOriginalValue,
  ...
  if (solveFor !== 'acceleration') return NaN;
  if (velocityUnit !== 'km/hr' || accelerationUnit !== 'km/hr²') return NaN;
  if (timeOriginalUnit !== 's' || timeUnit !== 'hr') return NaN;
  if (!Number.isFinite(Number(timeOriginalValue)) || Number(timeOriginalValue) === 0) return NaN;

  const roundedHours = Number((Number(timeOriginalValue) / 3600).toFixed(4));
  if (!Number.isFinite(roundedHours) || roundedHours === 0) return NaN;
  return (Number(finalVelocityValue) - Number(initialVelocityValue)) / roundedHours;
}

function formatAccelerationDisplay({ value, unit, classroomRoundedValue, ctx }) {
  const primary = formatAccelerationAnswer(value, unit, ctx);
  if (!Number.isFinite(classroomRoundedValue)) return primary;
  return `${primary} (${formatRoundedWithCommas(classroomRoundedValue)} ${unit} using 0.0019 hr)`;
}

function formatRoundedWithCommas(value) {
  if (!Number.isFinite(value)) return String(value);
  return Math.round(value).toLocaleString('en-US');
}

function buildOriginalVelocityFields({
  originalValue,
  originalUnit,
  convertedValue,
  convertedDisplay,
  ...
  if (!originalUnit || originalUnit === targetUnit) return {};
  if (!Number.isFinite(Number(originalValue))) return {};
  if (Number(originalValue) === Number(convertedValue)) return {};

  const originalDisplay = `${ctx.cleanNumber(originalValue)} ${originalUnit}`;
  const conversionExpression = buildVelocityConversionExpression({
    originalValue,
    originalUnit,
    targetUnit,
    ctx
  });
  const conversionStepId = `convert_${label.replace(/\s+/g, '_')}`;
  const conversionReason = buildVelocityConversionReason({
    originalUnit,
    targetUnit,
    timeUnit,
    accelerationUnit
  ...
  return {
    ...
    originalUnit,
    originalDisplay,
    conversionDisplay: `${originalDisplay} = ${convertedDisplay}`,
    conversionReminder: conversionReason,
    conversionStepId,
    conversionStep: buildVelocityConversionStep({
      id: conversionStepId,
      label,
      originalDisplay,
      originalUnit,
...
function buildVelocityConversionStep({
  id,
  label,
  originalDisplay,
  originalUnit,
  ...
  const expression = conversionExpression || `${originalDisplay} to ${targetUnit}`;
  const conversionTail = conversionExpression
    ? conversionExpression.replace(/^\s*[^×÷*/]+/, '').trim()
    : '';
  const calculationDisplay = conversionTail ? `${originalDisplay} ${conversionTail}` : originalDisplay;
  return {
    id,
    type: 'calculation',
    prompt: conversionExpression
      ? `Convert ${originalDisplay} to ${targetUnit}: ${conversionExpression} = ?`
      : `Convert ${originalDisplay} from ${originalUnit} to ${targetUnit}. What is the converted ${label}?`,
    expectedValue: convertedValue,
    expectedUnit: targetUnit,
    expectedDisplay: convertedDisplay,
    calculationExpression: conversionExpression,
    correctResponse: conversionExpression
      ? `Correct. ${calculationDisplay} = ${convertedDisplay}.`
      : `Correct. ${originalDisplay} = ${convertedDisplay}.`,
    hints: [
      conversionExpression
        ? `${originalDisplay} means ${expression}.`
        : `${originalDisplay} = ${convertedDisplay}.`
    ]
  };
}

function buildVelocityConversionExpression({ originalValue, originalUnit, targetUnit, ctx }) {
  const originalDef = ctx.VELOCITY_UNITS.find((unit) => unit.canonical === originalUnit);
  const targetDef = ctx.VELOCITY_UNITS.find((unit) => unit.canonical === targetUnit);
  if (!originalDef?.distanceUnit || !originalDef?.perTimeUnit || !targetDef?.distanceUnit || !targetDef?.perTimeUnit) {
    return '';
  }

  const distanceFactor = ctx.convertDistance(1, originalDef.distanceUnit, targetDef.distanceUnit);
  const timeFactor = ctx.convertTime(1, originalDef.perTimeUnit, targetDef.perTimeUnit);
  if (!Number.isFinite(distanceFactor) || !Number.isFinite(timeFactor) || timeFactor === 0) return '';

  const pieces = [ctx.cleanNumber(originalValue)];
  if (distanceFactor !== 1) pieces.push('×', ctx.cleanNumber(distanceFactor));
  if (timeFactor !== 1) pieces.push('÷', ctx.cleanNumber(timeFactor));
  return pieces.join(' ');
}

function buildVelocityConversionReason({ originalUnit, targetUnit, timeUnit, accelerationUnit }) {
  if (timeUnit === 's' && accelerationUnit === 'm/s²' && targetUnit === 'm/s') {
    return `Because time is in seconds and acceleration is in m/s², we convert ${originalUnit} to ${targetUnit}.`;
  }
  return `Because the formula uses ${targetUnit}, we convert ${originalUnit} to ${targetUnit}.`;
}

function buildConvertedTimeExpression({ originalValue, originalUnit, convertedUnit, ctx }) {
  const divisor = timeConversionDivisor(originalUnit, convertedUnit);
  if (!divisor) return '';
  return `${ctx.cleanNumber(originalValue)} / ${ctx.cleanNumber(divisor)}`;
}
