...
function tryAccelerationFromVelocity(text, lower, ctx) {
  ...
  const viForCalculation = vi || (/\b(from rest|at rest|starts?\s+from\s+rest|started\s+from\s+rest|begins?\s+from\s+rest)\b/.test(lower)
    ? {
      value: 0,
      unit: acceleration && acceleration.distanceUnit && acceleration.perTimeUnit
  ...
  if (target === 'acceleration' && vi && vf && time && time.value !== 0) {
    ...
    const formulaWork = buildAccelerationFormulaWork({
      ...
      initialVelocityValue: viMS,
      finalVelocityValue: vfMS,
      timeValue: tS,
