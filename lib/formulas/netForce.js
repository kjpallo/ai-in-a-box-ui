...
function looksLikeNetForceProblem(lower) {
  ...
  if (asksOnlyForBalancedUnbalancedClassification(lower)) return false;
...
function asksOnlyForBalancedUnbalancedClassification(lower) {
  if (!/\b(?:balanced\s+or\s+unbalanced|unbalanced\s+or\s+balanced)\b/.test(lower)) return false;
  return !/\b(?:calculate|find|solve|determine|compute|what\s+is\s+(?:the\s+)?(?:net\s+)?force|acceleration|mass|f\s*=|formula)\b/.test(lower);
}
