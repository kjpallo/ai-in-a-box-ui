...
function buildDiagramText({ forces, net, total, finalDirection, balanced, axis, ctx }) {
  ...
  if (sameDirection) {
    return [
      ...canceledAxes,
      `[box] ${arrowForSign(activeForces[0].direction.sign)} ${forceLabels.join(' + ')}`,
      netLine,
      status
    ].join('\n');
  }
...
function buildNetForceFormulaWork({ forces, net, finalDirection, balanced, canceledAxes = [], answerLines, diagramText, ctx }) {
