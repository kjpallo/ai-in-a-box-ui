...
function findExplicitDirection(text, start, end) {
  ...
  let match = new RegExp(`\\b(?:(?:push(?:es|ed|ing)?|pull(?:s|ed|ing)?|acting|acts|pointing|points)\\s+)?(?:to\\s+the\\s+|toward\\s+the\\s+|towards\\s+the\\s+|in\\s+the\\s+direction\\s+of\\s+)?(${DIRECTION_WORDS.join('|')})\\b`).exec(after);
