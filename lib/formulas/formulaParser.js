...
const DISTANCE_UNITS = [unit('mile', null, null, ['miles', 'mile', 'mi']), unit('km', null, null, ['kilometers', 'kilometer', 'km']), unit('m', null, null, ['meters', 'meter', 'm']), unit('ft', null, null, ['feet', 'foot', 'ft']), unit('cm', null, null, ['centimeters', 'centimeter', 'cm']), unit('block', null, null, ['blocks', 'block'])];
const TIME_UNITS = [unit('hr', null, null, ['hours', 'hour', 'hrs', 'hr']), unit('min', null, null, ['minutes', 'minute', 'mins', 'min', 'minets']), unit('s', null, null, ['seconds', 'second', 'secs', 'sec', 's'])];
...
const ACCEL_UNITS = [
  unit('m/s^2', 'm', 's', ['meters per second squared', 'meter per second squared', 'm/s^2', 'm/s²', 'm/s2', 'm/s/s', 'm/sec/sec', 'm/sec^2', 'm/sec²', 'm/sec2']),
  unit('ft/s^2', 'ft', 's', ['feet per second squared', 'foot per second squared', 'ft/s^2', 'ft/s²', 'ft/s2', 'ft/s/s', 'ft/sec/sec', 'ft/sec^2', 'ft/sec²', 'ft/sec2']),
  unit('km/hr^2', 'km', 'hr', ['kilometers per hour squared', 'kilometer per hour squared', 'km/hr^2', 'km/hr²', 'km/hr2', 'km/h^2', 'km/h²', 'km/h2'])
];
const DENSITY_UNITS = [unit('g/mL', null, null, ['g/ml', 'g/mL', 'grams per milliliter', 'gram per milliliter', 'g/cm^3', 'g/cm³', 'g/cm3'])];
...
const NUMBER_PATTERN = '-?(?:(?:\\d{1,3}(?:,\\d{3})+|\\d+)(?:\\.\\d+)?|\\.\\d+)(?:\\s*[x×]\\s*10\\s*\\^\\s*-?\\d+|e[+-]?\\d+)?';
const NUMBER_UNIT_SEPARATOR = '\\s*-?\\s*';
...
function findExplicitQuantity(text, labels, unitDefs, defaultUnit) {
  ...
  if (naturalLabels.length) {
    ...
    let match = new RegExp(`\\b(?:${labelPattern})\\b\\s*(?:is|=|:|of|as|equals?)?\\s*(${number})(?:${NUMBER_UNIT_SEPARATOR}(${unitPattern}))?${unitEnd}`, 'i').exec(text);
    if (match) return buildQuantity(match, unitDefs, defaultUnit);

    match = new RegExp(`(${number})(?:${NUMBER_UNIT_SEPARATOR}(${unitPattern}))?${unitEnd}\\s*(?:of\\s+)?(?:${labelPattern})\\b`, 'i').exec(text);
    if (match) return buildQuantity(match, unitDefs, defaultUnit);
  }

  if (symbolLabels.length) {
    const symbolPattern = symbolLabels.map(escapeRegex).join('|');
    const match = new RegExp(`\\b(?:${symbolPattern})\\b\\s*(?:=|:)\\s*(${number})(?:${NUMBER_UNIT_SEPARATOR}(${unitPattern}))?${unitEnd}`, 'i').exec(text);
    if (match) return buildQuantity(match, unitDefs, defaultUnit);
  }
...
function findNumberWithUnit(text, unitDefs) {
  const unitPattern = unitDefs.flatMap((def) => def.names).map(escapeRegex).sort((a, b) => b.length - a.length).join('|');
  const match = new RegExp(`(${NUMBER_PATTERN})${NUMBER_UNIT_SEPARATOR}(${unitPattern})${unitEndBoundary()}`, 'i').exec(text);
  if (!match) return null;
  return buildQuantity(match, unitDefs, null);
}

function findNumberOrArticleWithUnit(text, unitDefs) {
  return findNumberWithUnit(text, unitDefs) || findArticleWithUnit(text, unitDefs);
}

function findArticleWithUnit(text, unitDefs) {
  const unitPattern = unitDefs.flatMap((def) => def.names).map(escapeRegex).sort((a, b) => b.length - a.length).join('|');
  const match = new RegExp(`\\b(a|an|one)\\s+(${unitPattern})${unitEndBoundary()}`, 'i').exec(text);
  if (!match) return null;
  return buildQuantity([match[0], '1', match[2]], unitDefs, null, match.index);
}
...
function parseQuantityNumber(value) {
  const normalized = String(value || '')
    ...
    .replace(/\s+/g, ' ')
    .trim();
  const scientificProduct = /^(-?(?:\d+(?:\.\d+)?|\.\d+))\s*[x×]\s*10\s*\^\s*(-?\d+)$/i.exec(normalized);
  if (scientificProduct) {
    return Number(scientificProduct[1]) * (10 ** Number(scientificProduct[2]));
  }
  return Number(normalized);
}

function buildQuantity(match, unitDefs, defaultUnit, startIndex = match.index) {
...
function normalizeNumberWords(value) {
  ...
  const belowHundredPattern = [
    ...Object.keys(teens),
    ...Object.keys(tens),
    ...Object.keys(ones)
  ].join('|');
  text = text.replace(new RegExp(`\\b(${belowHundredPattern})\\s+hundred(?:\\s+(?:and\\s+)?(${belowHundredPattern}))?\\b`, 'gi'), (_match, hundredWord, remainderWord) => {
    const hundredValue = numberWordValue(hundredWord, ones, teens, tens);
    const remainderValue = remainderWord ? numberWordValue(remainderWord, ones, teens, tens) : 0;
...
function numberWordValue(word, ones, teens, tens) {
  const normalized = String(word || '').toLowerCase();
  if (Object.prototype.hasOwnProperty.call(ones, normalized)) return ones[normalized];
  if (Object.prototype.hasOwnProperty.call(teens, normalized)) return teens[normalized];
