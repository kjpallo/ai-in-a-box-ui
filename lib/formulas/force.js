...
function tryForce(text, lower, ctx) {
  ...
  if (target === 'force' && massForForce && acceleration) {
    ...
    const formulaWork = buildForceFormulaWork(m, acceleration.value, value, ctx);
...
function buildForceFormulaWork(massValue, accelerationValue, forceValue, ctx) {
    ...
    steps: [
      {
        ...
        expected: 'F = m × a',
        hints: ['This problem gives mass and acceleration.']
      },
      {
        id: 'identify_mass',
        type: 'quantity',
