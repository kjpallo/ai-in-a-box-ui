...
function tryAtomicStructureFormula(text, lower) {
  ...
  return tryIsotopeNotation(text, lower) ||
    ...
    tryAtomicNumberFromProtons(text, lower) ||
...
function tryProtonsFromAtomicNumber(text, lower) {
  ...
  if (/\belectrons?\b/.test(lower)) {
    answerLines.push(
      'Atomic number alone does not determine the electron count unless the atom is neutral.',
      `If this atom is neutral, Electrons = ${cleanNumber(atomicNumber)}.`
    );
  }

  return answer('Recognized atomic structure problem: protons from atomic number.', answerLines, buildAtomicNumberFormulaWork({
...
function tryAtomicNumberFromProtons(text, lower) {
  if (!/\batomic number\b/.test(lower)) return null;
  if (!/\b(?:how many|calculate|find|determine|solve|what is|whats|has)\b/.test(lower)) return null;
  const protons = findProtons(text);
  if (protons == null) return null;

  return answer('Recognized atomic structure problem: atomic number from protons.', [
    'Use the atomic number rule: atomic number = number of protons.',
    `Atomic number = ${cleanNumber(protons)}`
  ], buildAtomicNumberFormulaWork({
    originalQuestion: text,
    formulaId: 'unit7.atomic_number.from_protons',
    solveFor: 'atomic number',
