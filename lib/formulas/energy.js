...
function findHeightQuantities(text) {
  ...
  const groundLevel = /\b(?:on|at)\s+(?:the\s+)?ground(?:\s+level)?\b/i.exec(text);
  if (groundLevel) {
    const quantity = quantityFromRawUnit('0', 'm', DISTANCE_UNITS);
    quantity.start = groundLevel.index;
    quantity.end = groundLevel.index + groundLevel[0].length;
    quantity.inferredFrom = 'ground_level';
    results.push(quantity);
  }
