...
function tryFriction(text, lower, ctx) {
  ...
  const rollingFormulaRequest = answerRollingFrictionFormulaRequest(lower, ctx);
  if (rollingFormulaRequest) return rollingFormulaRequest;
...
function answerRollingFrictionFormulaRequest(lower, ctx) {
  if (!/\brolling\s+friction\b/.test(lower)) return null;
  if (!/\b(?:how\s+(?:do|would|can)\s+i\s+solve|solve\s+for|formula\s+for|calculate|find|determine)\b/.test(lower)) return null;

  return ctx.answer('Recognized rolling friction formula question, but more information is needed.', [
    'Use the rolling-friction formula:',
    '',
    'F_rolling = μ_r × F_N',
    '',
    'I need the coefficient of rolling friction, μ_r, and the normal force, F_N, to solve it.'
  ]);
}
