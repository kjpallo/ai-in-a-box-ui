...
function answerSymbolMeaning(text) {
  const match = /\bwhat\s+does\s+([a-z])\s+(?:stand\s+for|mean)\b/.exec(text);
...
function answerAmbiguousSymbolMeaning(text) {
  ...
  if (symbol === 'v') {
    ...
    return {
      intent: 'symbol_only',
      answer: [
        'V can mean more than one thing:',
        '',
        '1. Volume — how much space something takes up',
        '2. Voltage — electrical push in a circuit',
        '3. Velocity or speed — how fast something moves',
        '',
        'Which one are you working on?'
      ].join('\n'),
