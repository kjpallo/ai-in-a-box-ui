const { applyRoutingAliases } = require('./classifyQuestion');
...
const NARROW_FACTS = [
  ...
  },
  {
    id: 'temperature',
    names: ['temperature'],
    units: 'degrees Celsius (°C) in class; Kelvin (K) is the SI unit'
  },
  {
    id: 'work',
    names: ['work'],
...
function answerNarrowIntent(message) {
  ...
  const conceptualAnswer = answerTrustedConceptualQuestion(text);
  if (conceptualAnswer) return conceptualAnswer;
  ...
  if (hasFormulaIntent(text) && fact.formula) {
    return {
      intent: 'formula_only',
      answer: fact.id === 'density' ? `Density formula: density = mass / volume, or ${fact.formula}.` : `${fact.formula}.`,
...
function detectAnswerRepresentationIntent(message) {
  const text = normalize(message);
  const requestedRepresentation = detectRequestedRepresentation(text);
  const requestedLearningShape = detectRequestedLearningShape(text);
  const requestedInteractionMode = requestedLearningShape === 'flashcards' && hasInteractiveFlashcardIntent(text)
    ? 'interactive'
    : '';
  ...
  const intent = {
    ...
    shouldAskRepresentationFollowup: shouldAskRepresentationFollowup(text, requestedRepresentation)
  };
...
function detectRequestedRepresentation(text) {
  if (!text) return 'default';

  if (hasImageRequestIntent(text)) return 'image_request';
  if (hasFormulaStepsIntent(text)) return 'formula_steps';
  if (hasComparisonIntent(text)) return 'comparison';
  if (hasNumberedListIntent(text)) return 'numbered_list';
  if (hasParagraphIntent(text)) return 'paragraph';
  return 'default';
}

function detectRequestedLearningShape(text) {
  if (!text) return null;

  if (hasFlashcardIntent(text)) return 'flashcards';
  if (hasExamplesNonExamplesIntent(text)) return 'examples_non_examples';
  if (hasCommonMistakesIntent(text)) return 'common_mistakes';
  if (hasCompareContrastIntent(text)) return 'compare_contrast';
  return null;
}

function hasFlashcardIntent(text) {
  return /\bflash\s*cards?\b/.test(text) ||
    /\bstudy\s+cards?\b/.test(text) ||
    /\bquizlet\s+style\b/.test(text) ||
    /\bmake\s+cards?\s+for\b/.test(text);
}

function hasInteractiveFlashcardIntent(text) {
  return /\bstart\s+flash\s*cards?\b/.test(text) ||
    /\bstart\s+flash\s*card\s+practice\b/.test(text) ||
    /\bpractice\s+flash\s*cards?\b/.test(text) ||
    /\bstudy\s+with\s+flash\s*cards?\b/.test(text) ||
    /\bquiz\s+me\s+with\s+flash\s*cards?\b/.test(text) ||
    /\binteractive\s+flash\s*cards?\b/.test(text) ||
    /\bgo\s+through\s+flash\s*cards?\b/.test(text) ||
    /\bone\s+flash\s*card\s+at\s+a\s+time\b/.test(text);
}

function hasExamplesNonExamplesIntent(text) {
  return /\bexamples?\b/.test(text) ||
    /\bnon[-\s]?examples?\b/.test(text) ||
    /\bgive\s+me\s+examples?\b/.test(text) ||
    /\bexamples?\s+and\s+non[-\s]?examples?\b/.test(text);
}

function hasCommonMistakesIntent(text) {
  return /\bcommon\s+mistakes?\b/.test(text) ||
    /\bmistakes?\b/.test(text) ||
    /\bwhat\s+do\s+students\s+get\s+wrong\b/.test(text) ||
    /\bwhat\s+should\s+i\s+not\s+do\b/.test(text) ||
    /\bwatch\s+out\s+for\b/.test(text);
}

function hasCompareContrastIntent(text) {
  return hasComparisonIntent(text) ||
    /\bcontrast\b/.test(text);
}

function hasImageRequestIntent(text) {
  return /\b(?:picture|diagram|image)\b/.test(text) ||
    /\bshow\s+me\b/.test(text);
}

function hasFormulaStepsIntent(text) {
  return /\bhow\s+(?:do|would|can)\s+i\s+solve\b/.test(text) ||
    /\bsolve\s+for\b/.test(text) ||
    /\bformula\s+for\b/.test(text);
}

function hasComparisonIntent(text) {
  return /\b(?:vs|versus|compare)\b/.test(text) ||
    /\bdifference\s+between\b/.test(text);
}

function hasNumberedListIntent(text) {
  return /\blist\b/.test(text) ||
    /\bwhat\s+are\b/.test(text) ||
    /\bname\s+the\b/.test(text) ||
    /\btypes?\s+of\b/.test(text) ||
    /\blaws?\s+of\b/.test(text) ||
    /\b\d+\s+factors?\b/.test(text) ||
    /\bsteps?\b/.test(text);
}

function hasParagraphIntent(text) {
  return /\bwhat\s+is\b/.test(text) ||
    /\bwhat\s+does\b.+\bmean\b/.test(text) ||
    /\b(?:define|describe|explain|summarize)\b/.test(text);
}

function shouldAskRepresentationFollowup(text, requestedRepresentation) {
  if (requestedRepresentation !== 'default') return false;
  return /^tell\s+me\s+about\s+[a-z0-9\s'-]+$/.test(text);
}

function buildImageRequestMetadata(message, topic = '') {
  const intent = detectAnswerRepresentationIntent(message);
  if (intent.requestedRepresentation !== 'image_request') return null;

  return {
    ...intent,
    needsImageAsset: true,
    imageQuery: {
      topic: topic || detectImageTopic(normalize(message)),
      source: 'local_classroom_assets'
    }
  };
}

function detectImageTopic(text) {
  const withoutRequestWords = String(text || '')
    .replace(/\b(?:can\s+you\s+)?(?:show\s+me|give\s+me)\b/g, ' ')
    .replace(/\b(?:a|an|the|picture|diagram|image|of|for)\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();

  return withoutRequestWords || 'classroom topic';
}

function answerTrustedConceptualQuestion(text) {
  if (asksWhatSlowsOrResistsCurrent(text)) {
    return {
      intent: 'definition',
      answer: 'Electrical resistance, or resistance, is what slows or resists electric current. Resistance is measured in ohms (Ω). Higher resistance means less current flows for the same voltage.',
      notes: 'Answered student wording for what slows or resists electric current.'
    };
  }

  if (/\bspeed\s+of\s+light\b/.test(text)) {
    return {
      intent: 'science_concept',
      answer: 'In a vacuum, the speed of light is about 3.0 × 10^8 m/s, or about 300,000,000 m/s.',
      notes: 'Answered trusted local fact for speed of light.'
    };
  }

  if (/\btransverse\s+waves?\b/.test(text) && hasDefinitionIntent(text)) {
    return {
      intent: 'definition',
      answer: 'A transverse wave is a wave where particles of the medium move perpendicular, or at right angles, to the direction the wave travels.',
      notes: 'Answered trusted local definition for transverse wave.'
    };
  }

  if (/\bwhy\s+does\s+more\s+mass\s+make\s+it\s+harder\s+to\s+accelerate\b/.test(text)) {
    return {
      intent: 'science_concept',
      answer: 'More mass means more inertia, so more force is needed to change its motion or make it accelerate.',
      notes: 'Answered trusted local conceptual relationship between mass, inertia, and acceleration.'
    };
  }

  if (/\bhow\s+are\s+voltage\s+current\s+and\s+resistance\s+related\b/.test(text)) {
    return {
      intent: 'science_concept',
      answer: 'Ohm’s Law connects them: voltage pushes current, resistance opposes current, and V = I × R.',
      notes: 'Answered trusted local conceptual relationship for Ohm’s law.'
    };
  }

  if (/\bhow\s+are\s+wavelength\s+frequency\s+and\s+wave\s+speed\s+related\b/.test(text)) {
    return {
      intent: 'science_concept',
      ...
      notes: 'Answered trusted local conceptual relationship for waves.'
    };
  }

  if (/\bhow\s+are\s+force\s+and\s+energy\s+related\s+to\s+motion\b/.test(text)) {
    return {
      intent: 'science_concept',
      answer: 'Force can change an object’s motion by speeding it up, slowing it down, or changing its direction. Energy is connected to motion because moving objects have kinetic energy, and forces can transfer energy to or from an object.',
      notes: 'Answered trusted local conceptual relationship between force, energy, and motion.'
    };
  }

  if (/\bhow\s+are\s+work\s+and\s+energy\s+related\b/.test(text)) {
    return {
      intent: 'science_concept',
      answer: 'Work is energy transferred when a force moves an object over a distance. When work is done on an object, the object’s energy can change.',
      notes: 'Answered trusted local conceptual relationship between work and energy.'
    };
  }
...
function asksWhatSlowsOrResistsCurrent(text) {
  const mentionsCurrent = /\b(?:current|curent)\b/.test(text);
  const mentionsElectricity = /\b(?:electricity|electrical|electric)\b/.test(text);
  const asksSlows = /\b(?:what|thing)\b/.test(text) && /\bslow(?:s|ed|ing)?\b/.test(text);
  const asksResists = /\b(?:what|thing)\b/.test(text) && /\bresist(?:s|ed|ing)?\b/.test(text);

  return (mentionsCurrent && (asksSlows || asksResists)) ||
    (mentionsElectricity && mentionsCurrent && /\bthing\b/.test(text) && /\bslow(?:s|ed|ing)?\b/.test(text));
}
...
function hasUnitsIntent(text) {
  return /\bunits?\b/.test(text) ||
    ...
    /\bwhat\s+unit\s+(?:do|would|can)\s+(?:you|i)\s+use\b/.test(text);
}
...
function hasAccelerationSolveIntent(text) {
  return /\bhow\s+(?:do|would|can)\s+i\s+(?:solve for|find|calculate)\s+(?:the\s+)?acceleration\b/.test(text);
}
...
function normalize(value) {
  ...
  return applyRoutingAliases(String(value || '')
    .toLowerCase()
    .replace(/[’`]/g, "'")
    .replace(/\s+/g, ' ')
    .replace(/\b(?:vloume|volme|volum|denisty|dencity|dinsity|densitty|densaty|desity|accelleration|accleration|acceration|resitance|eletrical|velosity)\b/g, (term) => typoMap.get(term) || term))
    .trim();
}
