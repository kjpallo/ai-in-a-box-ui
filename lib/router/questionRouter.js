...
const { tryAcidsBasesKnowledge } = require('../knowledge/chemistryAcidsBasesKnowledge');
const {
  shouldBlockMixedTargetValenceQuestion,
  shouldPreferUnit7AtomicStructureKnowledge,
  shouldPreferUnit8BondingForValenceAction,
  tryUnit7AtomicStructureKnowledge
} = require('../knowledge/chemistry/atomicStructure/unit7AtomicStructureKnowledge');
const { tryUnit8BondingKnowledge } = require('../knowledge/chemistry/bonding/unit8BondingKnowledge');
const { tryUnit9ReactionsKnowledge } = require('../knowledge/chemistry/reactions/unit9ReactionsKnowledge');
const { tryPeriodicTable } = require('../knowledge/periodicTableTools');
const { tryUnit1ConversionsNotationKnowledge } = require('../knowledge/science/unit1/unit1ConversionsNotationKnowledge');
const { tryUnit1GraphingDataKnowledge } = require('../knowledge/science/unit1/unit1GraphingDataKnowledge');
const { tryUnit1MeasurementKnowledge } = require('../knowledge/science/unit1/unit1MeasurementKnowledge');
const { tryUnit1SafetyEquipmentKnowledge } = require('../knowledge/science/unit1/unit1SafetyEquipmentKnowledge');
const { tryUnit1ScientificMethodKnowledge } = require('../knowledge/science/unit1/unit1ScientificMethodKnowledge');
const { tryUnit3EnergyKnowledge } = require('../knowledge/physics/energy/unit3EnergyKnowledge');
const { tryUnit6MatterKnowledge } = require('../knowledge/physics/matter/unit6MatterKnowledge');
const { tryMotionForceKnowledge } = require('../knowledge/physics/motion-force/motionForceKnowledge');
const { tryUnit5WavesKnowledge } = require('../knowledge/physics/waves/unit5WavesKnowledge');
const {
...
const { buildQuestionContract } = require('./questionContract');
...
} = require('../tutor/activities/ammoniaBalancingActivity');

function routeStudentQuestion(message, matchedKnowledge = [], questionContract = null) {
  const text = String(message || '').trim();
  const normalized = normalize(text);
  const resolvedQuestionContract =
    questionContract || buildQuestionContract(text);

  const balancingActivity = tryAmmoniaBalancingActivity(text);
  if (balancingActivity) {
    return makeRoute(balancingActivity);
  }

  if (shouldRejectUnsupportedBalancingPrompt(text)) {
    const trustedExistingBalance = tryUnit9ReactionsKnowledge(text);
    if (trustedExistingBalance) return makeRoute(trustedExistingBalance);
    return makeRoute({
      type: 'no_match',
      confidence: 'none',
      toolsUsed: ['unit9_balancing_boundary'],
      notes: 'Unsupported or malformed balancing equation blocked before generic compound and arithmetic routes.',
      directAnswer: 'I do not have a trusted balancing activity or approved answer for that equation yet. Please check the equation with your teacher.',
      aiAllowed: false
    });
  }
  ...
  const teacherApprovedAnswer = tryTeacherApprovedAnswer(
    text,
    resolvedQuestionContract
  );
  if (teacherApprovedAnswer) {
    return makeRoute(teacherApprovedAnswer);
  }

  const earlyNarrowIntent = shouldAnswerNarrowIntentBeforeKnowledge(normalized)
    ? answerNarrowIntent(text)
    : null;
  if (earlyNarrowIntent) {
    return makeRoute({
      type: earlyNarrowIntent.intent,
      confidence: 'strong',
      toolsUsed: ['answer_intent_rules'],
      notes: earlyNarrowIntent.notes,
      directAnswer: earlyNarrowIntent.answer,
      pendingClarification: earlyNarrowIntent.pendingClarification,
      aiAllowed: false
    });
  }

  const earlyDirectDefinitionKnowledge = findEarlyDirectDefinitionKnowledge(normalized, matchedKnowledge);
  if (earlyDirectDefinitionKnowledge) {
    return makeKnowledgeRoute(earlyDirectDefinitionKnowledge, normalized, {
      notes: `Found direct local definition match: ${earlyDirectDefinitionKnowledge.title}. Answering before built-in knowledge.`
    });
  }

  const unit1MeasurementKnowledgeResult = tryUnit1MeasurementKnowledge(text);
  if (unit1MeasurementKnowledgeResult) {
    return makeRoute(unit1MeasurementKnowledgeResult);
  }

  const unit1ConversionsNotationKnowledgeResult = tryUnit1ConversionsNotationKnowledge(text);
  if (unit1ConversionsNotationKnowledgeResult) {
    return makeRoute(unit1ConversionsNotationKnowledgeResult);
  }

  const unit1GraphingDataKnowledgeResult = tryUnit1GraphingDataKnowledge(text);
  if (unit1GraphingDataKnowledgeResult) {
    return makeRoute(unit1GraphingDataKnowledgeResult);
  }

  const unit1ScientificMethodKnowledgeResult = tryUnit1ScientificMethodKnowledge(text);
  if (unit1ScientificMethodKnowledgeResult) {
    return makeRoute(unit1ScientificMethodKnowledgeResult);
  }

  const unit1SafetyEquipmentKnowledgeResult = tryUnit1SafetyEquipmentKnowledge(text);
  if (unit1SafetyEquipmentKnowledgeResult) {
    return makeRoute(unit1SafetyEquipmentKnowledgeResult);
  }
  ...
  const earlyMotionForceKnowledgeResult = shouldPreferMotionForceKnowledgeBeforeFormula(normalized)
    ? tryMotionForceKnowledge(text)
    : null;
  if (earlyMotionForceKnowledgeResult) {
    return makeRoute(earlyMotionForceKnowledgeResult);
  }

  const earlyCircuitDiagramResult = tryCircuitDiagram(text);
  if (earlyCircuitDiagramResult) {
    return makeRoute({
      type: 'circuit_diagram',
      confidence: 'strong',
      toolsUsed: ['circuit_diagram_rules'],
      notes: `Answered local circuit diagram request: ${earlyCircuitDiagramResult.id}.`,
      directAnswer: earlyCircuitDiagramResult.answer,
      diagramText: earlyCircuitDiagramResult.diagramText,
      aiAllowed: false
    });
  }
  ...
  const mixedTargetValenceResult = shouldBlockMixedTargetValenceQuestion(text)
    ? tryUnit7AtomicStructureKnowledge(text)
    : null;
  if (mixedTargetValenceResult) {
    return makeRoute(mixedTargetValenceResult);
  }
  const earlyUnit7AtomicStructureKnowledgeResult = shouldPreferUnit7AtomicStructureKnowledge(text)
    ? tryUnit7AtomicStructureKnowledge(text)
    : null;
  if (earlyUnit7AtomicStructureKnowledgeResult) {
    return makeRoute(earlyUnit7AtomicStructureKnowledgeResult);
  }
  const earlyUnit8BondingKnowledgeResult = shouldPreferUnit8BondingForValenceAction(text)
    ? tryUnit8BondingKnowledge(text)
    : null;
  if (earlyUnit8BondingKnowledgeResult) {
    return makeRoute(earlyUnit8BondingKnowledgeResult);
  }

  const earlyPeriodicIsotopeResult = looksLikePeriodicIsotopeLookup(normalized)
    ? tryPeriodicTable(text)
    : null;
  if (earlyPeriodicIsotopeResult) {
    return makeRoute({
      type: 'periodic_table',
      confidence: 'strong',
      toolsUsed: ['local_periodic_table'],
      notes: earlyPeriodicIsotopeResult.notes,
      directAnswer: earlyPeriodicIsotopeResult.answer,
      aiAllowed: false
    });
  }
  ...
  const unit7AtomicStructureKnowledgeResult = tryUnit7AtomicStructureKnowledge(text);
  if (unit7AtomicStructureKnowledgeResult) {
    return makeRoute(unit7AtomicStructureKnowledgeResult);
  }

  const unit8BondingKnowledgeResult = tryUnit8BondingKnowledge(text);
  if (unit8BondingKnowledgeResult) {
    return makeRoute(unit8BondingKnowledgeResult);
  }

  const unit9ReactionsKnowledgeResult = tryUnit9ReactionsKnowledge(text);
  if (unit9ReactionsKnowledgeResult) {
    return makeRoute(unit9ReactionsKnowledgeResult);
  }

  const acidsBasesKnowledgeResult = tryAcidsBasesKnowledge(text);
  if (acidsBasesKnowledgeResult) {
    return makeRoute(acidsBasesKnowledgeResult);
  }

  const earlyMotionLawKnowledgeResult = looksLikeMotionLawQuestion(normalized)
    ? tryMotionForceKnowledge(text)
    : null;
  if (earlyMotionLawKnowledgeResult) {
    return makeRoute(earlyMotionLawKnowledgeResult);
  }
  ...
  const unit6MatterKnowledgeResult = tryUnit6MatterKnowledge(text);
  if (unit6MatterKnowledgeResult) {
    return makeRoute(unit6MatterKnowledgeResult);
  }

  const unit5WavesKnowledgeResult = tryUnit5WavesKnowledge(text);
  if (unit5WavesKnowledgeResult) {
    return makeRoute(unit5WavesKnowledgeResult);
  }

  const shouldPreferEnergyKnowledge =
    resolvedQuestionContract?.candidateTopics?.[0] === 'energy';
  const earlyUnit3EnergyKnowledgeResult = shouldPreferEnergyKnowledge
    ? tryUnit3EnergyKnowledge(text)
    : null;
  if (earlyUnit3EnergyKnowledgeResult) {
    return makeRoute(earlyUnit3EnergyKnowledgeResult);
  }

  const earlyElectricityVocabResult = tryElectricityVocab(text);
  if (earlyElectricityVocabResult) {
    return makeRoute({
      type: earlyElectricityVocabResult.kind === 'electricity_concept' ? 'science_concept' : 'definition',
      confidence: 'strong',
      toolsUsed: ['electricity_magnetism_knowledge_pack'],
      notes: `Answered local electricity/magnetism knowledge question: ${earlyElectricityVocabResult.id}.`,
      directAnswer: earlyElectricityVocabResult.answer,
      aiAllowed: false
    });
  }

  const unit3EnergyKnowledgeResult = tryUnit3EnergyKnowledge(text);
  if (unit3EnergyKnowledgeResult) {
    return makeRoute(unit3EnergyKnowledgeResult);
  }
...
function findEarlyDirectDefinitionKnowledge(normalized, matchedKnowledge = []) {
  if (!looksLikeDefinitionQuestion(normalized)) return null;
  if (looksLikeNumericFormulaQuestion(normalized) || looksLikeFormulaProblem(normalized)) return null;

  return matchedKnowledge.find((item) =>
    String(item.category || '') === 'definition' &&
    (item.exactTermMatch || item.exactTitleMatch) &&
    isStrongKnowledgeMatch(item)
  ) || null;
}

function shouldAnswerNarrowIntentBeforeKnowledge(normalized) {
  if (looksLikeWaveMeasurementBoundaryQuestion(normalized)) return false;

  return /\bformula\s+for\b/.test(normalized) ||
    /\bwhat\s+is\s+the\s+formula\b/.test(normalized) ||
    /\bwhat\s+formula\s+(?:do|would|can)\s+i\s+use\b/.test(normalized) ||
    /\bunits?\b/.test(normalized) ||
    /\b(?:measured|mesured)\s+in\b/.test(normalized) ||
    /\bwhat\s+unit\s+is\b/.test(normalized) ||
    /\bwhat\s+unit\s+(?:do|would|can)\s+(?:you|i)\s+use\b/.test(normalized) ||
    /\bsymbol\s+for\b/.test(normalized) ||
    /\bwhat\s+is\s+the\s+symbol\b/.test(normalized) ||
    /\bwhat\s+does\s+[a-z]\s+(?:stand\s+for|mean)\b/.test(normalized) ||
    /\bwhat\s+is\s+[a-z](?:\s+in\b|[?.!]*$)/.test(normalized) ||
    /\bhow\s+(?:do|would|can)\s+i\s+solve\s+for\s+(?:the\s+)?(?:electrical\s+|electric\s+)?power\b/.test(normalized);
}

function looksLikeWaveMeasurementBoundaryQuestion(normalized) {
  return /\bwavelength\b/.test(normalized) &&
    /\bmeasured\b/.test(normalized) &&
    /\b(?:transverse|longitudinal)\s+wave\b/.test(normalized);
}
...
function looksLikeNumericFormulaQuestion(normalized) {
  ...
  const hasFormulaVocabulary = /\b(?:acceleration|accelerates?|accelerated|accelerating|speed|velocity|final\s+velocity|final\s+speed|distance|displacement|time|force|mass|weight|work|power|energy|thermal\s+energy|specific\s+heat|temperature|density|volume|momentum|height|atomic\s+number|mass\s+number|protons?|neutrons?|nuetrons?|newtrons?|electrons?|electons?|isotopes?|bohr|voltage|volts?|current|amps?|amperes?|resistance|ohms?|oms|watts?)\b/.test(normalized);
  const hasFormulaAsk = /\b(?:find|calculate|alculate|solve|determine)\b/.test(normalized) ||
    /\bwhat\s+(?:is|are|will|would|was|were)\b/.test(normalized) ||
    /\bhow\s+(?:far|fast|high|long|much|many)\b/.test(normalized) ||
    /\bat\s+what\s+height\b/.test(normalized) ||
...
function looksLikePeriodicIsotopeLookup(normalized) {
  const text = String(normalized || '');
  if (!/\b[A-Za-z]+\s*-\s*\d{1,3}\b/.test(text)) return false;
  if (/\batomic\s+number\b|\bmass\s+number\b|\b\d+\s+protons?\b/.test(text)) return false;
  if (/\bneutrons?\b/.test(text)) return false;
  return /\b(?:electrons?|protons?)\b/.test(text);
}

function looksLikeMotionLawQuestion(normalized) {
  return /\b(?:what|which)\s+(?:newton\s*)?law\b/.test(normalized) ||
    /\bnewton(?:\s*s|'s|s)?\s+law\b/.test(normalized);
}
...
function makeKnowledgeRoute(bestKnowledge, normalized, options = {}) {
  ...
  const approvedKnowledge = isApprovedKnowledgeItem(bestKnowledge);
  const reviewedApprovedKnowledge = isReviewedApprovedKnowledgeItem(bestKnowledge);
  const toolsUsed = reviewedApprovedKnowledge
    ? ['teacher_facts', 'approved_teacher_content']
    : ['teacher_facts'];
  const evidenceQuality = resolveKnowledgeEvidenceQuality(
    bestKnowledge,
    isStrong,
    approvedKnowledge
  );
  ...
  return makeRoute({
    ...
    answerTopics: bestKnowledge.knowledgeTopics || bestKnowledge.topics || [],
    answerConcepts: [
      bestKnowledge.targetConcept,
      bestKnowledge.title,
      ...(bestKnowledge.aliases || []),
      ...(bestKnowledge.terms || [])
    ].filter(Boolean),
    evidence: {
      quality: evidenceQuality,
      trust: reviewedApprovedKnowledge
        ? 'teacher_approved'
        : approvedKnowledge
          ? 'unreviewed_approved_content'
          : 'local_teacher_fact',
      source: bestKnowledge.source || '',
      knowledgeId: bestKnowledge.id || '',
      reviewStatus: bestKnowledge.reviewStatus || bestKnowledge.reviewState || '',
      sourceQuality: bestKnowledge.sourceQuality || bestKnowledge.confidence || '',
      provenance: bestKnowledge.provenance || null,
      score: Number(bestKnowledge.score || 0),
      exactTermMatch: Boolean(bestKnowledge.exactTermMatch),
      exactTitleMatch: Boolean(bestKnowledge.exactTitleMatch),
      strongestPhraseWordCount: Number(bestKnowledge.strongestPhraseWordCount || 0),
      topics: bestKnowledge.knowledgeTopics || bestKnowledge.topics || [],
      concepts: [
        bestKnowledge.targetConcept,
        bestKnowledge.title,
        ...(bestKnowledge.aliases || []),
...
function shouldPreferMotionForceKnowledgeBeforeFormula(normalized) {
  return /\bspeedometer\b/.test(normalized) && /\bspeed\b/.test(normalized) ||
    /\b\d+(?:\.\d+)?\s*(?:m\/s|meters?\s+per\s+second|mi\s*hr|mi\/hr|mph|km\s*hr|km\/hr|km\/h)\s+(?:north|south|east|west|northeast|northwest|southeast|southwest)\b/.test(normalized) ||
    /\bturn\b/.test(normalized) && (/\bconstant\b/.test(normalized) || /\bwhole\s+time\b/.test(normalized)) ||
    /\bdistance\b/.test(normalized) && /\bdisplacement\b/.test(normalized) && (/\bturn\s+around\b/.test(normalized) || /\bstarting\s+point\b/.test(normalized)) &&
      !looksLikeClosedLoopDistanceDisplacementFormula(normalized);
}

function looksLikeClosedLoopDistanceDisplacementFormula(normalized) {
  return /\b(?:what|find|calculate|determine)\b/.test(normalized) &&
    /\bdistance\b/.test(normalized) &&
    /\bdisplacement\b/.test(normalized) &&
    /\b(?:track|lap|around)\b/.test(normalized) &&
    /\b(?:starting\s+(?:point|position)|start(?:ed|ing)?\s+(?:point|position))\b/.test(normalized);
}
...
function guardWeakOrAmbiguousQuestion(normalized) {
  ...
  if (/\bdensity\b/.test(normalized) && /\bonly\s+know\s+(?:mass|volume)\b/.test(normalized)) {
    return {
      type: 'no_match',
      confidence: 'none',
      toolsUsed: ['router_clarification_guard'],
      notes: 'Rejected density calculation with only one density value given.',
      directAnswer: 'I do not have enough values to find density from only one value. For density, I need mass and volume.',
      aiAllowed: false
    };
  }

  if (/\bwave\s+speed\b/.test(normalized) && /\bonly\s+know\s+(?:frequency|wavelength)\b/.test(normalized)) {
    return {
      type: 'no_match',
      confidence: 'none',
      toolsUsed: ['router_clarification_guard'],
      notes: 'Rejected wave speed calculation with only one wave value given.',
      directAnswer: 'I do not have enough values to find wave speed from only one value. For wave speed, I need frequency and wavelength.',
      aiAllowed: false
    };
  }
  ...
  if (/\bexact\s+answer\b/.test(normalized) && /\bwithout\s+showing\s+work\b/.test(normalized)) {
    return {
      type: 'no_match',
      confidence: 'none',
      toolsUsed: ['router_clarification_guard'],
      notes: 'Blocked request for an exact answer without work because no trusted local problem was provided.',
      directAnswer: 'I do not have a trusted local problem to answer exactly. Please send the full question, and I can help using the local rules.',
      aiAllowed: false
    };
  }
