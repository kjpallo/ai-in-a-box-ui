...
function shouldAnswerNarrowIntentBeforeKnowledge(normalized) {
  if (looksLikeUnit1MeasurementUnitQuestion(normalized)) return false;
...
function looksLikeUnit1MeasurementUnitQuestion(normalized) {
  return /\b(?:units?|measured|mesured)\b/.test(normalized) &&
    /\b(?:length|mass|volume|time|temperature)\b/.test(normalized);
}
