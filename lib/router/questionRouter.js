...
function answerKnownComparison(normalized, matchedKnowledge = []) {
  ...
  if (!/\btransverse\b/.test(normalized) || !/\blongitudinal\b/.test(normalized) || !/\bwaves?\b/.test(normalized)) return null;

  const transverse = findMatchedKnowledgeById(matchedKnowledge, 'transverse-waves');
  const longitudinal = findMatchedKnowledgeById(matchedKnowledge, 'longitudinal-waves');
  const transverseExample = firstExample(transverse) || 'ripples on water or waves on a string';
  const longitudinalExample = firstExample(longitudinal) || 'sound waves or waves in a slinky';

  return {
    toolsUsed: ['teacher_facts', 'comparison_rules'],
    notes: 'Answered known comparison: transverse waves vs longitudinal waves.',
    answer: [
      'Transverse and longitudinal waves are different because the medium moves in different directions compared with the wave.',
      `In a transverse wave, particles of the medium move perpendicular, or at right angles, to the direction the wave travels. Examples include ${transverseExample}.`,
      `In a longitudinal wave, particles of the medium move parallel to the direction the wave travels. Examples include ${longitudinalExample}.`
    ].join(' ')
  };
}
