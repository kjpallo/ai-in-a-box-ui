...
function looksLikeNumericFormulaQuestion(normalized) {
  ...
  const hasFormulaVocabulary = /\b(?:acceleration|accelerates?|accelerated|accelerating|speed|velocity|final\s+velocity|final\s+speed|distance|displacement|time|force|mass|weight|work|power|energy|thermal\s+energy|specific\s+heat|temperature|density|volume|momentum|height)\b/.test(normalized);
