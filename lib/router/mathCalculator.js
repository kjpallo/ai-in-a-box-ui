...
function tryMathOnly(message) {
  const basicArithmetic = tryBasicArithmetic(message);
  if (basicArithmetic) return basicArithmetic;
...
function tryBasicArithmetic(message) {
  const text = normalizeBasicArithmeticQuestion(message);
  if (!text) return null;

  const numberPattern = '(-?\\d+(?:\\.\\d+)?)';
  const equalityMatch = text.match(new RegExp(`^${numberPattern}\\s*=\\s*${numberPattern}$`));
  if (equalityMatch) {
    const left = Number(equalityMatch[1]);
    const right = Number(equalityMatch[2]);
    if (!Number.isFinite(left) || !Number.isFinite(right)) return null;

    const displayExpression = `${cleanNumber(left)} = ${cleanNumber(right)}`;
    const isTrue = left === right;
    return {
      rawInput: message,
      expression: `${equalityMatch[1]}=${equalityMatch[2]}`,
      displayExpression,
      value: isTrue ? 1 : 0,
      displayValue: isTrue ? 'true' : 'false',
      answer: `${isTrue ? 'Yes' : 'No'}, ${displayExpression} is ${isTrue ? 'true' : 'false'}.`
    };
  }

  const arithmeticMatch = text.match(new RegExp(`^${numberPattern}\\s*(\\+|-|\\*|/|x|plus|minus|times|multiplied by|divided by)\\s*${numberPattern}$`));
  ...
  const operator = normalizeBasicOperator(arithmeticMatch[2]);
  if (!operator) return null;
  if (operator === '/' && right === 0) return null;

  const value = evaluateBasicArithmetic(left, operator, right);
  if (!Number.isFinite(value)) return null;

  const displayExpression = `${cleanNumber(left)} ${displayBasicOperator(operator)} ${cleanNumber(right)}`;
  const displayValue = cleanNumber(value);
  return {
    rawInput: message,
    expression: `${arithmeticMatch[1]}${operator}${arithmeticMatch[3]}`,
    displayExpression,
    value,
    displayValue,
    answer: `${displayExpression} = ${displayValue}.`
...
function normalizeBasicArithmeticQuestion(message) {
  ...
  text = text.replace(/^(?:please\s+)?(?:what\s+is|what's|calculate|solve|tell me|the answer to)\s+/, '');
  text = text.replace(/^is\s+/, '');
  text = text.replace(/\s+/g, ' ').trim();

  // Keep this parser out of formula territory such as "speed = distance / time".
  if (/[a-wyz]/.test(text.replace(/\b(?:plus|minus|times|multiplied by|divided by|x)\b/g, ''))) return '';

  return text;
}

function normalizeBasicOperator(operator) {
  switch (operator) {
    case '+':
    case 'plus':
      return '+';
...
function evaluateBasicArithmetic(left, operator, right) {
  switch (operator) {
    case '+':
      return left + right;
    case '-':
...
function displayBasicOperator(operator) {
  switch (operator) {
    case '*':
      return '×';
    case '/':
