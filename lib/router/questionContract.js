...
const UNIT_ALIASES = Object.freeze([
  ...
  { canonical: 'kg', source: '\\b(?:kg|kilograms?)\\b' },
  { canonical: 'mg', source: '\\b(?:mg|milligrams?)\\b' },
  { canonical: 'g', source: '\\b(?:grams?|g)\\b' },
  { canonical: 'km', source: '\\b(?:km|kilometers?)\\b' },
  { canonical: 'cm', source: '\\b(?:cm|centimeters?)\\b' },
  { canonical: 'mm', source: '\\b(?:mm|millimeters?)\\b' },
  { canonical: 'm', source: '\\b(?:meters?|metres?|m)\\b' },
  ...
  { canonical: 'min', source: '\\b(?:minutes?|mins?|min)\\b' },
...
function inferRequestedUnits(message, options = {}) {
  ...
  if (targetConcept === 'unit_conversion') {
    const countedUnit = /\bhow many\s+([A-Za-z°²^/]+)\s+are in\b/i.exec(text);
...
function looksLikeCalculation(message, details = {}) {
  ...
  if (
    ...
    /\bhow many\b[^?.]{0,36}\bare in\b/i.test(text)
  ) {
    return true;
  }
  if (
...
function inferNumericalRole(text, start, end, unit) {
  ...
  if (['ms', 's', 'min', 'h', 'year'].includes(unit)) return 'time';
