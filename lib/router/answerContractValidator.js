...
function validateTaskForm({ message, contract, route, answer }) {
  ...
  if (taskType === 'calculation') {
    if (normalizeIdentifier(route.type || route.public?.type) === 'definition') {
      return invalidCheck('calculation_answer_is_definition');
    }
...
function canonicalizeUnit(value) {
  ...
  const compound = /^([^/]+)\/([^/]+)$/.exec(lower);
  if (!compound) return unit;

  const numeratorAliases = {
    foot: 'ft',
    feet: 'ft',
    kilometer: 'km',
    kilometers: 'km',
  ...
  const numerator = numeratorAliases[compound[1]] || compound[1];
  const denominator = denominatorAliases[compound[2]] || compound[2];
  const canonicalCompound = `${numerator}/${denominator}`;
  return canonicalCompound === 'mi/h' ? 'mph' : canonicalCompound;
}
...
function collectAnswerTopics(route, matchedKnowledge, answer) {
  ...
  if (/weight/.test(family)) topics.add('forces');
...
function inferTopicsFromText(value) {
  ...
  const rules = [
    ...
    ['forces', /\b(?:force|weight|balanced|unbalanced|Newton(?:'s|s|’s)?\s+(?:First|Second|Third)\s+Law|friction|inertia)\b/i],
