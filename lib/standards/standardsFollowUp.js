...
function isStandardsFollowUp(message) {
  ...
  if (/\bstandard\b/.test(text) && /\bheating curve\b/.test(text)) return false;
  if (/\bstandard\s+(?:notation|notaton)\b/.test(text)) return false;
  if (/^what is (?:a )?standard(?: unit)?\??$/.test(text) || /^what are standard units?\??$/.test(text)) return false;
...
function formatWhyThisMattersText(whyImportant) {
  return cleanSentence(whyImportant);
}
