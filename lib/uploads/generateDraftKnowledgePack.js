...
function applySourceEvidenceValidation(pack, extraction) {
  ...
  SOURCE_GROUNDING_SECTIONS.forEach((sectionName) => {
    ...
    items.forEach((item) => {
      ...
      item.confidence = 'low';
      item.reviewStatus = result.status === 'unsupported' ? 'rejected' : 'pending';
...
function contentTokens(value) {
  const stopWords = new Set([
    'a', 'an', 'and', 'are', 'as', 'be', 'by', 'can', 'for', 'from', 'how', 'in', 'into', 'is', 'it', 'of', 'or', 'that', 'the', 'this', 'to', 'with', 'when'
  ]);
  return String(value || '')
    .normalize('NFKC')
    .toLowerCase()
  ...
  GENERATED_ITEM_SECTIONS.forEach((sectionName) => {
    ...
    items.forEach((item, index) => {
      ...
      if (item.reviewStatus === 'approved') {
        errors.push(`${sectionName}[${index}].reviewStatus must not be "approved" for generated drafts.`);
      } else if (!['pending', 'rejected'].includes(item.reviewStatus)) {
        errors.push(`${sectionName}[${index}].reviewStatus must be "pending" or "rejected" for generated drafts.`);
      }
