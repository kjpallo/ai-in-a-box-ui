...
function buildCoverageWarnings(details) {
  ...
  if (Number(details.coverageSummary && details.coverageSummary.failedChunks || 0) > 0) {
    const failedChunkCount = Number(details.coverageSummary.failedChunks || 0);
    warnings.push(`${failedChunkCount} section${failedChunkCount === 1 ? '' : 's'} failed after retry.`);
  }
