...
function planTeacherContentImport(input = {}) {
  ...
  const maxSectionsPerBatch = positiveInteger(
    input.maxSectionsPerBatch || settings.maxSectionsPerBatch || settings.maxBatchChunks || settings.importMaxSectionsPerBatch,
    0
  );
  ...
  const warnings = [];
  const ultraSafeAuto = settings.autoAnalyzeUltraSafe === true;
  ...
  const lowMemory = !ultraSafeAuto && memory.availableMemoryMb > 0 && memory.availableMemoryMb < positiveInteger(settings.lowMemoryMb, DEFAULT_LOW_MEMORY_MB);
  ...
  const model = String(input.model || settings.model || settings.importModel || process.env.OLLAMA_IMPORT_MODEL || process.env.OLLAMA_MODEL || '');
  ...
  const lowMemoryMaxCharactersPerBatch = Math.max(1, Math.min(
    maxCharactersPerBatch,
    positiveInteger(
      settings.lowMemoryMaxBatchCharacters || settings.importLowMemoryMaxBatchCharacters,
      Math.max(1, Math.floor(maxCharactersPerBatch / 2))
    )
  ));
  const plannedMaxCharactersPerBatch = lowMemory && maxCharactersPerBatch > lowMemoryMaxCharactersPerBatch
    ? lowMemoryMaxCharactersPerBatch
    : maxCharactersPerBatch;
  const plannedMaxEstimatedTokensPerBatch = Math.max(1, Math.min(
    maxEstimatedTokensPerBatch,
    estimateTokens(plannedMaxCharactersPerBatch)
  ));
  const plannedMaxSectionsPerBatch = lowMemory
    ? 1
    : maxSectionsPerBatch;
  const fullBatches = makeBatches(
    textBearingSections,
    plannedMaxCharactersPerBatch,
    plannedMaxEstimatedTokensPerBatch,
    plannedMaxSectionsPerBatch
  );
  const veryLarge = fullBatches.length > positiveInteger(settings.veryLargeBatchLimit, DEFAULT_VERY_LARGE_BATCH_LIMIT);

  if (lowMemory) {
    warnings.push(`Available memory is low (${memory.availableMemoryMb} MB). Automatically using smaller sequential chunks.`);
  }
  if (!ultraSafeAuto && veryLarge) {
    warnings.push('This upload is very large. Automatic adaptive chunking will process the full document sequentially.');
  }
  ...
  return makePlan({
    ...
    batches: fullBatches,
    limits: makeLimits(plannedMaxCharactersPerBatch, plannedMaxEstimatedTokensPerBatch, memory),
    warnings,
    reason: lowMemory
      ? `Available memory is low, so the planner reduced batch size and will process ${fullBatches.length} sequential chunk${fullBatches.length === 1 ? '' : 's'} automatically.`
...
function makeBatches(sections, maxCharactersPerBatch, maxEstimatedTokensPerBatch, maxSectionsPerBatch = 0) {
  const chunks = [];
  ...
  const sectionLimit = positiveInteger(maxSectionsPerBatch, 0);
  chunks.forEach((chunk) => {
    ...
    const wouldExceedSections = sectionLimit > 0 && current.length >= sectionLimit;
    if (wouldExceedCharacters || wouldExceedTokens || wouldExceedSections) {
