...
const { cleanupExtractedDocument } = require('./cleanupExtractedDocument');
...
async function extractTextFromFile(filePath, options = {}) {
  ...
  try {
    ...
    const cleaned = cleanupExtractedDocument({
      ...extracted,
    ...
    const text = normalizeText(cleaned.text || extracted.text || '');
    const warnings = [
      ...baseResult.warnings,
      ...(extracted.warnings || []),
      ...(cleaned.warnings || []),
    ...
    return {
      ...
      success: true,
      text,
      sections: cleaned.sections || extracted.sections || makeSections(text),
      pages: cleaned.pages || extracted.pages || [],
...
function makeBaseResult(filePath, detection) {
  return {
    ...
    metadata: {
      ...
      detectedBySignature: detection.detectedBySignature === true,
      filenameExtension: detection.filenameExtension || detection.extension || ''
    },
