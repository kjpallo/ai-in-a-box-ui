...
function getTeacherContentDashboard(options = {}) {
  ...
  const approvedPackIds = new Set(approvedResult.packs.map((record) => record.packId).filter(Boolean));
  const visibleDrafts = draftResult.packs.filter((record) => !approvedPackIds.has(record.packId));
...
function listDraftPacksForReview(options = {}) {
  ...
  return {
    draftPacks: draftResult.packs
      .filter((record) => !approvedPackIds.has(record.packId))
...
function collectPackUploadIds(pack) {
  ...
  if (metadata.sourceUpload && metadata.sourceUpload.uploadId) ids.push(metadata.sourceUpload.uploadId);
  (Array.isArray(pack && pack.sourceFiles) ? pack.sourceFiles : []).forEach((sourceFile) => {
    if (sourceFile && sourceFile.uploadId) ids.push(sourceFile.uploadId);
  });
