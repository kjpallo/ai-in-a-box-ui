...
} = require('../knowledge/matchDraftApprovedPacks');
...
const DEFAULT_UPLOAD_EXTRACTED_DIR = path.join(__dirname, '..', '..', 'knowledge', 'uploads', 'extracted');

function getTeacherContentDashboard(options = {}) {
  const draftResult = loadDrafts(options);
  const approvedResult = loadApproved(options);
  const visibleDrafts = draftResult.packs.filter((record) => !hasApprovedCounterpartForDraftVisibility(record, approvedResult.packs));
  const draftSummaries = visibleDrafts.map(makeDraftPackSummary);

  return {
    draftPacks: visibleDrafts.length,
...
function listDraftPacksForReview(options = {}) {
  ...
  return {
    draftPacks: draftResult.packs
      .filter((record) => !hasApprovedCounterpartForDraftVisibility(record, approvedResult.packs))
      .map(makeDraftPackSummary),
    warnings: [
      ...collectWarnings(draftResult.packs),
      ...collectErrorWarnings(draftResult.errors)
    ],
...
function listUploadedSourceHistory(options = {}) {
  const extractedDir = options.uploadExtractedDir || DEFAULT_UPLOAD_EXTRACTED_DIR;
  const draftResult = loadDrafts(options);
  const approvedResult = loadApproved(options);
  const draftMatches = buildUploadPackLookup(draftResult.packs);
  const approvedMatches = buildUploadPackLookup(approvedResult.packs);
  const extractionRecords = loadExtractionRecords(extractedDir);

  return {
    uploadedSources: extractionRecords.records.map((record) => makeUploadedSourceSummary(
      record,
      draftMatches.get(record.uploadId) || [],
      approvedMatches.get(record.uploadId) || []
    )),
    warnings: [
      ...extractionRecords.warnings,
      ...collectErrorWarnings(draftResult.errors),
      ...collectErrorWarnings(approvedResult.errors)
    ],
    errors: [
      ...extractionRecords.errors,
      ...prefixErrors('draft', draftResult.errors),
      ...prefixErrors('approved', approvedResult.errors)
    ]
  };
...
function makeDraftPackSummary(record) {
  ...
  const sourceUploadIds = Array.from(collectPackUploadIds(record.pack));
  const sourceFileNames = listPackSourceFileNames(record.pack);
  const sourceSummary = summarizePackSource(record.pack);
  ...
  return {
    ...
    sourceSummary,
    sourceUploadIds,
    hasStrongSourceMetadata: hasStrongSourceMetadata(record.pack),
    importScope,
...
function makeApprovedPackSummary(record) {
  ...
  const importScope = summarizeImportScope(record.pack);
  const metadata = record.pack && record.pack.metadata && typeof record.pack.metadata === 'object'
    ? record.pack.metadata
    : {};
  const sourceUploadIds = Array.from(collectPackUploadIds(record.pack));
  const combinedSourceDraftPackIds = Array.from(collectCombinedSourceDraftPackIds(record.pack));
  ...
  return {
    ...
    sourceFileNames: listPackSourceFileNames(record.pack),
    sourceSummary: summarizePackSource(record.pack),
    sourceUploadIds,
    combinedSourceDraftPackIds,
    hasStrongSourceMetadata: hasStrongSourceMetadata(record.pack),
    importScope,
    sampleOnly: importScope.sampleOnly === true,
    rangeLimited: importScope.rangeLimited === true,
...
function summarizePackSource(pack) {
  const sourceNames = listPackSourceFileNames(pack);
...
function listPackSourceFileNames(pack) {
  return Array.from(new Set(
    (Array.isArray(pack && pack.sourceFiles) ? pack.sourceFiles : [])
      .map((sourceFile) => sourceFile && (sourceFile.originalFileName || sourceFile.fileName || sourceFile.storedFileName))
      .filter(Boolean)
  ));
}

function loadExtractionRecords(extractedDir) {
  if (!fs.existsSync(extractedDir)) {
    return {
      records: [],
      warnings: [`Upload extraction history directory not found: ${extractedDir}`],
      errors: []
    };
  }
  ...
  const warnings = [];
  const errors = [];
  const records = fs.readdirSync(extractedDir, { withFileTypes: true })
    .filter((entry) => entry.isFile() && entry.name.endsWith('_extraction.json'))
    .map((entry) => {
      const sourcePath = path.join(extractedDir, entry.name);
      try {
        const extraction = JSON.parse(fs.readFileSync(sourcePath, 'utf8'));
        const stat = fs.statSync(sourcePath);
        return makeExtractionHistoryRecord(extraction, {
          sourcePath,
          fileName: entry.name,
        ...
        errors.push({
          kind: 'upload',
          sourcePath,
          errors: [`Upload extraction history could not be read: ${error instanceof Error ? error.message : String(error)}`],
          warnings: []
        });
        return null;
      }
...
function makeExtractionHistoryRecord(extraction = {}, fileInfo = {}) {
  const upload = extraction.upload && typeof extraction.upload === 'object' ? extraction.upload : {};
  const metadata = extraction.metadata && typeof extraction.metadata === 'object' ? extraction.metadata : {};
  ...
  const textBearingPages = Array.isArray(metadata.textBearingPages)
    ? metadata.textBearingPages
    : Array.isArray(metadata.pagesWithText)
      ? metadata.pagesWithText
      : pages.filter((page) => String(page && page.text || '').trim()).map((page) => page.pageNumber);
  const firstTextPage = Number(metadata.firstTextPage || textBearingPages[0] || sections.find((section) => String(section && section.text || '').trim())?.pageNumber || 0);
  const pageCount = Number(metadata.pageCount || pages.length || sections.length || 0);
  const textBearingPageCount = textBearingPages.length || sections.filter((section) => String(section && section.text || '').trim()).length;
  const warnings = Array.isArray(extraction.warnings) ? extraction.warnings.slice() : [];

  if (pageCount > textBearingPageCount) {
    warnings.push(`${pageCount - textBearingPageCount} image-only page/slide${pageCount - textBearingPageCount === 1 ? '' : 's'} may need OCR later.`);
  }

  return {
    uploadId: upload.uploadId || stripExtractionSuffix(fileInfo.fileName),
    originalFileName: upload.originalFileName || extraction.originalFileName || metadata.originalFileName || extraction.fileName || '',
    storedFileName: upload.storedFileName || extraction.fileName || '',
    fileType: metadata.detectedType || extraction.extension || extraction.mimeGuess || '',
    extractedUnitCount: pageCount,
    textBearingUnitCount: textBearingPageCount,
    firstTextBearingUnit: firstTextPage || null,
    characterCount: Number(metadata.characterCount || String(extraction.text || '').length || 0),
    warnings,
    errors: Array.isArray(extraction.errors) ? extraction.errors : [],
    createdAt: metadata.createdAt || '',
...
function buildUploadPackLookup(records) {
  const lookup = new Map();
  records.forEach((record) => {
    collectPackUploadIds(record.pack).forEach((uploadId) => {
      if (!lookup.has(uploadId)) lookup.set(uploadId, []);
      lookup.get(uploadId).push({
        packId: record.packId,
        title: record.title,
        status: record.pack && (record.pack.status || record.pack.reviewStatus) || ''
      });
    });
  });
  return lookup;
...
function makeUploadedSourceSummary(record, draftPacks, approvedPacks) {
  return {
    ...record,
    draftPackExists: draftPacks.length > 0,
    draftPacks,
    approvedPackExists: approvedPacks.length > 0,
    approvedPacks
  };
}

function stripExtractionSuffix(fileName = '') {
  return String(fileName).replace(/_extraction\.json$/, '');
}
  ...
  COUNTED_SECTIONS.forEach((sectionName) => {
    ...
    items.forEach((item) => {
      if (item && REVIEW_STATUSES.includes(item.reviewStatus)) {
        const bucket = item.reviewStatus === 'needs_review' ? 'pending' : item.reviewStatus;
        counts[bucket] += 1;
      }
    });
  });
...
function hasApprovedCounterpartForDraftVisibility(draftRecord, approvedRecords = []) {
  return Boolean(matchApprovedRecordForDraft(draftRecord, approvedRecords));
}
