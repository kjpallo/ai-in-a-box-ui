...
const { hasTeacherLowConfidenceOverride, isLowConfidenceValue } = require('../knowledge/teacherReviewState');
...
} = require('../knowledge/standardsMetadata');
...
function buildImportPipelineReport(options = {}) {
  ...
  const validation = pack
    ? validateKnowledgePack(pack, { standardsBank: options.standardsBank, standardsPaused: true })
    : { valid: false, errors: draftResult.errors, warnings: [] };
  const pendingReview = buildPendingReview(pack);
  const reviewItems = buildReviewItems(pack);
  const draftPacketItems = buildDraftPacketItems(pack);
  ...
  const metadataImportWarnings = getMetadataImportWarnings(pack);
  const invalidGeneratedItemDetails = getInvalidGeneratedTechnicalErrors(pack);
  const coverageResult = safeBuildCoverageReport({
    extraction,
    pack,
  ...
  const coverageReport = coverageResult.coverageReport;
...
function getMetadataImportWarnings(pack) {
  return Array.isArray(pack && pack.metadata && pack.metadata.importWarnings)
    ? pack.metadata.importWarnings.filter(isNonEmptyString)
    : [];
}

function getInvalidGeneratedTechnicalErrors(pack) {
  ...
  return invalidItems
    .map((entry) => {
      ...
      const index = Number.isFinite(Number(entry.index)) ? Number(entry.index) : -1;
      const reason = firstNonEmptyString(entry.reason, 'Invalid generated item.');
      return index >= 0
        ? `Removed ${section}[${index}] during draft salvage: ${reason}`
        : `Removed ${section} item during draft salvage: ${reason}`;
    })
    .filter(isNonEmptyString);
}

function safeBuildCoverageReport(input = {}) {
  try {
    return {
      coverageReport: buildImportCoverageReport(input),
      technicalErrors: []
    };
  } catch (error) {
    return {
      coverageReport: makeFallbackCoverageReport(input.extraction, input.pack),
      technicalErrors: [`Import coverage report failed to load: ${error instanceof Error ? error.message : String(error)}`]
    };
  }
}

function makeFallbackCoverageReport(extraction, pack) {
  const itemCounts = buildItemCounts(pack || {});
  const totalChunks = Array.isArray(extraction && extraction.sections) ? extraction.sections.length : 0;
  ...
  return {
    ...
    pagesWithDraftItems: 0,
    chunksWithDraftItems: 0,
    chunksWithNoExtractedKnowledge: 0,
    noKnowledgeChunks: [],
    sectionsDetected: [],
    sourceSignals: {},
    itemCounts,
    ...
    coverageSummary: {
      ...
      needsReviewChunks: 0,
      failedChunks: 0,
      totalSourceChars: Number(extraction && extraction.text ? String(extraction.text).length : 0),
      completedChunks: totalChunks,
      allChunksTerminal: totalChunks > 0
    },
    queuedChunks: 0,
    allChunksTerminal: totalChunks > 0,
    importComplete: totalChunks > 0,
    warnings: []
  };
}

function buildReviewItems(pack) {
  const grouped = emptySectionGroups();
  if (!pack) return { totalItems: 0, items: grouped };
...
function buildDraftPacketItems(pack) {
  const sections = ['vocabulary', 'concepts', 'referenceFormulas'];
  const result = {};
  sections.forEach((sectionName) => {
    result[sectionName] = Array.isArray(pack && pack[sectionName]) ? pack[sectionName] : [];
  });
  return result;
}
...
function buildDraftPackSummary(pack, validation) {
  ...
  return {
    ...
    plannerNotes: summarizePlannerNotes(pack),
...
function summarizePlannerNotes(pack) {
  const metadata = pack && pack.metadata && typeof pack.metadata === 'object' && !Array.isArray(pack.metadata)
    ? pack.metadata
    : {};
  const plan = metadata.autoImportPlan && typeof metadata.autoImportPlan === 'object' && !Array.isArray(metadata.autoImportPlan)
    ? metadata.autoImportPlan
    : {};
  ...
  return {
    ...
    reason: plan.reason || '',
    warnings,
    teacherNotes: makeTeacherPlannerNotes(warnings, plan)
  };
}
...
function buildPendingReview(pack) {
  ...
  REVIEWABLE_SECTIONS.forEach((sectionName) => {
    const items = Array.isArray(pack[sectionName]) ? pack[sectionName] : [];
    items.forEach((item, index) => {
      if (!item || !isPendingReviewStatus(item.reviewStatus)) return;
      grouped[sectionName].push(makePendingItem(sectionName, index, item));
    });
  });
...
function buildPromotionReadiness(pack, options = {}) {
  ...
  const pendingCount = countPendingReviewStatuses(pack, PROMOTABLE_SECTIONS);
  if (pendingCount > 0) {
  ...
  return {
    ...
    blockedReasons,
    blockerSummary: buildPromotionBlockerSummary(pack, blockedReasons)
  };
}

function buildPromotionBlockerSummary(pack, blockedReasons = []) {
  const categories = new Map();
  let totalItems = 0;

  function add(category, action, count = 1) {
    const key = `${category}|${action}`;
    const existing = categories.get(key) || { category, count: 0, action };
  ...
  PROMOTABLE_SECTIONS.forEach((sectionName) => {
    ...
    items.forEach((item) => {
      ...
      if (isPendingReviewStatus(item.reviewStatus)) {
        totalItems += 1;
        add('pending review', 'Accept valid items or reject/exclude them before approval.');
        return;
      }
      if (item.reviewStatus !== 'approved') return;

      const blockers = getApprovedPromotionBlockerCategories(sectionName, item);
      if (!blockers.length) return;
      totalItems += 1;
      blockers.forEach((category) => {
        add(category, category === 'missing required source information'
          ? 'Edit source-backed fields when available, or reject/exclude the item before approval.'
          : 'Edit the item when available, or reject/exclude it before approval.');
      });
    });
  });

  if (!totalItems && Array.isArray(blockedReasons) && blockedReasons.length) {
    blockedReasons.forEach((reason) => {
      add(categorizePromotionReason(reason), 'Resolve the listed promotion issue, then approve again.');
    });
  }
  ...
  return {
    ...
    message: makePromotionBlockerMessage(totalItems, Array.from(categories.values()))
  };
}

function getApprovedPromotionBlockerCategories(sectionName, item) {
  const categories = [];
  const grounding = item.sourceGrounding && typeof item.sourceGrounding === 'object' && !Array.isArray(item.sourceGrounding)
    ? item.sourceGrounding
    : null;
  if (grounding && (grounding.status !== 'supported' || grounding.termOrTitleFound === false || grounding.explanationSupported === false)) {
    categories.push('missing required source information');
  }
  if (isLowConfidenceValue(item.confidence) && !hasTeacherLowConfidenceOverride(item)) {
    categories.push('low confidence');
  }
  if (sectionName === 'referenceFormulas' && item.solverStatus !== 'reference_only') {
    categories.push('formula safety settings');
  }
  return Array.from(new Set(categories));
}

function categorizePromotionReason(reason) {
  const text = String(reason || '').toLowerCase();
  if (/pending/.test(text)) return 'pending review';
  if (/source|ground|evidence/.test(text)) return 'missing required source information';
  if (/low confidence/.test(text)) return 'low confidence';
  if (/solverstatus|reference_only|formula/.test(text)) return 'formula safety settings';
  return 'promotion requirements';
}

function makePromotionBlockerMessage(totalItems, categories) {
  if (!totalItems || !categories.length) return '';
  const first = categories.sort((a, b) => b.count - a.count)[0];
  return `${totalItems} item${totalItems === 1 ? '' : 's'} ${totalItems === 1 ? 'is' : 'are'} blocking approval because ${first.count} ${first.count === 1 ? 'item is' : 'items are'} in ${first.category}. ${first.action}`;
}
...
function buildReviewCountsBySection(pack) {
  ...
  COUNTED_SECTIONS.forEach((sectionName) => {
    ...
    items.forEach((item) => {
      ...
      const bucket = normalizeReviewStatusForCounts(item.reviewStatus);
      counts[sectionName][bucket] += 1;
    });
  });
  return counts;
}
...
function countPendingReviewStatuses(pack, sections) {
  return sections.reduce((sum, sectionName) => {
    const items = Array.isArray(pack[sectionName]) ? pack[sectionName] : [];
    return sum + items.filter((item) => item && isPendingReviewStatus(item.reviewStatus)).length;
  }, 0);
}
...
function makePendingItem(sectionName, index, item) {
  return {
    section: sectionName,
    index,
    itemId: firstNonEmptyString(item.itemId, item.term, item.conceptId, item.formulaId, item.problemId, item.standardId),
    label: makeItemLabel(sectionName, item),
    reviewStatus: item.reviewStatus || '',
    confidence: item.confidence || '',
    title: firstNonEmptyString(item.title, item.term, item.question, item.equation, item.standardId),
    term: item.term || '',
    question: item.question || '',
    equation: item.equation || '',
    standardId: item.standardId || '',
    draftWording: makeDraftWording(sectionName, item),
    sourceFile: item.sourceFile || '',
    sourceLocation: item.sourceLocation || '',
    sourceTextSnippet: item.sourceTextSnippet || '',
    standards: normalizeStandardsMetadata(item.standards),
    standardsStatusLabel: getStandardsAlignmentLabel(item.standards),
    ...
    sourceGrounding: item.sourceGrounding && typeof item.sourceGrounding === 'object' && !Array.isArray(item.sourceGrounding)
      ? item.sourceGrounding
      : null,
...
function makeDraftWording(sectionName, item) {
  if (!item || typeof item !== 'object') return '';
  if (sectionName === 'vocabulary') {
    return firstNonEmptyString(item.studentDefinition, item.teacherDefinition, item.misconception);
  }
  if (sectionName === 'concepts') {
    return firstNonEmptyString(item.studentExplanation, Array.isArray(item.keyIdeas) ? item.keyIdeas.join(' ') : '');
  }
  if (sectionName === 'referenceFormulas') {
    return firstNonEmptyString(item.equation, item.description);
  }
  if (sectionName === 'problemBank') {
    return firstNonEmptyString(item.question, item.expectedAnswer);
  }
  if (sectionName === 'standardsMap') {
    return firstNonEmptyString(item.title, item.description, item.standardId);
  }
  if (sectionName === 'smokeTests') {
    return firstNonEmptyString(item.prompt, item.expectedAnswer);
  }
  return firstNonEmptyString(item.title, item.question, item.equation, item.standardId);
}

const NON_EDITABLE_REVIEW_FIELDS = new Set([
  'term',
  'title',
  'sourceFile',
  'sourceLocation',
...
function makeEditableFieldValues(sectionName, item) {
  ...
  allowedFields.forEach((fieldName) => {
    if (NON_EDITABLE_REVIEW_FIELDS.has(fieldName)) return;
...
function collectStandardIds(pack) {
  ...
  ['vocabulary', 'concepts', 'problemBank'].forEach((sectionName) => {
    const items = Array.isArray(pack[sectionName]) ? pack[sectionName] : [];
    items.forEach((item) => {
      getLinkedStandardIds(item).forEach((standardId) => {
...
function normalizeReviewStatusForCounts(reviewStatus) {
  return isPendingReviewStatus(reviewStatus) ? 'pending' : reviewStatus;
}
